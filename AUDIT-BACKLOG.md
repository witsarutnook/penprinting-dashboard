...
- [ ] ⏳ **LOG-1-slip-diagnostic-verbose** — diagnostic `console.log` 5 จุดที่เพิ่มตอน debug slip-verify บน prod (`7d32666`): `lib/ai-quote/webhook-router.ts:41` (inbound) `:49` (slip pre-filter verdict) `:53` (thunder result) `:60` (reply sent) + `lib/ai-quote/slip.ts:105` (isSlipImage haiku answer). flow นิ่งแล้ว + `slip_checks` metrics (`2669ee8`) ครอบ observability เดิม → ถอดออกหลัง soak. **คง** `app/api/ai-quote/line/route.ts:62` (handleInbound error log — ของจริง). **Defer reason**: low-risk log noise; รอ slip flow soak 2-3 วัน + ดู `/api/admin/slip-metrics` ว่า `filtered_out` vs `thunder_calls` ทูนถูกก่อนถอด log.
