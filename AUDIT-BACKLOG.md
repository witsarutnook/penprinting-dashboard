...
- [ ] ⏳ **M5-loadsession-idor** — `loadSession` (`lib/ai-quote/db.ts`) ไม่ผูก owner/`created_by` → staff คนใดก็โหลด session ของคนอื่นได้ด้วย sessionId. **คงเปิดไว้ตั้งใจ — shared team inbox คือ behavior ที่ออกแบบไว้ตอน 1a** (admin/sales ภายในเท่านั้น). owner-binding ตอนนี้ = regression ของ shared inbox + ยังไม่มี identity model. **ปิดพร้อม Phase 1b** (LINE OA ลูกค้ามี sessionId ของตัวเอง = IDOR จริง) — ผูก session กับ owner/LINE userId + เช็คก่อนโหลด ตอนออกแบบ LINE identity.
