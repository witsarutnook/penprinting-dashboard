...
- [ ] ⏳ **M3-aiquote-escalation-not-wired** — `lib/ai-quote/db.ts` `markEscalated()` = **dead code (ไม่มี caller)** + escalation lead (กล่อง/ถุง/งานนอก scope) บันทึกเป็น `lead_status` "ใหม่" เฉยๆ ไม่ auto-escalate. route ส่ง `escalated` hint flag กลับ แต่ client ไม่อ่าน → ทีมขายแยก lead escalate กับ lead ปกติด้วยตาไม่ได้. **Decision ต้องตัดสิน**: wire badge "ต้องประเมินเอง" ใน `/quote-leads` (เรียก markEscalated หรือ set status ตอนบันทึก) **หรือ** ลบ dead code ถ้าไม่ทำ.
- [ ] ⏳ **M4-leadclaim-race** — `PATCH /api/ai-quote/leads/[id]` หยิบงาน (`assigned_to`) = last-write-wins เงียบ: 2 staff กดหยิบ lead เดียวกันพร้อมกัน คนหลังทับคนแรกโดยไม่เตือน. **Fix**: conditional `UPDATE ... WHERE assigned_to IS NULL` + คืน 409 ถ้า 0 rows affected (มีคนหยิบไปแล้ว). ยอมรับได้ตอน internal team เล็ก.
- [ ] ⏳ **M5-loadsession-idor** — `loadSession` (`lib/ai-quote/db.ts`) ไม่ผูก owner/`created_by` → staff คนใดก็โหลด session ของคนอื่นได้ด้วย sessionId (shared team inbox = ตั้งใจตอน 1a). **ต้องปิดก่อน Phase 1b** (LINE OA ลูกค้ามี sessionId ของตัวเอง — IDOR จริง): ผูก session กับ owner/LINE userId + เช็คก่อนโหลด.
...
- [ ] ⏳ **L-aiquote-id-array-index** — quote `id` ที่ client อ้างถึง = **array index** ไม่ใช่ DB id (`ai_quotes.id`). harmless ตอน UI ปัจจุบัน แต่จะเพี้ยนถ้า future feature ลบ quote กลางรายการ. swap เป็น DB id ตอนแตะ quote list ครั้งหน้า.
- [ ] ⏳ **L-aiquote-run-comment-typo** — `lib/ai-quote/run.ts:73` comment typo (cosmetic).
- [ ] ⏳ **L-dbmigrate-rowcount-newtables** — `GET /api/admin/db-migrate` row-count summary ไม่นับ table ใหม่ (`ai_quote_sessions`/`ai_quotes`) — count เฉยๆ ไม่กระทบ migration เอง.
