...
>
> Latest update: **2026-05-27** — §12 Step 1-5 Postgres-only ship ([`745d36f`](https://github.com/witsarutnook/penprinting-dashboard/commit/745d36f), -4,968 LOC). Apps Script fallback ตัดทั้งหมดจาก dashboard reads + writes. 17 routes Postgres-direct. Dead code purge: feature-flags.ts + sync-to-sheet.ts + sync-from-sheet.ts + 4 cron routes + 9 admin diagnose/import routes + bench-audit + quota-widget. Gates ผ่าน Node 22 (type-check/lint/120 tests/build). + `/track` shipping-queue active step fix ([`0cb98c3`](https://github.com/witsarutnook/penprinting-dashboard/commit/0cb98c3)) — ใบที่ staff='ship' โชว์ step 5 active ถูก (เคยค้าง step 4). + chore: ลบ generatedAt impure side effect (`3da9266`). ใหม่: **OPEN — UI-1 hydration warnings /board** (pre-existing since 5/21, ไม่ block ship — ดู Low section).
>
