...
- [x] **2.3** ✅ **PUBLISHED 2026-07-09** — คุณนุ๊กสั่ง "กดเลย", Claude กดผ่าน Chrome MCP: dialog ยืนยัน **"Your app was successfully published. Your app is now available for the public to use."** + badge Publish → **Published** + ปุ่มเปลี่ยนเป็น Unpublish (rollback มีจริง)
  - ✅ **verify non-tester ผ่าน (2026-07-09 same-day)** — บัญชีคนจริง (ไม่ใช่ tester) ทัก "ทดสอบๆ" → hint copy ใหม่ขึ้น → กดปุ่ม → intro → AI ถาม spec 3 ข้อ → ตีราคาหนังสือธรรมะ 300 เล่ม 54.98 บ./เล่ม รวม 16,493.50 (ก่อน VAT) — **standard access + Publish พอจริงสำหรับสาธารณะ** (ยืนยันข้อสรุป 2.2). พบ polish เล็ก: ราคาในข้อความมี `**...**` markdown ดิบ (Messenger ไม่ render — follow-up prompt fix)
...
- [x] **2.4** ✅ **DEPLOYED + VERIFIED 2026-07-09** — คุณนุ๊กรันสคริปต์เอง: `{"result":"success"}` + verify GET ครบ 3 (get_started/menu/ice breaker ทุกตัว payload `ai_quote_start`) + **เมนู ☰ ขึ้นจริงบน Messenger app มือถือคุณนุ๊ก** (ตอนแรกไม่เห็น = client cache ~ครึ่งชม. — force-close app แล้วรอ). Kit อยู่ workspace `../messenger-profile/` (นอก repo, ข้างๆ `line-oa-richmenu/`):
