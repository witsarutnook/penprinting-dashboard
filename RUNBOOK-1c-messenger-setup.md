...
- [ ] **2.2** ทำ requirements ให้ครบแล้ว **Publish** (แก้ 2026-07-09 หลังเห็น dashboard จริง — **app นี้เป็น business app แบบ use case, ไม่มีเมนู App Review → Permissions แบบเก่า**; เส้นทาง go-live = เมนู **Publish** ซ้ายมือที่ติดป้าย Unpublished):
  - ⚠️ กล่อง **"Become a Tech Provider" = ไม่เกี่ยว** — สำหรับ app ที่ให้บริการธุรกิจอื่น (agency/SaaS); เราใช้กับ Page ตัวเองเท่านั้น ห้ามกด
  - **Dashboard checklist 3 ข้อ**: ① Customize use case ✅ (เขียวแล้ว 7/09) ② **Test use cases** — smoke จริงผ่านแล้ว 8/9, กดเข้าไปดูว่าต้อง mark อะไรเพิ่ม ③ **"Check that all requirements are met, then publish your app"** → โชว์รายการที่ขาด (คาด: **Business Verification** ของ portfolio Penprinting [business.facebook.com → Settings → Security Center, ใช้เอกสารจดทะเบียน/DBD — ตัวที่ใช้เวลานานสุด เริ่มก่อน] + app icon 1024×1024; privacy/data-deletion URL ✅ 7/09)
  - ครบแล้ว → กด **Publish** → app เป็นสาธารณะ ลูกค้าจริงใช้ได้
  - **ถ้า flow ถามรายละเอียดการใช้ permission / ขอ review เพิ่ม** — ใช้ use-case text (EN) นี้:
  ...
  - **ถ้าโดนขอ screencast** (1-2 นาที, mp4, บัญชี tester): ① เปิดแชท Page ② ทัก → เห็น hint+ปุ่ม "ขอราคา AI" ③ กดปุ่ม → พิมพ์สเปก (ใบปลิว A4 4สี 1,000) → ได้ราคา ④ "ขอคุยกับทีมงาน" → hand-off ⑤ "จบ" → ออกโหมด — ใส่ caption อังกฤษต่อช็อต. ⚠️ hint gate ≤1/24ชม. → ใช้ tester คนที่ 2 (คู่กับ M5 2-account test)
