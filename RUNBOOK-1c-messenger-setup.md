...
- [ ] **2.2** ยื่น **App Review `pages_messaging`** ผ่าน use case แล้ว **Publish** (แก้ 2 รอบ 2026-07-09 หลังเห็นหน้าจอจริง — app แบบ use case **ยังต้อง App Review** แต่จุดเข้าย้ายไปอยู่ใน use case ไม่ใช่เมนูเก่า):
  - **จุดเข้า**: app AI Quoting → use case **Messenger from Meta** → **Messenger API Settings** → ข้อ **"3. Complete App Review"** → ปุ่ม **Request permission** (`pages_messaging`) — ข้อ 1 webhooks + ข้อ 2 tokens เขียวแล้ว ✅ (7/08)
  - ⚠️ กล่อง **"Become a Tech Provider" บน Dashboard = ไม่เกี่ยว** — สำหรับ app ที่ให้บริการธุรกิจอื่น; เราใช้กับ Page ตัวเองเท่านั้น ห้ามกด
  - ฟอร์ม review จะขอ: ① คำอธิบายการใช้ permission → วาง use-case text (EN) ด้านล่าง ② **screencast** สาธิต flow → สคริปต์ด้านล่าง ③ อาจตามด้วย **Business Verification** ของ portfolio (business.facebook.com → Settings → Security Center, เอกสารจดทะเบียน/DBD — ใช้เวลานานสุด เช็ค/เริ่มก่อนได้เลย) + app icon 1024×1024; privacy/data-deletion URL ✅ 7/09
  - Approve แล้ว → กลับ Dashboard checklist ข้อ ② Test use cases (smoke ผ่านแล้ว 8/9) + ข้อ ③ → กด **Publish** → ลูกค้าจริงใช้ได้
  - **Use-case text (EN)**:
  ...
  - **Screencast** (1-2 นาที, mp4, บัญชี tester): ① เปิดแชท Page ② ทัก → เห็น hint+ปุ่ม "ขอราคา AI" ③ กดปุ่ม → พิมพ์สเปก (ใบปลิว A4 4สี 1,000) → ได้ราคา ④ "ขอคุยกับทีมงาน" → hand-off ⑤ "จบ" → ออกโหมด — ใส่ caption อังกฤษต่อช็อต. ⚠️ hint gate ≤1/24ชม. → ใช้ tester คนที่ 2 (คู่กับ M5 2-account test)
- [ ] **2.3** **Publish สำเร็จ = Live** (app แบบ use case ไม่มีสวิตช์ Development→Live แยก) — verify: ทัก Page ด้วยบัญชีที่**ไม่ใช่** tester → บอทต้องตอบ
