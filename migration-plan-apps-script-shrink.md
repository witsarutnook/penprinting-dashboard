...
2. ✅ **audit_log Sheet → Postgres import = ลบ cron** — consistent กับ #4 (Viewer-only Sheet → ไม่มีคนจดด้วยมือได้ → import ไม่จำเป็น). audit จาก dashboard ทั้งหมดเป็น `source='postgres'` อยู่แล้ว (Phase 2). **rows `source='sheet'` ที่อยู่ใน Postgres ตอนนี้ (~500 rows, last 180 วัน) คงไว้ตามเดิม (Option A)** — `getAuditByTargetFromPostgres` query ไม่ filter by source → history user เห็นครบเหมือนเดิม. หลัง §12 audit ใหม่ทั้งหมด `source='postgres'` natively. audit_log Sheet กลายเป็น frozen historical record ก่อนวันตัด
