...
## 10. Decisions — TBD (ก่อน execute)

ต้องคุณนุ๊ก confirm 4 ข้อก่อนเริ่ม session execute:

1. **r2-backup** — Option A (keep AS) · B (ลบทั้งหมด, ใช้ Neon PITR) · C (replace pg_dump → R2)?
   - **Recommend B** — Neon PITR เพียงพอ, ลด code + dep
2. **audit_log Sheet → Postgres import** — ยังใช้ admin Sheet-side audit ไหม?
   - **Assume ไม่ใช้** → ลบ sync-from-sheet ทั้งหมด
3. **r2-backup R2 snapshot ปัจจุบัน** — เก็บไว้ ก่อนตัด (manual download จาก Cloudflare console) หรือไม่ต้อง?
   - **Recommend เก็บ** — 1 GB ประมาณ, ฟรี R2 storage
4. **Sheet permission หลัง §12** — Viewer only · Editor (เก็บ access edit) · Public link disabled?
   - **Recommend Viewer + share เฉพาะคุณนุ๊ก + admin** — kill edit ที่จะไม่ sync ที่ไหนเลย
