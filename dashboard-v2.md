...
| **สลิปขึ้น "อ่านสลิปไม่ได้" รัวๆ — Thunder `SLIP_NOT_FOUND`** (LINE OA, 2026-09-11, [`c233d38`](https://github.com/witsarutnook/penprinting-dashboard/commit/c233d38)) | คุณนุ๊ก report สลิปทดสอบ 100฿ ได้การ์ดเทาซ้ำๆ. `slip-metrics?recent=20` ชี้ชัด: `SLIP_NOT_FOUND` "Bank slip not found or invalid" — **ไม่ใช่ quota/auth/config** (จะเป็นคนละ code และคง fail 100%). Pattern = **รายใบ ไม่ใช่ช่วงเวลา**: id 1238 ✅ (21:31:28, KBANK→พร้อมเพย์ 0707) แทรกกลาง 1235-1237 ❌ (21:31:15-16) และ 1239-1241 ❌ (21:32-21:33); สลิปใบเดิมส่งกี่รอบก็ fail รอบเดิม ส่วน `...CPP01815` ผ่านทุกรอบ → ขา Thunder↔ธนาคาร ไม่ใช่ฝั่งเรา (โค้ด slip ไม่ถูกแตะตั้งแต่ 7/23). **ผลข้างเคียงที่แพงกว่าตัวปัญหา**: การ์ดบอก "รบกวนส่งรูปสลิปใหม่ให้ชัดเจน" → ลูกค้าส่งซ้ำ → กิน Thunder quota เพิ่มทุกรอบเพื่อคำตอบเดิม (3 รูป 3 รอบ = 7 calls เปล่า) | Root cause อยู่ฝั่ง Thunder (แจ้ง support พร้อม transRef) — โค้ดแก้ **ปลายทางของคำตอบ** ไม่ใช่วิธีเรียก: `slipFailureKind()` แยก `'slip'` (Thunder ตัดสินรูปนี้แล้ว — deterministic) ออกจาก `'system'` (ไม่ได้รู้อะไรเลย: key/quota/plan/Thunder ล่ม) → state ใหม่ `'error'` + copy ที่ **ไม่ขอให้ส่งซ้ำทั้งคู่** · retry 1 ครั้งเฉพาะ connection ตาย/5xx/408 (ไม่ retry 4xx, ไม่ retry 429) · `_meta {status, contentType, attempts}` ลง `slip_checks.raw` (ไม่ต้อง migrate — jsonb เดิม) เพราะคืนนั้นแยก 429 กับ 500 ไม่ออกเลย. Lesson: **ข้อความ error ที่สั่งให้ลูกค้า retry หน้า paid API = ตัวคูณค่าใช้จ่าย** ถ้า retry นั้นสำเร็จไม่ได้ — แยก "ของจริงตอบว่าไม่ผ่าน" ออกจาก "ระบบตอบไม่ได้" ตั้งแต่ชั้น classify |
...
### 🔍 Track status parity — การ์ด LINE ตามทัน `awaitingShipment` + วันที่ฝั่งลูกค้า pad (2026-09-17, tests 708→712)
คุณนุ๊ก report จาก screenshot คู่ (LINE vs เว็บ ใบ #202609034): **`lib/ai-quote/track-flex.ts` อ่าน `deriveTrackStatus` แค่ `currentDept`** ไม่เคยอ่าน `awaitingShipment` (`dept=post && staff=ship`) → งานในคิวรอจัดส่งค้างที่ "ขั้นตอนหลังพิมพ์" บน LINE ขณะที่เว็บขึ้น "สินค้าพร้อมรับ" มาตลอด. `lib/track-status.ts` ถูกสกัดมาเพื่อให้ 3 surface ตรงกัน แต่ field ที่เพิ่มทีหลังไหลเข้าแค่ `track-result.ts` + `customer-jobs-flex.ts` — **การ์ด `/track` ตกขบวน** (PR #27, `3952c33`). รอบสอง คุณนุ๊กเลือกยึดรูปแบบวันที่ของ LINE → เพิ่ม `displayDatePadded` ใน `lib/jobs.ts` ให้ surface ฝั่งลูกค้าทั้งหมด (`/track`, `/track/c/[token]`, Flex 2 ใบ) ใช้ `DD/MM/YYYY` เหมือนกัน **จอฝั่งพนักงานคงรูปแบบเดิม** (PR #28, `7682f82`). เคสที่ 3 — ชื่อลูกค้า mask ไม่ตรงกัน — **ไม่ใช่บั๊ก**: `loadOrderFromPostgres` ดึงแถวเดียวด้วย `WHERE id = $1 LIMIT 1` ทั้งสอง surface อ่าน field เดียวกัน; การ์ด Flex เป็นภาพนิ่งที่ค้างค่าเก่าไว้ ยืนยันด้วยการส่งการ์ดใหม่แล้วตรงกัน.

### 🪦 §15 close — ลบโปรเจกต์ Apps Script, Phase 4.3 ปิดสนิท (2026-09-12, zero code change)
soak 7 วันหลัง §13 (9/05→9/12) เงียบ → คุณนุ๊กลบโปรเจกต์ "penprinting dashboard data". §15 ครบทุกขั้น: env `APPS_SCRIPT_URL`/`APPS_SCRIPT_TOKEN` ถอดจาก Vercel ✅ 9/05 · trigger ลบ + archive deployment ครบทุกตัว ✅ 9/05 (probe URL → 404 HTML) · soak ✅ · ลบโปรเจกต์ ✅ 9/12. **ไม่มี Apps Script อยู่ใน live path หรือ rollback path ของระบบใดอีก** — Postgres = sole source of truth, Google Sheet = frozen historical archive. ⚠️ rollback dashboard AS **ทำไม่ได้แล้ว**: source ใน `production-monitoring/apps-script/dashboard/` เหลือเป็น reference. Docs กวาด 8 ไฟล์ + ลบ `/check-quota` + `/push-apps-script` ตัด target `dashboard` ([`f4b7c68`](https://github.com/witsarutnook/penprinting-dashboard/commit/f4b7c68), post-deploy smoke เขียว pin sha).

### 🩹 Slip failure handling — แยก "ระบบพัง" ออกจาก "อ่านสลิปใบนี้ไม่ได้" + retry + transport meta (2026-09-11, ☁️ cloud/iPad, tests 686→706)
คุณนุ๊ก report "เช็คสลิปมีปัญหา" พร้อม screenshot การ์ด "อ่านสลิปไม่ได้" + Thunder dashboard (API Status ไม่สำเร็จ / ประเภทสลิป ไม่มีข้อมูล). **วินิจฉัยจาก evidence ที่มีอยู่แล้ว** (`slip_checks.raw` จาก incident 7/23 คุ้มค่าตัวเองรอบที่สอง): error = `SLIP_NOT_FOUND` ทุกใบที่ fail → ตัด quota/auth/multipart/endpoint ออกหมด และแถว ✅ id 1238 แทรกกลางแถว ❌ ห่างกัน 12 วิ = ไม่ใช่ระบบล่ม ไม่ใช่ rate-limit ไม่ใช่บัญชีปลายทาง → **ฝั่ง Thunder รายใบ**. Ship ([`c233d38`](https://github.com/witsarutnook/penprinting-dashboard/commit/c233d38), ไม่แตะวิธีเรียก Thunder เลย): (1) `slipFailureKind(r)` ใน [lib/ai-quote/slip.ts](lib/ai-quote/slip.ts) — `'slip'` = Thunder ตัดสินรูปนี้แล้ว (SLIP_NOT_FOUND ฯลฯ) · `'system'` = ไม่ได้รู้อะไรเกี่ยวกับสลิป (NO_KEY/NETWORK/INVALID_RESPONSE ของเรา + quota/auth/plan/5xx ของเขา); **อ่าน HTTP status ก่อน error code** เผื่อ code เปลี่ยนชื่อ และ match แบบ case-insensitive (Thunder v2 ตะโกน `SLIP_NOT_FOUND` / รุ่น EasySlip ใช้ `quota_exceeded`) (2) `SlipState` เพิ่ม `'error'` → header "ระบบขัดข้องชั่วคราว" แยกจาก "อ่านสลิปไม่ได้" (พนักงาน triage จากการ์ดได้) และ **copy ทั้งสอง state เลิกขอให้ส่งรูปใหม่** — ของเดิมทำให้ลูกค้าส่งซ้ำแล้วเผา Thunder quota เพื่อคำตอบเดิม; LINE Flex + Messenger text เปลี่ยนพร้อมกันผ่าน `classifySlipState` ตัวเดียว (3) `verifyBankSlipImage` retry **1 ครั้ง เฉพาะ connection ตาย / 5xx / 408** — 4xx deterministic และ 429 คือเกินลิมิตอยู่แล้ว (retry = จ่ายโควตาซื้อคำตอบเดิม / ยิ่งโดนแบน); สร้าง FormData ใหม่ทุก attempt เพราะ blob ถูก drain ไปแล้ว (4) ทุก return แนบ `_meta {status, contentType, attempts}` → `slip_checks.raw` (jsonb เดิม ไม่ต้อง migrate). **Tests +20 (686→706)** + mutation check ครบ 4 แบบ (ปิด retry / retry SLIP_NOT_FOUND / ยุบ `'error'` / คืน copy เดิม → แดงตรงเป้าทุกตัว). **Self-correction ระหว่างทาง**: เกือบ ship "fix" mojibake ภาษาไทยด้วยการ decode UTF-8 เอง โดยอ้างว่า `res.json()` เชื่อ charset ที่ header ประกาศ — **ผิด** (fetch spec: "parse JSON from bytes" = UTF-8 เสมอ ไม่สน charset); เทสต์เอง 3 charset แล้วได้ไทยถูกทุกอัน → ถอดทั้งโค้ดและเทสต์ออก (เทสต์ที่ผ่านทั้งก่อนและหลังแก้ = ไม่ได้การันตีอะไร). Mojibake ยังไม่สรุป — เหลือ 2 ทาง: ข้อมูล Thunder เพี้ยนมาเอง หรือ Safari มั่ว charset ตอน copy JSON; ยืนยันด้วยตาจากการ์ด ✅ ใน LINE.
### 🔒 Edit-lock port — shipped/cancelled "แก้ไขไม่ได้" ย้ายจาก render เข้า route + API (2026-09-05, tests 660→685)
งานคู่ขนานกับ §13 (chip จาก review `4da9191`): กฎ `canEdit = admin && orderStatus !== 'shipped' && !== 'cancelled'` เคยอยู่แค่ใน render ของ `orders-table.tsx` → `/archive` (ซ่อนลิงก์ฝั่ง client), bookmark เก่า, URL พิมพ์มือ ยังเปิดฟอร์ม `/orders/[id]/edit` ได้ และ `POST /api/orders/update` ก็ยังรับ — คง `status` ไว้แต่ cascade ชื่อ/กำหนดส่งเข้า job rows ที่ tombstone แล้ว. **Fix (TDD, commit 6d052a0)**: (1) `lib/order-lock.ts` — `orderLockReason(order, shipped, cancelled)` ตัวเดียวตัดสิน ทั้ง route + API mirror derivation ของ `lib/orders-list` เป๊ะ (`status==='shipped' || shipped row` / cancelled ชนะ shipped / normalise trim+lowercase) + `orderLockMessage` "ใบสั่งงานนี้จัดส่งแล้ว/ยกเลิกแล้ว — แก้ไขไม่ได้" (2) `loadOrderLockStateFromPostgres(id)` — 1 round-trip: `raw->>'status'` + `EXISTS shipped/cancelled` (server-authoritative แบบเดียวกับ `loadJobDeptStaffFromPostgres` ของ reassign) + wrapper `loadOrderLockState` ใน `lib/api.ts` (3) `/api/orders/update` เช็คล็อค **ก่อน** validate และก่อน `srcUnchanged` perf gate → 409 `{error, locked}` · null → 404 · read fail → 502; client `order-form.tsx` แยก 409 duplicate ด้วย `error === 'duplicate'` จึงตกไป error banner ตามปกติ (4) `/orders/[id]/edit` โหลด lock state ขนานกับ order → ถ้าล็อคแสดง panel (pill + ข้อความ + ลิงก์กลับ /orders · ดูใบสั่งงาน /print · รายการ /shipped หรือ /cancelled) แทนฟอร์ม. **Tests +25**: helper 14 (precedence / normalise / EXISTS boolean / null order) · reader 5 (1 query, coercion 't'/'f', null) · route 6 (409 shipped/cancelled, srcOrder gate bypass ไม่ได้, 404, 502, happy path ยัง save). `/archive` ไม่แตะ.
...
### 🗑 DATA-orphan-cancelled ×4 ปิดฝั่ง Postgres + decommission decisions (2026-08-20, tests 631→636)
คุณนุ๊ก (`/session-start`) เลือก "ตัดสิน decommission" → ตัดสิน 2 เรื่องค้างนาน: (1) **Decommission LINE-webhook Apps Script + CF Worker** — evidence: นอก live path ตั้งแต่ 6/30 (~7 สัปดาห์ traffic จริงบนเส้น Vercel ไม่มี incident) + ตรวจสดพบ worker ยังตอบ HTTP 200 พร้อม secrets สด = attack surface ค้าง; เส้นเก่าไม่มี AI quote → ถอยกลับ = downgrade ที่ลูกค้าเห็น. **✅ Executed same-day**: คุณนุ๊กลบ worker (metrics ก่อนลบ: **1 invocation/30 วัน = health-check curl ของ Claude เอง — traffic จริงศูนย์สนิท**; verified 404 หลังลบ) + archive GAS deployment (ห้ามแตะโปรเจกต์ Dashboard ที่ยังใช้ `searchArchive`) → Claude กวาด docs ครบ (monitoring.md · CLAUDE.md ×2 · /check-quota · [runbook](../production-monitoring/RUNBOOK-decommission-line-worker.md) → done). Source ทั้งคู่ยังอยู่ใน repo คืนชีพได้ ~30-60 นาที. (2) **ลบ orphan cancelled ทั้ง 4** ([`863c3d2`](https://github.com/witsarutnook/penprinting-dashboard/commit/863c3d2)) — endpoint ใหม่ `/api/admin/cleanup-orphan-cancelled` ตาม pattern fix-date-anomaly (dry-run default + `?apply=1`, idempotent): data layer [lib/orphan-cleanup.ts](lib/orphan-cleanup.ts) ลบด้วย **ONE conditional-gated DELETE** (precondition `NOT EXISTS` order แม่ อยู่ใน WHERE + RETURNING — ตาม [[feedback_conditional_gate_first_transition]], ไม่มี read-then-delete window) + audit_log entry + bustLoadAllCache. Ground truth ก่อนตัดสิน: query ผ่าน prod API (Chrome คุณนุ๊ก, DB URL เป็น Sensitive ดึงตรงไม่ได้) — 4 rows อยู่ Postgres จริง (ids 142/218/395/463 ตรง Sheet 1:1) order แม่ไม่มีจริง (bootstrap หลัง `603d1ac` UNION event-refs เสมอ → ไม่เจอ = orphan แท้). **Applied on prod same-session**: dry-run 4/4 deletable → apply deleted ×4 → rescan 0 (idempotent ✅) → delta cancelled 33→29, orphan hits 0. เหลือฝั่ง Sheet (frozen archive, cosmetic): helper `_cleanup-orphan-cancelled.gs` ครอบ 4 rows เดียวกัน รอคุณนุ๊กรันใน GAS editor. TDD RED-first +5 tests (target pin / scan partition / single-statement gate shape / idempotent) · gates เขียวครบ · post-deploy smoke เขียว pin `863c3d2` (หมายเหตุ: `gh run list --commit <sha>` คืน `[]` สำหรับ run ที่ trigger ด้วย deployment_status — ต้อง match `headSha` จาก `--json` แทน).

### 🐞 Template quick-fill เติมชื่องาน + ชื่อลูกค้า (2026-08-10, tests 626→631)
คุณนุ๊กรายงาน: เลือก template แล้วชื่องาน/ชื่อลูกค้าว่าง — `/diagnose` พบไม่ใช่ regression: `applyTemplate` **จงใจ blank** ตั้งแต่ commit แรกของฟีเจอร์ (`f897155`, สมมติฐาน "template = generic sample") ซึ่ง session 7/03 เคย reconfirm; การใช้จริง template ผูกกับงาน/ลูกค้าประจำ (เช่น "ใบปลิวซุปเปอร์") = **class เดียวกับ duplicate fix `c7daeb0`**. Snapshot เก็บ name/customer ครบอยู่แล้ว (saveAsTemplate ส่ง form เต็ม). **Fix** ([`42d17ae`](https://github.com/witsarutnook/penprinting-dashboard/commit/42d17ae)): fix ที่ 3 ในโซน prefill (e0b1ae4 · c7daeb0 · วันนี้) → **3+ fixes rule**: extract `prefillOrderFormFromRaw` ([lib/photobook.ts](lib/photobook.ts)) = shared pure policy (carry full spec รวม name/customer · reset dateIn=วันนี้/dateDue ว่าง · `keepOrderer` option) ให้ทั้ง 3 flows — duplicate effect / ดึงงานล่าสุด / applyTemplate — วิ่งเส้นเดียว มี test seam จริง (เดิม policy กระจาย inline ใน component ที่ไม่มี RTL harness). ปิด bug แฝง 2 ตัวใน applyTemplate พร้อมกัน: **orderer โดน snapshot ทับเงียบ** (comment อ้าง "preserve current orderer" แต่ `orderFormFromRaw` merge raw.orderer ทับ — `keepOrderer` pin เป็นคนปัจจุบัน) + **extra-bills toggle ไม่ sync** (template ที่มีบิลสี 4-6 ซ่อน UI). Submit ยังเตือน "พบใบสั่งงานคล้ายกัน" = safety net เดิม. TDD RED-first +5 tests pin policy ทุกแกน.

### 🔒 L1 fullLists role gate — cancelled payload เหลือ admin เท่านั้น (2026-08-08, tests 622→626)
คุณนุ๊กตัดสิน L1 (item สุดท้ายที่ actionable จาก audit round 8/03): **gate ตาม role** — เดิม `?fullLists=1` ส่ง cancelled rows เต็ม (เหตุผล+ผู้ยกเลิก) ให้ทุก role ที่ login ทั้งที่หน้า `/cancelled` เป็น admin-only (staff/sales ดึงได้ทาง curl/console — pre-existing ตั้งแต่ port, สอดคล้อง WP เดิม). **Fix** ([`1897f9b`](https://github.com/witsarutnook/penprinting-dashboard/commit/1897f9b)): `loadBoardDelta` เปลี่ยน opts เป็น union — `fullLists: true` **บังคับระบุ `includeCancelled`** (type-level pairing แบบ `LoadSessionOpts`: caller ใหม่ลืมตัดสินใจ = compile error, fail-closed by construction); caller ที่โดน gate **ข้าม query cancelled rows/ids ทั้งหมด** + response ไม่มี `cancelled`/`cancelledAllIds`/`cancelledCheck` (stats query คง combined — สอง subselect ถูกมาก, แค่ strip ตอน assemble). Route derive จาก `session.role` ผ่าน `includeCancelledForRole()` ใหม่ (admin only — test pin policy). `/shipped` SSR ส่ง `false` (หน้า seed `cancelled: []` อยู่แล้ว — ได้ perf ฟรี: ตัด windowed cancelled scan ทุก navigation ของทุก role) · `/cancelled` SSR ส่ง `true` (page redirect non-admin ก่อนถึง fetch). `lists`-mode orderId sets (badge /orders) **คงเปิดทุก role ตั้งใจ** = WP parity (bare ids, ไม่มี row content). **Client zero change** — `fullListsStale`/`upsertRows`/`sameIdList` treat field ที่หายเป็น "not carried" อยู่แล้ว + `FullListsTrack` (H1 fix 8/03) กัน /shipped ของ staff ไม่ไล่ reconcile ตารางที่ไม่มีวันมา. TDD RED-first +4 tests · gates เขียวครบ (bundle เท่าเดิม) · post-deploy smoke เขียว pin `6295e7d`. **Audit backlog เหลือ open ตัวเดียว: L4** (ผูก §13, ตั้งใจ defer).

### 🟢 L2+L3 จาก audit round 8/03 ปิด + H1 smoke จอจริง prod-verified (2026-08-05, tests 620→622)
**H1 follow-up (same-session, คุณนุ๊กสั่ง "ทำ 1 ให้เลย")**: real-screen smoke ที่ automation เคยทำไม่ได้ → เจาะได้ด้วย **visibility patch** (`refreshGuard` อ่านแค่ `document.visibilityState` → `Object.defineProperty` เป็น 'visible' ใน main world ผ่าน Chrome MCP javascript_tool = polling loop วิ่งเหมือนจอจริง). ผล `/cancelled` ~12 นาที: **≥5 fullLists polls ติดกัน cursor เดินปกติ, ZERO `?ids=1` reconcile chain, zero burst, console clean** — bug เดิม chain ตั้งแต่ poll แรกสำเร็จเสมอ + เป็น promise-driven (throttle ซ่อนไม่ได้) → **FullListsTrack scope fix `6b71d33` พิสูจน์บน prod แล้ว**. Caveat: /shipped ไม่ได้ exercise แยก (tab hidden 25+ นาที โดน Chrome intensive throttling จน timer ตาย) — hook เดียวกัน + config verified จากโค้ด; optional glance บนจอจริงถ้าอยากชัวร์. Lesson ชั้นที่ 4 เข้า memory [[browser-automation-hidden-tab-smoke]] (patch ใช้ test promise-driven ได้ / timer cadence ต้องจอจริง / CDP network = ground truth).
คุณนุ๊ก (`/session-start`) เลือก "ปิด L cluster จาก round 8/03" → ปิด 2/4 (L1 role gate รอตัดสิน · L4 ผูก §13 defer ตั้งใจ). **L3-photobook-summary-silent-partial** ([`c230496`](https://github.com/witsarutnook/penprinting-dashboard/commit/c230496)) — `photobookRowSummary` ใน [lib/photobook.ts](lib/photobook.ts) เลิก skip เงียบ item ที่ qty parse ไม่ได้ (legacy data ก่อน validatePhotobook เช่น qty "10 เล่ม"): item ไม่ว่าง (มี field ใดๆ) ที่ qty ใช้ไม่ได้ → qty แสดง **"N+ เล่ม"** เมื่อมียอดบางส่วน (lower bound) / **"? เล่ม"** เมื่อไม่มี qty ใช้ได้เลย (แยกความหมายจาก "-" = ไม่มี item); แถวว่าง + junk non-object ไม่ trigger — pin ทุก branch ด้วย test (TDD RED-first +2). Caller เดียว = buildRow ใน lib/analytics.ts → blast radius จำกัดที่ report modal. **L2-loadOrderAndJobs-contract-footnote** ([`e474d16`](https://github.com/witsarutnook/penprinting-dashboard/commit/e474d16)) — doc-only footnote ใน [lib/api.ts](lib/api.ts): order-not-found + jobs query ล้ม เดิม (serial) เงียบ ตอนนี้ throw (callers absorb — orders/update → 502, promote-draft → try/catch). Gates เขียว + smoke เขียว pin `e474d16`.

### 🐞 Audit round 2026-08-03 — H1 reconcile loop + M1/M2 window arms ปิด same-day (tests 610→620)
คุณนุ๊ก (`/session-start`) เลือก "Audit round ใหม่" → penprinting-auditor สแกน 9 commits หลัง round 7/23 → เปิด 1H+3M+4L, ปิด H+M ครบวันเดียว (TDD RED-first, commit ละก้อน). **(1) H1-fulllists-reconcile-loop** ([`6b71d33`](https://github.com/witsarutnook/penprinting-dashboard/commit/6b71d33)) — regression จาก `b56b156`: /shipped seed `cancelled:[]` + /cancelled seed `shipped:[]` แต่ `fullListsStale` เทียบ checks **ทั้งสองตาราง** → ตารางที่ไม่ track ไม่มีวัน match + reconcile `?ids=1` materialize rows ที่ client ไม่เคยมีไม่ได้ → **infinite back-to-back poll loop** ที่ chain ผ่าน `pollNowRef` ตรง = ข้าม `refreshGuard` + hard-stop 30 นาที — ยิง Postgres ถาวรทุกครั้งที่เปิดหน้าค้างบนจอจริง (automation smoke ไม่เจอเพราะ tab hidden → guard บล็อก tick แรกก่อน loop เริ่ม). **Fix**: `FullListsTrack` scope (แต่ละหน้าเช็คเฉพาะตารางตัวเอง) + **`planReconcile` pure fn = circuit breaker** — stale จาก normal poll → chain 1 ครั้ง; reconcile ที่ยัง stale → เก็บ flag รอ scheduled tick ห้าม chain (ปิด M3-reconcile-cannot-materialize ในตัว — backdated insert ระหว่าง data repair ไม่ loop อีก). **(2) M1+M2 window arms alignment** ([`603d1ac`](https://github.com/witsarutnook/penprinting-dashboard/commit/603d1ac)) — `29baea0` ทิ้ง 2 timeline: orders = YYYYMM id cutoff / sets+rows = rolling `imported_at`; comment invariant "event follows creation" **เท็จ** เมื่อ event row age out ก่อน order id หลุด cutoff → fake orphan ใน data-audit + badge หาย (เริ่ม ~2027-05) + /shipped คอลัมน์ลูกค้า "—" เมื่อ order แม่หลุด window. **Fix**: bootstrap orders + UNION orders ที่ EVENT-windowed ship/cancel rows อ้าง (ทุก row resolve order แม่เสมอ + order เก่าที่เพิ่ง ship ยังโชว์พร้อม badge) · orderId sets mirror ทุก arm ของ orders window (cutoff OR recent-event OR active-ref) = invariant ถูกโดย construction · fullLists rows คง event-window ตั้งใจ (งาน ship เมื่อวานต้องอยู่ /shipped ไม่ว่า order อายุเท่าไร). **L×4 ลง backlog** (L1 fullLists role gate รอคุณนุ๊กตัดสิน · L2 footnote · L3 photobook silent-partial · L4 year filter ผูก §13). Gates เขียวทุก commit + smoke เขียว pin `603d1ac`. Lesson → memory [[reconcile-protocol-convergence]] (repair loop ต้องพิสูจน์ convergence + self-triggered calls ห้ามข้าม guard).

### 📊 Report กราฟฟิก — photobook rows แสดงขนาด/จำนวน (2026-08-01, tests 603→610)
คุณนุ๊กส่ง screenshot report modal มิ.ย. — งาน Photobook (เช่น Print-25June2026) ขึ้น "-" ทั้งคอลัมน์ขนาด/จำนวน. **Root cause**: photobook เก็บสเปคต่อเล่มใต้ `rawData.photobook` (top-level `size`/`qty` ว่างเสมอ) แต่ `buildRow` ใน [lib/analytics.ts](lib/analytics.ts) อ่านแค่ top-level. **Fix** ([`ac7076f`](https://github.com/witsarutnook/penprinting-dashboard/commit/ac7076f)): `photobookRowSummary()` ใหม่ใน [lib/photobook.ts](lib/photobook.ts) — dual-key read (`photobookItems` legacy / `photobook` canonical, precedence เดียวกับ `orderFormFromRaw`) → ขนาด = sizes deduped join (`8"x8", A3`) / จำนวน = ยอดรวม `N เล่ม`; ไม่มี item ใช้ได้ = "-" เดิม; งานปกติ path เดิมเป๊ะ. มีผลทุกเดือน/ทุกแผนก/ทั้ง tab รวม+แยกตามช่าง (renderer แสดง `r.size`/`r.qty` ตรงๆ — แก้ data layer จุดเดียวจบ). TDD RED-first (`tests/analytics-monthly-report.test.ts` ใหม่ +7) · smoke เขียว pin sha · คุณนุ๊ก prod-verify บนจอจริง ✅

### 📄 กระดาษ Green Read 75 / ถนอมสายตา (2026-07-24, same session) — ship ครบวงจร 2 repos (tests 600→603)
เคสลูกค้า #46 escalate เพราะเนื้อใน "Green Read 75 แกรม" นอกรายการ → คุณนุ๊กเพิ่มผ่าน Settings UI บนเว็บ calc ซึ่งเก็บแค่ **localStorage ต่อเครื่อง** (`useConfig` merge: `parsed.papers` ทับทั้งก้อน) — **API `/api/quote` อ่าน `DEFAULT_CONFIG` เท่านั้น** (by design "no price drift") → น้อง PP ไม่เห็น. **Fix**: calc [`561577f`](https://github.com/witsarutnook/penprinting-calc/commit/561577f) `Green Read 75` 856/1350 + baseline `book-a5-greenread-offset` (validate 19/19, 18 เดิม identical) · dashboard [`1f0012f`](https://github.com/witsarutnook/penprinting-dashboard/commit/1f0012f) `VALID_PAPER_NAMES` + alias twin ทั้ง 2 prompts (ถนอมสายตา/กรีนรีด/Green Read → Green Read 75, แกรมอื่น = out-of-list เดิม). Smoke เขียว pinned ต่อ sha ทั้งคู่. **ปิดครบทุกชั้น same-day**: ชีท master (คุณนุ๊กเพิ่มแถวใต้ Bond 100) + โค้ด + prompt + localStorage reset. Backlog จดไว้: DB-backed paper prices (Neon ไม่เอา Supabase) — revisit เมื่อราคาขยับบ่อย (ดู NEXT-SESSION pending #4).

### 🟢 L ×5 ปิดครบ (2026-07-24, same session) — perf audit round 7/23 ปิดสมบูรณ์ 100% (tests 586→600)
คำสั่ง "ทำ L เลย" → 4 commits ปิด Low ทั้งหมดของ round: **loadOrderAndJobs** order+jobs `Promise.all` (−1 Neon hop ทุก cascade write path) · **rate-limit** ทุก composite op (check INCR+EXPIRE+TTL / peek GET+TTL / recordFailure INCR+EXPIRE) รวมเป็น Upstash **`/pipeline` request เดียว** — จุดที่โดนทุก modal open + AI webhook hot path (~50-150ms/hop); refundAttempt คง 2 hops ตั้งใจ (DEL conditional ตาม DECR) · **loadQuoteLogDetail** quotes+flags parallel หลัง session gate · **db-migrate** เพิ่ม `idx_ai_quote_sessions_updated` (รอกด migrate บน prod) · **orders/raw** comment โกหก 2 จุดกวาด ("Apps Script getOrder"/"30s ISR" → จริงคือ Postgres fresh single-row ตั้งแต่ Phase 4.2). Parallelism pins แบบ §1.14 ทุกตัวที่มี seam. **Perf audit round 2026-07-23 = High 1/1 + Medium 4/4 + Low 5/5 ปิดครบทุกระดับ.**

### 📉 Growth-risk M ×3 ปิดครบ (2026-07-24, Mac) — window + check/reconcile + coalesced bootstrap (tests 564→586)
ปิด Medium ที่เหลือจาก perf audit 7/23 ทั้ง 3 ตัว (TDD RED-first, commit ละตัว) — theme เดียวกัน: list-shaped reads ที่โตเชิงเส้นตาม row count ตลอดไป ตอนนี้มีเพดานถาวรทุกเส้น. **(1) Windowed bootstrap** ([`29baea0`](https://github.com/witsarutnook/penprinting-dashboard/commit/29baea0)) — `LIST_WINDOW` 12 เดือนเดียว apply ทุกชิ้นที่ cross-reference กัน: orders = `id >= ordersCutoffId()` (YYYYMM id cutoff Bangkok — เดือนจริง ไม่ใช่ imported_at backfill) UNION orders ที่ active jobs อ้าง (card/calendar resolve เสมอ) · fullLists rows + /orders orderId sets = `imported_at` 12mo. Invariant: ship/cancel event ตามหลัง creation ⟹ ไม่มีคู่ครึ่ง window → badge/orphan ไม่เพี้ยน. วันนี้ข้อมูลยังไม่ถึงอายุ window → behavior change ~0. **(2) Check/reconcile protocol** ([`b56b156`](https://github.com/witsarutnook/penprinting-dashboard/commit/b56b156)) — /shipped /cancelled default poll เลิก scan+ส่ง PK id array เต็ม (~30KB@3K rows ทุก 15-30s/tab) → stats query เดียว `{count,maxId}` (~30B, −1 query); client `fullListsStale()` เทียบ merged state → mismatch (restore/aging-out) = reconcile poll `?ids=1` ครั้งเดียวทันทีแล้ว allow-list drop เดิม; `mergeDelta` ได้ `upsertRows` path ใหม่. **(3) Coalesced bootstrap** ([`3923558`](https://github.com/witsarutnook/penprinting-dashboard/commit/3923558)) — `loadBoardDelta` router: bootstrap plain+lists ผ่าน `loadBootstrapCached` (unstable_cache, tag LOAD_ALL_TAG, 15s) → N tabs เช้า = ~1 fill/15s/shape เหมือนยุค loadAll; fullLists ไม่ cache ตั้งใจ (~2MB entry limit). Route ได้ param `?ids=1`. Test infra: `tests/helpers/mock-next-cache.ts`. Gates เขียวทุก commit + build (bundle เท่าเดิม). Product note เล็ก: /orders /shipped /cancelled แสดง ~12 เดือนล่าสุด (ของเก่ากว่ารอ archive §13). PATTERNS §1.15. **Perf audit round 2026-07-23 = High 1/1 + Medium 4/4 closed — เหลือ L ×5.**

### ⚡ Perf audit round 2026-07-23 (Mac PM) — prod healthy + fixes เดิม hold 100% + ปิด H+M ตัวหนัก same-day (tests 554→564)
คุณนุ๊กสั่ง "ตรวจ performance" → 3 ชั้น: **(1) Live prod วัดจริงแข็งแรงทุกจุด** — TTFB warm ~170-320ms ทุก endpoint (LINE webhook ~200ms) · CDN ถูกต้อง (login prerender HIT, chunks immutable 1y, /track dynamic by design) · bundle shared 103 kB / /board 141 kB first-load · smoke เขียวทุก Production deploy ("pending" ที่ rtk โชว์ = skipped ของ preview — rtk distortion). **(2) penprinting-auditor verify 14 fixes เดิม hold ครบ** ไม่มี regression (useMemo context · tombstone gates · slim bootstraps/delta · lazy imports · adaptive polling · loadAll เหลือ morning-report cron ตัวเดียว). **(3) เปิดใหม่ 1H+4M+5L → ปิด 2 ตัวหนักตามคำสั่ง "ตามแนะนำ"**: 🔴 **bulk-forward batch** ([`0df9efd`](https://github.com/witsarutnook/penprinting-dashboard/commit/0df9efd)) — per-item loop (~75 hops/batch-25 ≈ 1-4s) → ANY() gate เดียว + multi-VALUES INSERT เดียว + `appendAuditBatchToPostgres` = **~4 hops/batch**; H2 per-row mutual exclusion + compensation + collision read-back คงเดิมครบ (PATTERNS §1.13) · 🟡 **webhook parallel persists** ([`14013e1`](https://github.com/witsarutnook/penprinting-dashboard/commit/14013e1)) — `Promise.all(saveConversation, touchMode)` ก่อน reply + escalate 6 ops → 3 stages; persist-before-reply คงทุกเส้น (PATTERNS §1.14). **Prod-smoke same-day ครบ** (Claude ผ่าน Chrome คุณนุ๊ก): bulk 2 งานจริง → audit 2 entries stamp เดียวกัน (multi-row เดียว พิสูจน์บน prod) + AI turn ลง /quote-logs ครบ + board กู้คืนทุกใบ — เหลือ optional: เส้น forward-undo ยังไม่ prod-exercised (undo toast 9s ไม่ทันกด). เหลือ open ใน backlog: M growth-risk ×3 + L ×5 (defer ตั้งใจ). บวก: [MEMORY-QUEUE] จาก cloud session เช้า saved ครบ 2 ตัว.

### 🔥 Slip-verify incident (2026-07-23, ☁️ cloud session) — silent drop + wrong-account-as-✅ ปิดทั้งคู่ (tests 533→554)
ลูกค้าส่งสลิปจ่ายบิล 580฿ (memo "sticker") เข้า LINE OA เงียบ 3 ครั้ง ขณะสลิปอื่นนาทีเดียวกันได้การ์ดปกติ + Thunder dashboard เห็น "ผู้รับไม่ถูกต้อง" แต่บอทตอบ ✅. **จังหวะ 1 — diagnosability (PR [#23](https://github.com/witsarutnook/penprinting-dashboard/pull/23) [`88c1c81`](https://github.com/witsarutnook/penprinting-dashboard/commit/88c1c81)):** `slip_checks` + `prefilter_answer` + `raw` jsonb (คำตอบดิบ Haiku + Thunder response เต็มทุก event) · record ก่อน reply (send พัง = evidence ไม่หาย) · `?recent=N` บน slip-metrics อ่าน raw จากเบราว์เซอร์ · parse fix: refusal = `no\b`/`ไม่ใช่`/`ไม่` เดี่ยว ("ไม่แน่ใจ" เคยโดนกินเป็น refusal). **Root cause จาก prod evidence:** (A) Haiku ตอบ "no" ตรงๆ กับสลิปใบนี้ (id 424) — คาดชน "สติกเกอร์" ใน no-list กับ memo "sticker" ในสลิป; (B) **Thunder v2 ส่ง `matchedAccount`** (object=match/null=ไม่) **ไม่ใช่ `isAccountMatched`** ที่ `classifySlipState` อ่าน → `=== false` ไม่เคยจริง → wrong-account ทุกใบ = ✅ ตั้งแต่ ship ทั้ง LINE+Messenger. **จังหวะ 2 (PR [#24](https://github.com/witsarutnook/penprinting-dashboard/pull/24) [`ceb7419`](https://github.com/witsarutnook/penprinting-dashboard/commit/ceb7419)):** prompt memo/theme-immunity + no-list เจาะจง "สติกเกอร์ไลน์/รูปการ์ตูน" (`SLIP_PREFILTER_PROMPT` export + verbatim pins) · `slipAccountMatched()` dual-read v2+legacy = source เดียวของ classify + คอลัมน์ `is_account_matched` (เดิม NULL ตลอด — `mismatches` rollup เริ่มนับจริง 7/23). **Discipline ที่ทำให้ปิดได้ใน 1 วัน:** capture raw ก่อนแก้ + **positive control** (โอน 1฿ เข้าพร้อมเพย์บริษัท → `matchedAccount` object, id 426) ก่อน flip logic. **Prod-verified:** สลิป 580 → ✅ ครบ field · สลิป 75฿ เข้าร้านอื่น → ❌ บัญชีปลายทางไม่ตรง. Smoke เขียวทั้ง 2 deploys.
