...
- `/quote-leads` — ตาราง lead + เปลี่ยน `lead_status` (ใหม่/กำลังติดตาม/ปิดการขาย/ไม่สนใจ/**ต้องประเมินเอง**=escalated/ถูกทิ้ง=abandoned) + หยิบงาน (`assigned_to`, race-safe 409) + **กดแถวขยายอ่านบทสนทนาเต็ม** + **ปุ่มลบ (admin only, confirm)**. escalation lead มี badge ส้ม "⚠ ต้องประเมินเอง".
