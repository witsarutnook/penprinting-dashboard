...
>
> **🟡 Phase 3.6 Path A — RETIRE WP (in progress)** — code/redirect/Apps Script prep ครบใน session 2026-05-09 เช้า. **รอ user manual steps** (DNS switch + add custom domain ที่ Vercel + push Morning Report Apps Script). ดู Section "Phase 3.6 Path A — รายการที่ user ต้องทำเอง" ด้านล่าง.
>
> **Session 2026-05-09 morning (Phase 3.6 prep)** ✅ — audit hard-coded WP refs ครบทุก project + แก้ 9 จุด:
> - **v2 codebase** (3 จุด): `lib/api.ts:5` comment fix + `app/orders/orders-table.tsx:423` + `app/board/card.tsx:1070` ลบ link "ระบบ WP" จาก history tab placeholder, แทนด้วย "ประวัติงาน (audit log) อยู่ระหว่างพัฒนา"
> - **penprinting-web/next.config.js** (4 redirects): `penprinting.co/production-monitoring*` → `dashboard.penprinting.co/board`, `/track*` → `dashboard.penprinting.co/track`
> - **morning-report Apps Script** (2 mirrored files): `DASHBOARD_URL` `penprinting.co/production-monitoring/` → `dashboard.penprinting.co/board` (เก่า broken มาตั้งแต่ migration 2026-04-26)
>
> Type-check + production build ผ่านทั้ง 2 Vercel projects. Spawned task: Morning Report `ICON_BASE` (`penprinting.co/icons/`) ก็ broken เหมือนกัน — แยก task ไว้.
>
...
### 1. 🟡 Phase 3.6 — Path A RETIRE WP (in progress)

**User committed** end of 2026-05-08 session. Code/redirect prep เสร็จใน session 2026-05-09 morning. รอ user manual steps.

#### ✅ Done by Claude (2026-05-09 morning, code-side prep)
- Audit hard-coded WP refs ครบทุก project (v2 + marketing site + photobook + calc + Apps Script + Cloudflare Worker)
- Fix 3 v2 refs: `lib/api.ts:5` comment + 2 history-tab WP links → "อยู่ระหว่างพัฒนา"
- Fix 4 marketing-site redirects → ชี้ตรง `dashboard.penprinting.co`
- Fix Morning Report `DASHBOARD_URL` (Apps Script source committed; ต้อง user push ผ่าน push.sh)
- Type-check + production build ผ่านทั้ง 2 Vercel projects
- Verified: paper QR codes ที่พิมพ์ไปแล้ว (`app.penprinting.co/track?id=...`) ยังทำงานหลัง DNS switch

#### 🟡 Pending — Phase 3.6 Path A — รายการที่ user ต้องทำเอง
...
_อัปเดตล่าสุด: 2026-05-09 morning — Phase 3.6 prep day. 9 hard-coded WP refs แก้ครบ (3 v2 + 4 marketing redirects + 2 Morning Report mirrors) + type-check + production build ผ่าน 2 projects. รอ user manual: backup WP + add custom domain Vercel + DNS A→CNAME HostAtom + push Morning Report Apps Script + LINE comm._
