...
>
> ## ⏳ Pending (2026-07-15)
> 1. 🔭 **Follow-up เล็ก (เสนอแล้วยังไม่ confirm)**: staff prompt ([lib/ai-quote/prompt.ts](lib/ai-quote/prompt.ts)) ยังไม่มีกฎ current-turn-only แบบ `a521529` — พนักงานวางสเปกซ้ำหลังราคากระดาษเปลี่ยน = bug class เดียวกัน (~5 นาที TDD)
> 2. 🪪 **Smoke นามบัตรบน prod (carryover 7/13)**: ทักน้อง PP "นามบัตร 2 กล่อง เคลือบเงา" → กล่องละ 250 รวม 500 · "250 ใบ 2 หน้า" → 3 กล่อง รวม 900 + สมมติฐานไม่เคลือบ — หมายเหตุ: หลัง `ddafb7b` ต้องเช็คด้วยว่าราคารวมนามบัตรยังถูก (กล่อง × fix rate ไม่ใช่ qty × unit — test pin แล้ว แต่ smoke จริงยังไม่มี)
