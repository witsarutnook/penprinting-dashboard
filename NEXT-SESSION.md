...
>
> ## ⏳ Pending (2026-07-09)
> 0. ~~📱 verify post-publish~~ ✅ **ผ่าน (same-day)** — non-tester ทัก → hint → ปุ่ม → intro → ตีราคาหนังสือธรรมะ 300 เล่ม 54.98 บ./เล่ม ครบ flow (สาธารณะใช้ได้จริง). ~~2.4 menu/ice breakers~~ ✅ **deployed** (คุณนุ๊กรันสคริปต์ `messenger-profile/` — success + verify GET ครบ; เหลือเช็คตาเปล่าเมนู ☰ บนมือถือ). **เหลือ: M5 2-account test** (สำคัญ — live แล้ว) + **polish เล็ก: AI ตอบราคามี `**...**` markdown ดิบใน Messenger** (แชตไม่ render — แก้ที่ customer prompt/send path, Claude ทำได้ ~15 นาที TDD)
...
> 3. (carryover 7/08) 📱 M5 2-account Messenger+LINE (รอบัญชี Tester ที่ 2) · 🎨 rich menu รอรูปดีไซน์ · smoke LINE ที่เหลือ (④ "สั่งเลย" + /track/สลิประหว่างโหมด) · revert gate "/" หลัง soft launch นิ่ง
