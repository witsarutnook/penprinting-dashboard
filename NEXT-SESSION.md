...
>
> ## ⏳ Pending (2026-07-03 cont.) — คุณนุ๊ก action
> 1. **iPad PWA** (หลัง deploy): **ลบไอคอน dashboard เดิมออกจาก home screen → เปิด Safari → Add to Home Screen ใหม่ → login → สลับ app → กลับมา = ควรยัง login ค้าง**. (verify hypothesis: เปิดใน Safari ปกติ สลับ app ถ้ายัง login = ยืนยัน standalone storage 100%)
> 2. **smoke `/orders/new` บน prod** (H1 แตะ SQL, verify DB จริงบนเครื่องไม่ได้ → try/catch degrade graceful) — เปิดหน้า → **template dropdown ขึ้นครบ + customer autocomplete + กด "สั่งซ้ำ" ใบจริง prefill ชื่องาน/ลูกค้า/spec ครบ**. ถ้าอันไหนหาย = SQL loader มีปัญหา → ping ให้ revert `8720a9e`
> 3. **Deferred perf → AUDIT-BACKLOG:** H2/M2 (slim Order display shape แยกจาก spec — board/orders delta re-send `orders.raw` เต็มทุกใบ, งาน **phase** blast-radius กว้าง: computeBoard/computeOrdersList/detail modal พึ่ง inline rawData) · M3 (annotate delta route `dynamic`/Cache-Control, S)
>
> ---
>
