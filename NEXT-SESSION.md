...
>
> ## ⏳ Pending (2026-07-09)
> 0. 📱 **verify post-publish (สำคัญสุด, ทำได้ทันที)**: ให้คนที่**ไม่มี role ใน app** (พนักงานคนไหนก็ได้) ทัก Page Penprinting → ต้องได้ hint + เข้าโหมด AI ได้ — ถ้าเงียบ → unpublish แล้วแจ้ง Claude สืบต่อ. **M5 2-account test ทำตามหลังโดยเร็ว** (publish ก่อน test ตามคำสั่ง) + **2.4 persistent menu + ice breakers** (Claude เตรียม curl payload ให้ — token ผ่าน `read -s`)
