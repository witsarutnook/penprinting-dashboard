...
>
> ## ⏳ Pending (2026-08-28)
> 1. 🧪 **Smoke มือถือจริง (คุณนุ๊ก)**: เปิด `card.penprinting.co/demo` บน iPhone (Safari) + Android → กด "บันทึกรายชื่อ" → contact เข้าเครื่อง ชื่อไทยไม่เพี้ยน · ปุ่ม tel/LINE/FB/Maps เด้งแอปถูกตัว · ⚠️ ใน LINE in-app browser (iOS) ดาวน์โหลด .vcf ไม่ได้ = expected, ใช้ "Open in Safari"
> 2. 📟 **เขียน NFC จริง 1 ใบ** (แอพ NFC Tools): URL `https://card.penprinting.co/demo?s=n` · QR ใช้ `?s=q`
> 3. 👀 **carryover 8/27**: watch การ์ดเขียว Thunder ยังไม่ได้เช็ค (วันนี้ทำงานใหม่แทน) — ถ้าสะดวกให้ Claude เช็ค `slip_checks` ย้อนหลัง 48 ชม. session หน้า + ข้ออื่นจาก 8/27 (ไล่แชท OA / hardening reissue token) ยังค้างตามเดิม
> 4. 💡 ลูกค้าจริงรายแรก: ส่ง `penprinting-cards/docs/customer-intake-checklist.md` ให้ลูกค้ากรอก → เพิ่ม JSON + push = live (ดู README) · โลโก้ขอ PNG/JPG ≥500px
