...
>
> ## 🔴 Pending user action (สำคัญสุด — ทำหลัง deploy เขียว)
> 1. **รัน DROP migration** — เปิด `dashboard.penprinting.co` → ยิง `POST /api/admin/db-migrate` (ปุ่ม/วิธีเดิมที่เคยใช้ apply migration เช่น indexes/fix-date-anomaly). **ทำหลัง Vercel deploy `587044a` เขียวแล้วเท่านั้น** — deploy ลบ writes ของ column ออกก่อน, ถ้า DROP ก่อน deploy เขียว → old-bundle write ชน missing column = error. Migration idempotent (รันซ้ำปลอดภัย) จะ DROP `phase2_dirty_at` + partial index จาก 4 ตาราง (jobs/orders/shipped/cancelled). คาดผลลัพธ์ `applied: ["ALTER TABLE jobs DROP phase2_dirty_at + partial index", ...]` ×4.
> 2. **(optional) dismiss background-task chip** "ลบ phase2_dirty_at cluster" ถ้ายังโผล่อยู่ใน UI — งานนี้ทำเสร็จใน session นี้แล้ว
>
