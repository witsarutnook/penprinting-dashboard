...
>
> **Session 2026-09-17 (☁️ cloud/iPad) — 🔍 สถานะ /track ใน LINE ไม่ตรงกับหน้าเว็บ → เจอ drift ของ surface ที่ port มา (PR [#27](https://github.com/witsarutnook/penprinting-dashboard/pull/27) `3952c33` + PR [#28](https://github.com/witsarutnook/penprinting-dashboard/pull/28) `7682f82`, tests 708→712):** คุณนุ๊กส่ง screenshot คู่ (การ์ด `/track` ใน LINE vs หน้าเว็บ `/track` ใบ #202609034) ถามว่าทำไมสถานะไม่ตรงกัน — LINE ขึ้น "ขั้นตอนหลังพิมพ์" เว็บขึ้น "สินค้าพร้อมรับ". **root cause: `lib/ai-quote/track-flex.ts` อ่าน `deriveTrackStatus` แค่ `currentDept` ไม่เคยอ่าน `awaitingShipment`** (= `dept=post && staff=ship` คือคิวรอจัดส่ง) ทั้งที่ `lib/track-result.ts` (เว็บ) และ `lib/ai-quote/customer-jobs-flex.ts` อ่านครบทั้งคู่ — หลุดมาตั้งแต่ port `buildOrderFlex_` จาก Apps Script. แก้ badge + step builder ให้มิเรอร์ `Steps` ใน `app/track/client.tsx` (post = done ✓, สินค้าพร้อมรับ = current ●) + เทสต์ 2 เคส. **รอบสอง (#28)**: คุณนุ๊กเลือก "เอาตาม LINE" สำหรับรูปแบบวันที่ที่ต่างกัน (`5/9/2026` เว็บ vs `05/09/2026` LINE) → เพิ่ม `displayDatePadded` ใน `lib/jobs.ts` แล้วให้ `track-result.ts` + `customer-track.ts` ใช้ — **จอฝั่งพนักงาน (board/orders/archive/shipped) ยังใช้ `displayDate` แบบเดิม ไม่แตะ** (ถูกใช้ ~15 จุด เปลี่ยนทั้งระบบเกินที่ขอ). **ประเด็นที่ 3 — ชื่อลูกค้า mask ไม่ตรงกัน (`บร•••` vs `US•••`) ไม่ใช่บั๊ก**: คุณนุ๊กเดาว่าเป็น font แต่ซูมภาพ 10× แล้วเป็นคนละสคริปต์จริง (U/S ละติน vs บ/ร มีหัวขมวด) + จำนวนจุดไม่เท่า → ไล่ต่อจนยืนยันว่า `loadOrderFromPostgres` ดึง `WHERE id = ${id} LIMIT 1` แถวเดียว ทั้งสอง surface อ่าน `orders.raw->>'customer'` ตัวเดียวกัน = เป็นไปไม่ได้ที่โค้ดทำให้ต่าง → **สรุปว่าการ์ด LINE ในภาพเป็นข้อความเก่า (17:11) ที่ค้างชื่อก่อนถูกแก้ ส่วนเว็บเปิด 17:18** → คุณนุ๊กส่ง `/track` ใหม่หลัง deploy = **"บร" ทั้งคู่ ✅ ปิดเคส**. deploy: smoke ผ่านทั้ง 2 commit (10:37 + 10:42 UTC).
>
> ## ⏳ Pending (2026-09-17)
> 1. 🧠 **[MEMORY-QUEUE]** — session Mac ถัดไป save เข้า memory จริงแล้วลบ tag นี้: (a) **การ์ด Flex = ภาพนิ่ง** ส่งแล้วไม่อัปเดตตาม DB — ก่อนสรุปว่า "LINE ไม่ตรงกับเว็บ = บั๊ก" ต้องให้ส่งการ์ดใหม่เทียบก่อนเสมอ ไม่งั้นไล่บั๊กจาก snapshot คนละเวลา; (b) **surface ที่ port มาจาก Apps Script ต้อง audit ว่าใช้ผลลัพธ์ของ shared core ครบทุก field** — `deriveTrackStatus` ถูกสร้างมาเพื่อให้ 3 surface ตรงกัน แต่ field ที่เพิ่มทีหลัง (`awaitingShipment`) ไหลเข้าแค่ 2 ใน 3; (c) เมื่อ user เสนอสมมติฐานที่ตรวจสอบได้ (เช่น "น่าจะแค่ font") **ให้ตรวจก่อนเห็นด้วย** — ซูมภาพ 10× ด้วย `sharp` (มีใน node_modules อยู่แล้ว, PIL ไม่มีใน cloud container) พิสูจน์ได้ใน 2 นาที
> 2. 🧪 optional: `/audit` รอบเต็ม (ยกมาจาก 9/12) — ยังไม่ได้ทำ
> 3. 🐞 audit backlog เปิดค้าง 3 Low (ยกมาจาก 9/12): L2-archive-pill-date-value · L3-analytics-precedence-shipped-over-cancelled · L6-edit-page-metadata-title-static
> 4. 💡 backlog: Beam Checkout integration (ยกมาจาก 9/12 — memory `reference_beam_checkout_api.md`)
>
> ---
> **Session 2026-09-12 (Mac) — 🪦 Apps Script ปิดสนิท + สำรวจ Beam Checkout (ops + docs only, zero code change):** คุณนุ๊ก (`/session-start`) ถาม **Beam Checkout API** ก่อน (โรงพิมพ์สมัครไว้แล้วสำหรับรูดบัตร) → สำรวจ docs + ไล่ระบบเรา แล้ว **คุณนุ๊กสั่งเก็บเป็นข้อมูลไว้ก่อน ยังไม่ทำ** → บันทึกเป็น memory `reference_beam_checkout_api.md` (endpoint + auth + ค่าธรรมเนียม 1.80% บัตร / QR PromptPay ฟรี / T+1 บัตร T+3 QR + จุดต่อ A-E). **ข้อค้นพบที่ต้องจำ: QR Slip Verification ของ Beam แทน Thunder ไม่ได้** — มันรู้จักเฉพาะเงินที่วิ่งผ่าน Beam ไม่ใช่สลิปที่โอนเข้าบัญชีโรงพิมพ์ตรงๆ. จากนั้น **soak §15 ครบ 7 วัน → คุณนุ๊กลบโปรเจกต์ Apps Script "penprinting dashboard data"** → Claude กวาด doc ครบ 8 ไฟล์: workspace `CLAUDE.md` (สถานะ + deploy routing + ตาราง "อยากทำ X" + tree) · `Tech-Roadmap-Status.md` (4.3 ✅ + timeline + External + header) · `production-monitoring/monitoring.md` + `CLAUDE.md` · `dashboard-v2.md` · `migration-plan-apps-script-shrink.md` §15 ✅ · **ลบ `.claude/commands/check-quota.md`** · `/push-apps-script` ตัด target `dashboard`. **Phase 4.3 ปิดสนิท — ไม่มี Apps Script ใน live path หรือ rollback path ของระบบใดอีก** (Google Sheet = frozen archive, Postgres = sole source of truth). ⚠️ **rollback dashboard AS ไม่ได้แล้ว** — source ใน `production-monitoring/apps-script/dashboard/` เหลือเป็น reference อย่างเดียว. **ช่วงท้าย session**: context check ตอนเช้าอ่าน `NEXT-SESSION.md` บน main อย่างเดียว → พลาด session 9/11 (cloud) ที่ ship ครบแล้วบน branch ทั้ง session → เจอตอนไล่ `gh run list` เห็น sha แปลกปลอม. แก้ต้นเหตุแล้ว: `/session-start` Step 2 บังคับ `git fetch` + `gh pr list` ทั้ง 2 ฉบับ. จากนั้นคุณนุ๊กสั่ง **merge PR #26** → merge main เข้า branch (conflict เดียวที่ dashboard-v2 version history, additive เก็บทั้งคู่) — cloud session push merge ของตัวเองมาพร้อมกันหลังได้ SendMessage ของผม ใช้ของเขาแทน ([[same-tree-parallel-sessions]] ใช้ได้กับ cloud ด้วย) → รอ Vercel preview เขียว → merge `26dd734` + ลบ branch. และคุณนุ๊กยืนยันว่า **Thunder แก้หายแล้ว ตรวจสลิปได้ปกติ** → ปิด pending 9/11 ข้อ 2 (วินิจฉัยว่า root cause อยู่ขา Thunder↔ธนาคารถูกต้อง — หายเองโดยเราไม่ได้แก้อะไรฝั่งเรา).
>
> ## ⏳ Pending (2026-09-12)
> 4. 🧰 **สกิลใหม่ที่ลงไว้ 2026-09-12** (จาก [mattpocock/skills](https://github.com/mattpocock/skills) — รายละเอียด + กับดักอยู่ใน memory `reference_mattpocock_skills.md`): `domain-modeling` (⚠️ auto-trigger เวลาคุยเรื่องศัพท์ — งานแรกที่คุ้ม: pin precedence `cancelled > shipped` ลง `CONTEXT.md` แล้วปิด audit **L3** ไปในตัว) · `/wayfinder` · `/setup-matt-pocock-skills` (**ยังไม่รัน** — มันแก้ `CLAUDE.md` ของ repo + สร้าง `docs/agents/` + `.scratch/`; ถ้าจะรันให้รันใน repo นี้ ไม่ใช่ vault root แล้วดู diff ก่อน commit)
> 0. ✅ **CLOSED 2026-09-12 — PR [#26](https://github.com/witsarutnook/penprinting-dashboard/pull/26) merged** (merge commit `26dd734`, Vercel preview เขียวก่อน merge · gates เขียวบน tree ที่รวมแล้ว **706 tests**) → fix live บน prod แล้ว. ~~เปิดค้าง — ยังไม่ merge = fix ยังไม่ live~~ (branch `claude/check-slip-issue-b74d2n`, cloud session 2026-09-11 "ปัญหาเช็คสลิป"): แยก "ระบบขัดข้อง" ออกจาก "อ่านสลิปใบนี้ไม่ได้" + retry 1 ครั้ง + `_meta` ลง `slip_checks.raw`. สถานะ: MERGEABLE/CLEAN · Vercel preview เขียว · merge `main` (`f4b7c68`) เข้า branch แล้ว. **entry session 9/11 + pending ของมันอยู่บน branch ยังไม่ไหลเข้า main** — อ่านได้ที่ `git show origin/claude/check-slip-issue-b74d2n:NEXT-SESSION.md`. root cause ตัวจริงอยู่ฝั่ง Thunder (`SLIP_NOT_FOUND` รายใบ) ยังเปิด — ต้องแจ้ง support พร้อม transRef
> 1. 🧪 optional (ยกจาก 9/07): `/audit` รอบเต็มหลัง §13 + edit-lock (2 feature ใหญ่วันเดียว ยังไม่กวาดข้ามไฟล์) · smoke ตาเปล่า role พนักงาน → `/archive` redirect + `/shipped` ข้อความ "ต้องให้แอดมินค้นให้"
> 2. 🐞 audit backlog เปิดค้าง 3 Low (จาก round edit-lock 9/05): L2-archive-pill-date-value · L3-analytics-precedence-shipped-over-cancelled · L6-edit-page-metadata-title-static (+ stale comment `lib/api.ts:140`)
> 3. 💡 backlog: **Beam Checkout integration** (memory `reference_beam_checkout_api.md` — จุด A "ลิงก์ชำระต่อใบสั่งงาน" คุ้มสุด เพราะ `Order` ยังไม่มี field ชำระเงินเลย) — **คุณนุ๊กเลือกเก็บไว้ session หน้า: เริ่มด้วย `/wayfinder`** (งานนี้ decision เยอะกว่าโค้ด: มัดจำกี่ % · บวกค่าธรรมเนียม 1.80% หรือดูดเอง · QR PromptPay อย่างเดียวหรือเปิดบัตร · payment state เก็บยังไงไม่ให้ชน optimistic UI ของ /board) · DB-backed paper prices (Neon) · growth path `/archive` (pg_trgm + LATERAL หลัง LIMIT) เมื่อ orders โตเป็นหมื่นแถว
>
> ---
> **Session 2026-09-11 (☁️ cloud/iPad) — 🩹 เช็คสลิปมีปัญหา → root cause ฝั่ง Thunder + แก้ปลายทางของคำตอบฝั่งเรา (commit [`c233d38`](https://github.com/witsarutnook/penprinting-dashboard/commit/c233d38) บน branch `claude/check-slip-issue-b74d2n` — ⚠️ ยังไม่ merge = ยังไม่ live):** คุณนุ๊กถาม "ทำไมเช็คสลิปมีปัญหา" พร้อม screenshot การ์ด "อ่านสลิปไม่ได้" ในแชท OA + Thunder dashboard (API Status ไม่สำเร็จ, ประเภทสลิป "ไม่มีข้อมูล"). **(1) วินิจฉัย** — ไล่จากโค้ดก่อน: การ์ดเทาจะโผล่ได้ก็ต่อเมื่อ Thunder ถูกเรียกแล้วตอบ not-success ([webhook-router.ts:181-194](lib/ai-quote/webhook-router.ts)) = LINE webhook/download/Haiku pre-filter ผ่านหมด + โค้ด slip ไม่ถูกแตะตั้งแต่ 7/23 → ชี้ฝั่ง Thunder แต่ยังแยก quota/auth/รูปไม่ได้ → ขอให้คุณนุ๊กเปิด `slip-metrics?recent=20`. **`raw` จาก incident 7/23 คุ้มค่าตัวเองรอบที่สอง**: `SLIP_NOT_FOUND` "Bank slip not found or invalid" ทุกใบที่ fail = ตัด quota/auth/multipart/endpoint ออกหมด (คนละ code ทั้งนั้น + จะ fail 100% ไม่ใช่บางใบ). Pattern **รายใบ ไม่ใช่ช่วงเวลา**: id 1238 ✅ (21:31:28 · KBANK→พร้อมเพย์ 0707 · สลิปลงเวลา 21:06) แทรกกลาง 1235-1237 ❌ (21:31:15-16) กับ 1239-1241 ❌ (21:32-33); ใบเดิมส่งกี่รอบก็ fail รอบเดิม ส่วน `...CPP01815` ผ่านทุกรอบ → ขา Thunder↔ธนาคาร. **(2) Ship** — ไม่แตะวิธีเรียก Thunder เลย แก้แค่สิ่งที่ทำกับคำตอบ: `slipFailureKind()` แยก `'slip'` (Thunder ตัดสินรูปนี้แล้ว) ออกจาก `'system'` (ไม่ได้รู้อะไรเลย) อ่าน HTTP status ก่อน error code + case-insensitive (v2 ตะโกน / EasySlip snake_case) · `SlipState` เพิ่ม `'error'` header "ระบบขัดข้องชั่วคราว" · **copy ทั้ง 2 state เลิกขอให้ส่งรูปใหม่** (ของเดิมทำลูกค้าส่งซ้ำ = เผา Thunder quota ซื้อคำตอบเดิม — 3 รูป 3 รอบ 7 calls เปล่า) LINE+Messenger เปลี่ยนพร้อมกัน · retry 1 ครั้งเฉพาะ connection ตาย/5xx/408 (ไม่ retry 4xx/429) สร้าง FormData ใหม่ทุก attempt · `_meta {status, contentType, attempts}` ลง `slip_checks.raw` (jsonb เดิม ไม่ต้อง migrate) เพราะคืนนั้นแยก 429 กับ 500 ไม่ออก. Gates เขียว Node 22: type-check / lint 0-err / **706 tests (+20)** / build + **mutation check 4 แบบแดงตรงเป้า** (ปิด retry · retry SLIP_NOT_FOUND · ยุบ `'error'` · คืน copy เดิม). **(3) Self-correction** — เกือบ ship "fix" mojibake ไทยโดยอ้างว่า `res.json()` เชื่อ charset ที่ header ประกาศ → เทสต์จริง 3 charset ก่อน commit: **ผิด** fetch spec บอก "parse JSON from bytes" = UTF-8 เสมอ → ถอดทั้งโค้ดและเทสต์ที่อ้างว่ากันบั๊กตัวนั้นออก. **Lessons `[MEMORY-QUEUE]`**: (a) **ข้อความ error ที่สั่งลูกค้า retry หน้า paid API = ตัวคูณค่าใช้จ่าย** ถ้า retry นั้นสำเร็จไม่ได้ — ต้องแยก "ของจริงตอบว่าไม่ผ่าน" (deterministic, ห้ามชวน retry) ออกจาก "ระบบตอบไม่ได้" ตั้งแต่ชั้น classify ไม่ใช่ที่ชั้น copy (b) **สมมติฐานเรื่อง encoding/decoding ต้อง reproduce ให้เห็นก่อนเขียน fix** — 3 บรรทัด node พิสูจน์ได้ว่าผิด หลังเขียนโค้ด+เทสต์ไปแล้ว; และ **เทสต์ที่ผ่านทั้งก่อนและหลังแก้ไม่ใช่ guard** (mutation check จับได้ทันที) (c) evidence column ที่ลงทุนไว้ตอน incident เก่า (`slip_checks.raw` 7/23) ทำให้ incident ใหม่ปิดได้โดยไม่ต้อง repro — `_meta` รอบนี้คือการต่อยอดอันเดียวกัน
>
> ## ⏳ Pending (2026-09-11)
> 1. 🔍 **คุณนุ๊ก — mojibake ยังไม่สรุป (10 วินาที)**: เลื่อนดูการ์ด ✅ ใบล่าสุดในแชท LINE ว่าชื่อธนาคาร/ชื่อผู้โอนอ่านออกปกติไหม → **ปกติ** = เพี้ยนตอน copy JSON ใน Safari เฉยๆ ไม่ต้องแตะโค้ด · **เพี้ยน** = ข้อมูล Thunder เพี้ยนมาเอง ต้องใส่ตัวซ่อมแบบมีเงื่อนไข (ระวัง false positive กับชื่อที่ถูกอยู่แล้ว)
> 2. ✅ **CLOSED 2026-09-12 — คุณนุ๊กยืนยัน "Thunder แก้หายแล้ว ตรวจได้ปกติแล้ว"** (ไม่ต้องแจ้ง support แล้ว · ไม่ได้แก้อะไรฝั่งเรา = ยืนยันว่า root cause อยู่ขา Thunder↔ธนาคารจริงตามที่วินิจฉัย). ⚠️ ที่ ship ใน PR #26 ยังมีค่าอยู่: เป็น **ตัวกันความเสียหายรอบหน้า** — ถ้า Thunder พังอีก ลูกค้าจะเห็น "ระบบขัดข้อง ลองใหม่อีกครั้ง" แทน "อ่านสลิปไม่ได้" ที่ชวนส่งซ้ำ + `_meta` ลง `slip_checks.raw` ให้วินิจฉัยได้เร็วขึ้น. ~~root cause ตัวจริงยังเปิดอยู่ (ฝั่ง Thunder)~~ — ที่ ship วันนี้คือกันความเสียหาย ไม่ได้ทำให้สลิปอ่านได้. ทดสอบให้ apples-to-apples: กดค้างรูปสลิปที่ fail **ในแชท LINE** → Save image (ไบต์ที่ใกล้ที่สุดกับที่เราส่ง) → อัปโหลดที่หน้าทดสอบ `developer.thunder.in.th` → fail เหมือนกัน = ยืนยันฝั่งเขา แจ้ง support พร้อม transRef `016254210810BPP12360` + `016254213247APP11161` (ใบที่ผ่าน `016254210618CPP01815` เป็น control)
> 3. 📈 เปิด `/api/admin/slip-metrics` (ไม่ใส่ `?recent=`) ดู rollup 30 วัน — `unreadable` พุ่งหลัง Thunder migrate ไป EasySlip ปลาย ส.ค. หรือเป็นแบบนี้มานาน? มีผลต่อน้ำหนักตอนคุยกับ support
> 4. ✅ **CLOSED 2026-09-12 — merged + branch ลบแล้ว** (`26dd734`). ~~branch ยังไม่ merge~~ — `claude/check-slip-issue-b74d2n` · **PR [#26](https://github.com/witsarutnook/penprinting-dashboard/pull/26) เขียว** (Vercel preview success · 0 review threads · repo นี้ไม่มี CI บน PR — `smoke.yml` fire จาก Production `deployment_status` เท่านั้น ⇒ gate จริง = husky pre-commit). **merge `main` เข้า branch แล้ว** หลัง `f4b7c68` (§15 ปิด) ชน docs กัน — conflict เป็น additive ล้วน (ต่างคนต่างเพิ่ม session entry บนหัวไฟล์) เก็บทั้ง 2 entry ไม่มีใครทับใคร. **ยังไม่เข้า main = prod ยังใช้ copy เดิมที่ชวนลูกค้าส่งซ้ำอยู่** และ `_meta` ยังไม่ถูกเขียนลง `slip_checks.raw`. หมายเหตุ: cloud session นี้ subscribe PR + ตั้ง self check-in ซ้ำไว้จนกว่า PR merge/close — หยุดเองเมื่อ PR ปิด ไม่ต้องไล่ลบ
> ---
> **Session 2026-09-07 (Mac) — 🧹 วันเคลียร์ pending (ops-only, zero code change · docs commits `d7a7687` → `1d48cb2`):** คุณนุ๊ก (`/session-start`) เลือก B "เคลียร์ pending". ทำ: (1) **smoke lost-update gate `/admin` cards บน prod** ด้วย Chrome 2 แท็บบนการ์ด `demo` — แท็บ A บันทึก ★ ผ่าน → แท็บ B stale โดน reject "การ์ดถูกแก้ไขจากที่อื่นระหว่างที่เปิดฟอร์มอยู่…" → public ยัง ★ → revert เรียบร้อย (หน้า + vCard ตรงของเดิม) (2) ที่อยู่ `rathakul` = `ksk` เป๊ะ → คุณนุ๊กยืนยัน "ออฟฟิศเดียวกัน ถูกแล้ว" (3) `/demo` ที่ 375px Android-UA: layout ครบ + href ทุกปุ่มถูก scheme + `?s=n`/`?s=q` 200 + vCard CRLF/ไทยถูก (4) คุณนุ๊กปิดส่วนที่ต้องเครื่องจริง/บัญชีเอง: มือถือจริง import vCard + เด้งแอป · NFC 1 ใบ · ทีม login รหัสใหม่ · แจ้ง ksk · ไล่แชท OA → **carryover 9/02 ปิดทั้งชุด** (5) ตัดสิน **ไม่ reissue LINE token** (ปิดถาวร) (6) soak §15 วันที่ 3/7 — AS URL ยัง 404, prod smoke เขียว, ไม่มี commit ใหม่จากใคร. **Lessons**: (a) Chrome MCP: ปุ่ม submit ใน **background tab** กดแล้วไม่ submit (React onSubmit ไม่ fire) ทั้งที่ tool ตอบ "Clicked" — แท็บ front ทำงานปกติ; workaround ที่ใช้ได้ทั้ง 2 แท็บ = `javascript_tool` → `form.requestSubmit()` (ผ่าน onSubmit เดียวกัน) + ตั้งค่า input ด้วย native value setter + `input` event; ยืนยันผลจาก public page ด้วย curl เสมอ ไม่เชื่อ "Clicked" (เพิ่มลง memory [[browser-automation-hidden-tab-smoke]]) (b) iOS Simulator tool ต้องมี Xcode เต็ม — เครื่องนี้ไม่มี → smoke มือถือจริงยังต้องคุณนุ๊ก
>
> ## ⏳ Pending (2026-09-07)
> 1. ✅ **CLOSED 2026-09-12** — ~~soak §15 ถึง ~2026-09-12~~ → คุณนุ๊กลบโปรเจกต์ Apps Script "penprinting dashboard data" (+ เช็ค trigger `r2BackupWeekly` ถูกลบด้วย) → Claude ติ๊ก 🪦 ใน workspace CLAUDE.md / monitoring.md / Tech-Roadmap + ลบ `.claude/commands/check-quota.md`
> 2. 🧪 optional: `/audit` รอบเต็มหลัง §13 + edit-lock (2 features ใหญ่ในวันเดียว ยังไม่กวาดข้ามไฟล์) · smoke ตาเปล่า role พนักงาน → `/archive` redirect + `/shipped` ข้อความ "ต้องให้แอดมินค้นให้"
> 3. 💡 backlog: DB-backed paper prices (Neon) · growth path `/archive` (pg_trgm + LATERAL หลัง LIMIT) เมื่อ orders โตเป็นหมื่นแถว
>
> ---
> **Session 2026-09-05 (Mac, คู่ขนานกับ §13) — 🔒 edit-lock port: shipped/cancelled "แก้ไขไม่ได้" ย้ายจาก render ของ orders-table เข้า route `/orders/[id]/edit` + 409 ที่ `/api/orders/update` (TDD · tests 660→685 · commit [`6d052a0`](https://github.com/witsarutnook/penprinting-dashboard/commit/6d052a0) · deploy + post-deploy smoke เขียว — [run 33964893961](https://github.com/witsarutnook/penprinting-dashboard/actions/runs/33964893961) pin `6d052a0`):** chip จาก review `4da9191` (/archive ซ่อนลิงก์แก้ไขฝั่ง client แต่ route ยังเปิดได้). ทำ: (1) `lib/order-lock.ts` helper ตัวเดียว (`orderLockReason` / `isOrderLocked` / `orderLockMessage`) mirror `lib/orders-list` — cancelled > shipped, status normalise, รับทั้ง row object และ EXISTS boolean (2) `loadOrderLockStateFromPostgres` 1 query (status + EXISTS shipped/cancelled) + wrapper `loadOrderLockState` (3) API เช็คล็อคก่อน validate + ก่อน `srcUnchanged` gate → 409 `{error, locked}` (4) edit page แสดง panel แทนฟอร์ม (pill + ลิงก์ /orders · /print · /shipped|/cancelled). `/audit` (penprinting-auditor) รอบหลังทำ: 0C / 0H / 1M / 10L — ปิดในคอมมิตเดียวกัน: **M1** test pin ว่า lock รันก่อน field validation (locked + body ว่าง → 409 ไม่ใช่ 400) · L1 comment ระบุ status source (`raw->>'status'` = ตัวเดียวกับ /orders pill) · L4 a11y ถอด `role=status`/`aria-live` บน static section → `aria-labelledby` · L5 header `flex-wrap` + `truncate` · L7 lock null → 404 เหมือน API · L8 route test แยก status-only. **Deferred**: L2 `/archive` pill ตัดสินจาก date value (`shippedDate`/`cancelledAt`) ไม่ใช่ row existence → shipped row ที่ date ว่าง (Sheet-era) โชว์ "กำลังทำ" + ลิงก์แก้ไข แล้วชน lock panel (integrity ไม่กระทบ; fix = LATERAL ส่ง `s.id IS NOT NULL` หรือ page เรียก `isOrderLocked`) · L3 `lib/analytics.ts:448` precedence shipped > cancelled กลับด้านกับทุกที่ (report row only) · L6 `metadata.title` "แก้ไขใบสั่งงาน" static บนหน้า locked (ต้อง generateMetadata) · stale comment `lib/api.ts:140` อ้าง `/api/orders/delete` ที่ไม่มี. **Lessons**: (a) กฎ permission ที่อยู่แค่ใน render = ทุก entry point ใหม่ต้อง copy กฎเอง (archive เป็นตัวอย่างสด) → ย้ายเข้า route + API แล้วให้ UI เป็นแค่ hint (b) ทำ lock read เป็น query เดียวที่ทั้ง page + API ใช้ร่วม = UI กับ server ไม่มีทางเห็นไม่ตรงกัน (c) session คู่ขนานใน tree เดียว: SendMessage ปิดอยู่ฝั่งผม แต่ peer ส่งข้อความมาได้ — ประสานผ่าน git status + ไม่แตะไฟล์ WIP ของกันและกัน (peer ฝาก NEXT-SESSION.md ที่ staged ไว้ให้รวม commit นี้).
>
> **Session 2026-09-05 (Mac) — 📦 §13 archive port: `/archive` ค้น Postgres ทุกปี + ถอด Apps Script client ทั้งก้อน (§15 ขั้นแรก) — spec → plan → subagent-driven ship ใน session เดียว (tests 635→660 · 15 commits [`fcd6b30`](https://github.com/witsarutnook/penprinting-dashboard/commit/fcd6b30) spec → [`bc60fc9`](https://github.com/witsarutnook/penprinting-dashboard/commit/bc60fc9) · deploy + post-deploy smoke เขียว pin sha · prod smoke ผ่าน Chrome ครบ):** คุณนุ๊ก (`/session-start`) เลือก A "§13 archive port". **ข้อค้นพบก่อนออกแบบ (ยืนยันจากของจริงผ่าน Chrome ไม่ใช่จาก doc)**: Google Sheet มีแค่ 8 tabs **ไม่มี `*_archive_*` เลย** + `/archive` บน prod ค้น "20" ได้ 0 → Apps Script `searchArchive` ไม่เคยคืนอะไรตั้งแต่ port; Postgres มีทุกแถว (mirror เต็ม Phase 1.5 + เขียนตรงตั้งแต่ 5/18, audit_log 180 วันครอบอายุระบบ) → **ไม่มี Sheet archive ให้ migrate** — scope หดจาก "2-3 sessions" ใน plan เหลือ data-source swap. Decisions (brainstorm): D1 ค้นใบสั่งงานทุกปี+สถานะ · D2 ถอด AS client ทั้งก้อน + soak 7 วัน · D3 ILIKE บนคอลัมน์จริง (ไม่ tsvector — ตัดคำไทยไม่ได้; ไม่ขยาย /orders — ชน windowing) · D4 admin-only เดิม · D5 L4 ปิด by design. Ship: (1) [lib/archive-search.ts](lib/archive-search.ts) — `normalizeArchiveQuery` (≥2 ≤100 code points) / `escapeLikePattern` / `searchArchiveOrders` (orders ILIKE name·customer·orderer + id prefix เมื่อตัวเลขล้วน, `LEFT JOIN LATERAL` shipped/cancelled ล่าสุด, `COUNT(*) OVER()` = 1 round-trip, LIMIT 100 clamp, self-normalize) / `archiveRowState` (cancelled > shipped > draft > active) — 25 tests (2) `/archive` ตารางเดียว + pill + ลิงก์ print (edit ซ่อนบน shipped/cancelled ตามกฎ orders-table) + a11y + `min-w` (3) `/shipped` + `/cancelled` บอกว่าแสดง 12 เดือน + ปุ่ม "ค้นข้อมูลเก่า" (admin) → **L4 closed** (4) **lib/api.ts เหลือ Postgres readers** — ลบ post/searchArchive/AppsScriptError/currentActor/getApiBase + sweep `instanceof AppsScriptError` 8 ไฟล์ + `/api/audit` → 503 บน `PostgresReadError` — `grep -rn APPS_SCRIPT app lib components` = **0** (5) docs กวาด 8 ไฟล์ (dashboard 4 + workspace CLAUDE.md / monitoring.md / Tech-Roadmap / check-quota). **Subagent-driven review จับ Important 4 ตัวก่อน ship** (implementer ตามโค้ดใน plan เป๊ะ = บั๊กอยู่ใน plan): NaN limit หลุด clamp → SQL error · `''` → `ILIKE '%%'` dump ทั้งตาราง · ปุ่มแก้ไขบนแถว shipped/cancelled ขัดกฎ /orders · cancelledAt ISO ดิบเมื่อไม่มีเหตุผล — ปิดใน [`466d46d`](https://github.com/witsarutnook/penprinting-dashboard/commit/466d46d) + [`4da9191`](https://github.com/witsarutnook/penprinting-dashboard/commit/4da9191). Prod smoke (Chrome admin): `?q=2026` 820 รายการ แสดง 100 · `?q=Kojimori` 1 · `?q=2026040` 72 (pill ยกเลิก+เหตุผล / ร่าง+แก้ไข / ส่งแล้ว+วันที่ ครบ) · `?q=%%` ไม่พบ (escape จริง) · `?q=a` hint · print order เก่าสุด 202604001 เปิดได้ · hint+ปุ่มบน /shipped //cancelled. Chip แยก (คุณนุ๊กเปิด session คู่ขนานแล้ว): status guard ใน `/orders/[id]/edit` route (กฎ shipped/cancelled อยู่แค่ render ของ orders-table). Spec/plan: [docs/superpowers/specs/2026-09-05-archive-postgres-port-design.md](docs/superpowers/specs/2026-09-05-archive-postgres-port-design.md) · [docs/superpowers/plans/2026-09-05-archive-postgres-port.md](docs/superpowers/plans/2026-09-05-archive-postgres-port.md). **Lessons**: (a) [[roadmap-doc-drift]] เคส 2 = scope drift — plan ประเมินจากสมมติฐานว่า archive tabs มีอยู่; **ก่อน scope งาน migration/port ต้องเปิดตัวข้อมูลจริงก่อนเสมอ** (5 นาที ประหยัดได้เป็น session) — memory updated (b) plan code block ที่ copy ลง implementer ตรงๆ = บั๊กใน plan ไหลผ่าน TDD ได้ (test ก็มาจาก plan เดียวกัน) → reviewer อิสระคุ้มค่า
>
> ## ⏳ Pending (2026-09-05)
> 1. ✅ **Vercel env ถอดแล้ว + Redeploy + verify ผ่าน (2026-09-05 ~12:05)** — deployment `56a9465` Production success + post-deploy smoke เขียว · Claude เช็ค prod: `/archive?q=Kojimori` เจอ · `/analytics` โหลดปกติ · `/api/audit?orderId=202604001` ตอบ JSON · print order เก่าเปิดได้ = runtime ไม่พึ่ง `APPS_SCRIPT_*` แล้วจริง — ~~เดิม: 🔑 **คุณนุ๊ก — Vercel env (ทำได้เลย)**: Settings~~ → Environment Variables → ลบ `APPS_SCRIPT_URL` + `APPS_SCRIPT_TOKEN` (Production/Preview/Development) → **Redeploy** → Claude verify build เขียว + `/archive?q=2026` ยังค้นได้ (positive control ว่า runtime ไม่แตะ env นี้แล้ว)
> 2. ✅ **Apps Script deployment archive ครบทุกตัวแล้ว (2026-09-05 21:05) — probe URL ได้ 404 HTML ของ Google (ไม่ใช่ JSON) = URL ตายจริง · soak เริ่มวันนี้ → ลบโปรเจกต์ ~2026-09-12** · ตัวที่ค้างคือ deployment ชื่อ "Untitled" (Version 44 May 28 = URL หลักที่ Edit-existing มาตั้งแต่ยุค WP) ถูกมองข้ามตอน archive รอบแรกเพราะไม่มีชื่อ version — หาเจอด้วย Executions + Manage deployments · ประวัติ: 🟡 คุณนุ๊ก archive deployment + ลบ trigger (เย็น) แต่ probe จากภายนอกพบ deployment `AKfycbzcoYTdbwRE…` (URL ที่ WP `page-production-monitoring.php`/`page-track-order.php` เคยใช้) ยังตอบ 200 JSON `{"error":"Unauthorized"}` = ยัง active** → โปรเจกต์น่าจะมีหลาย deployment; ต้อง archive ทุกตัวที่ยัง Active ใน Manage deployments (ไม่มี consumer เหลือแล้วทั้งหมด) แล้ว Claude probe ซ้ำ (คาดว่าได้ HTML error ไม่ใช่ JSON) — ~~เดิม: 🗄 **คุณนุ๊ก — Apps Script editor** (scriptId ใน~~ `production-monitoring/apps-script/dashboard/.clasp.json`): Deploy → Manage deployments → **Archive** web-app deployment ของ dashboard · Triggers → ลบ `archiveOldData` ถ้ามี · **ยังไม่ลบโปรเจกต์** (rollback path ระหว่าง soak)
> 3. ✅ **CLOSED 2026-09-12** — ~~soak 2026-09-05 → ~2026-09-12~~ (เช็คด้วยว่า trigger `r2BackupWeekly` ที่ fail ทุกสัปดาห์ถูกลบไปพร้อม `archiveOldData` แล้ว — Executions 8/30 ยังเห็น): ไม่มี Sentry error จาก /archive / print / edit ของ order เก่า → คุณนุ๊กลบโปรเจกต์ Apps Script = §15 จบ → Claude ติ๊ก "ลบแล้ว" ใน workspace CLAUDE.md + monitoring.md + Tech-Roadmap + ลบ `.claude/commands/check-quota.md`
> 4. ~~🔄 session คู่ขนาน (chip): status guard `/orders/[id]/edit` shipped/cancelled~~ ✅ **ปิดแล้ว** — ดู entry 🔒 edit-lock port ด้านบน (commit `6d052a0`)
> 5. 🧪 optional: `/audit` รอบเต็มหลัง §13 (subagent review ครอบต่อ task แล้ว; audit ข้ามไฟล์ยังไม่ทำ) · smoke ตาเปล่า role พนักงาน → `/archive` redirect `/analytics` + `/shipped` เห็นข้อความ "ต้องให้แอดมินค้นให้" ไม่มีปุ่ม
> 6. 🔁 carryover 9/02 — **update 2026-09-07 (Claude เคลียร์ส่วนที่ทำแทนได้)**: ✅ smoke /admin cards lost-update gate บน prod (Chrome 2 แท็บ, การ์ด demo: แท็บ A บันทึก ★ ผ่าน → แท็บ B stale โดน reject "การ์ดถูกแก้ไขจากที่อื่น…" · public ยัง ★ · revert เรียบร้อย; หมายเหตุ: ปุ่มบันทึกใน background tab ของ Chrome MCP ไม่ submit — ใช้ `form.requestSubmit()` แทน) · ✅ vCard demo header/CRLF/ไทย ปกติ · ✅ ที่อยู่ `rathakul` = `ksk` (9/54 หมู่บ้านพิมานธานี หมู่ 11 ต.เมืองเก่า) — **คุณนุ๊กยืนยัน 2026-09-07: ถูกต้อง ออฟฟิศเดียวกัน** ไม่ต้องแก้ (ปิดข้อสงสัยตั้งแต่ 8/28) · ✅ (emulated) `/demo` ที่ 375px Android-UA ใน Browser pane: layout ครบ + href ทุกปุ่มถูก scheme (`tel:+66…` / `line.me/R/ti/p/@…` / `maps.google.com` / `/demo/contact.vcf`) + `?s=n` `?s=q` 200 — **ยังต้องมือถือจริงเฉพาะ** vCard import เข้า Contacts + app handoff (iOS Simulator ใช้ไม่ได้: เครื่องไม่มี Xcode เต็ม) · ✅ **คุณนุ๊กยืนยัน 2026-09-07 ทำครบแล้ว**: ทีมลองรหัส admin ใหม่ · แจ้ง ksk ธีม ocean-blue · smoke `/demo` มือถือจริง (import vCard + เด้งแอป) · เขียน NFC จริง 1 ใบ `?s=n` · ไล่แชท OA 8/27 → **carryover 9/02 ปิดทั้งชุด** · ~~reissue LINE token~~ **คุณนุ๊กตัดสิน 2026-09-07: ไม่ reissue** (watch 8/29 ยืนยัน webhook เป็นของเราต่อเนื่อง + Thunder ปลด token แล้ว — ปิดเรื่อง ไม่ต้องยกมาอีก)
> 7. 💡 backlog: DB-backed paper prices (Neon) · growth path `/archive`: `pg_trgm` GIN บน orders(name, customer) + ย้าย LATERAL หลัง LIMIT (CTE) เมื่อ orders โตเป็นหมื่นแถว — query contract ไม่ต้องแก้
>
> ---
> **Session 2026-09-02 (Mac) — 💳 cards: audit round แรกทั้งโปรเจกต์ → ปิดครบ 13/13 same-day (0H+4M+9L · TDD 72→111 tests · 10 commits [`673e084`](https://github.com/witsarutnook/penprinting-cards/commit/673e084)→[`f70c33b`](https://github.com/witsarutnook/penprinting-cards/commit/f70c33b) · deploy+smoke เขียว):** คุณนุ๊ก (`/session-start`) เลือก "Audit round ฝั่ง cards" → auditor สแกนทั้งโปรเจกต์ (per-task review เคยครอบแล้ว แต่ไม่เคย round เต็ม) → คุณนุ๊กสั่ง **"ปิดหมดเลย"**. ไฮไลต์: (1) **M1** create race ทับการ์ดเงียบ (ON CONFLICT class เดิม) → `createCard` plain INSERT+23505 (2) **M2** payload เสีย 1 row เคย 500 ทั้ง /admin+sitemap → safeParse ต่อ row + error.tsx (3) **M3** policy noindex 8/29 กลายเป็น default ในโค้ด (4) **M4** contrast guard ใน schema — override accentText/textMuted ได้แล้ว + save ที่อ่านไม่ออกโดน reject (5) **L5** edit lost-update gate บน base updated_at (retire upsertCard — ทุก write path explicit) (6) **L7+L2** logo sniff magic bytes + ลบ blob เก่าตอนเปลี่ยน (7) **L4+L6+L3+L1+L8+L9** — toggle error banner / rate-limit นับเฉพาะ fail / junk-slug กัน cache bloat / disabled ไม่ leak OG / vCard fold RFC 2426 (ตัดที่ code point ไม่แตกอักษรไทย) / จดห้ามปิดการ์ด demo. **Deploy-risk check ก่อน push**: sample CSS vars การ์ด live ทั้ง 4 → ไม่มี override → M4 ปลอด regress. Smoke prod เขียวครบ (pin deployment ของ `f70c33b`): การ์ด 4 ใบ + vcf fold จริงบน ksk + admin gate + sitemap root-only. รายละเอียดเต็ม + Verified clean list ใน AUDIT-BACKLOG "2026-09-02". **Lessons**: (a) `vitest | grep` ใน pipeline เดียวกับ `&& commit` กลืน exit code — เกือบ commit ทั้งที่ 2 tests แดง (จับได้จาก summary line) → เช็ค exit จริงแยกคำสั่งเสมอ + memory ใหม่ [[pipeline-grep-masks-exit-code]] (b) rtk บิดผล vitest suite-level error ("PASS (0) FAIL (0)" ทั้งที่ import พัง) — ยืนยันด้วย `rtk proxy` อีกเคส ตาม [[rtk-output-distortion]]
>
> ## ⏳ Pending (2026-09-02)
> 1. 🧪 **Smoke /admin จอจริงเบาๆ (คุณนุ๊ก, เมื่อสะดวก)**: แก้การ์ดใดก็ได้ 1 ครั้ง → บันทึกผ่าน (ยืนยัน lost-update gate ไม่ block งานปกติ) · ลองเปิดฟอร์ม 2 แท็บ แก้แท็บแรก save แล้ว save แท็บสอง → ต้องเจอ error "ถูกแก้ไขจากที่อื่น" (logic มี test ครอบแล้ว ข้อนี้ยืนยันตาเปล่า)
> 2. ⚠️ หมายเหตุ UI: การ์ดใหม่ตอนนี้ default ติ๊ก noindex อยู่แล้ว (ไม่ต้องติ๊กเอง) + override สีเพิ่ม accentText/textMuted แล้ว — ถ้า override แล้ว save ไม่ผ่านเพราะ contrast = ตั้งใจ ระบบบอกคู่สีที่พัง
> 3. 🔁 carryover 8/29 (ยังค้างทั้งชุด): ทีมลองรหัส admin ใหม่ 1 ครั้ง · แจ้งลูกค้า ksk ดูธีม ocean-blue · smoke มือถือจริง `/demo` (iPhone+Android) · เขียน NFC จริง 1 ใบ (`?s=n`) · ยืนยันที่อยู่ `rathakul` (สงสัย copy จาก ksk)
> 4. 💬 carryover 8/27: ไล่แชทค้าง OA ช่วง Thunder outage (ถ้ายังไม่ทำ) · 🔐 hardening reissue LINE token (optional ไม่เร่ง)
> 5. 💡 backlog เดิม: DB-backed paper prices (Neon) · L4 year filter ฝั่ง dashboard (ผูก §13, deadline 2027-05-18)
>
> ---
> **Session 2026-08-29 (Mac) — 💳 cards: action-layer tests + preset `ocean-blue` (ลูกค้า ksk) + เพิ่ม admin users + ปิด watch Thunder ด้วย DB evidence (3 commits cards: [`e13516e`](https://github.com/witsarutnook/penprinting-cards/commit/e13516e) → [`b9d145a`](https://github.com/witsarutnook/penprinting-cards/commit/b9d145a)):** `/session-start` → คุณนุ๊กเลือก A แล้วสลับ B กลาง turn: (1) **Watch Thunder (carryover 8/27) ปิดสมบูรณ์** — คุณนุ๊กยืนยันการ์ดเขียวไม่โผล่ + Claude ยืนยันชั้น DB ผ่าน `slip-metrics?recent=20` (Chrome session): `slip_checks` ไหลต่อเนื่อง 28–29/08 ทั้ง LINE+Messenger, สลิปจริง verified+matched ครบทุกใบ (7,000฿/642฿/72฿/ชุด 100฿×8), รูปไม่ใช่สลิปโดน filter ถูก, ไม่มี `VALIDATION_ERROR` = webhook ยังเป็นของเรา; hardening reissue token คง optional ตามเดิม (2) **cards action-layer tests** [`e13516e`](https://github.com/witsarutnook/penprinting-cards/commit/e13516e) — ปิด residual จาก final review เฟส 2: 22 tests (49→71) ครอบ `saveCardAction` ทุก branch (auth gate รวม revocation / JSON parse / slug freeze / db-read fail / dup slug / logo required+type+size / blob fail / ZodError flatten / success + tag busts) + loginAction (รหัสผิด/cookie/rate-limit XFF hop แรก) + logout + toggle; mock ชั้น next/*+blob+cards-db แต่ **admin-auth ใช้ HMAC token จริง**; mutation check: ถอด slug-freeze guard → fail ตรงเป้า 1 test (3) **preset `ocean-blue`** [`e0380af`](https://github.com/witsarutnook/penprinting-cards/commit/e0380af) — ลูกค้า ksk ขอโทนโลโก้: sample pixel จริงด้วย PNG decoder zlib ล้วน (เครื่องไม่มี PIL/ImageMagick) ได้คลื่นเข้ม `#00587C` (= accent ตรงตัว) + ฟ้า `#30B0D8`; โครง fresh hue-shift เป็นน้ำเงิน; WCAG ทุกคู่ผ่าน (ต่ำสุด 4.85, invariant test ครอบอัตโนมัติ); TDD RED-first 71→**72 tests**; picker /admin ได้ปุ่มใหม่ฟรี (iterate จาก `THEME_PRESETS`); **สลับการ์ด ksk ผ่าน /admin ด้วย Chrome** — รอ redeploy live ก่อน save (ใช้ปุ่มที่ 4 ใน picker เป็น deploy probe กันหลุม [[db-migrate-stale-deploy]]) + verified หน้าจริง header/footer (4) **เพิ่ม admin users** (คุณนุ๊กแก้ `ADMIN_AUTH_USERS` + Redeploy เอง หลังผม walk-through) — verify แบบไม่แตะรหัส: redeploy Ready+current บน Vercel + **positive control**: session `nook` เดิมยังเข้า /admin ได้ = JSON shape ถูก (ระบบ fail-closed — ถ้า shape พังทุกคนหลุดรวม nook) (5) **🔍 ระหว่าง verify เจอการ์ดทุกใบกลายเป็น noindex + sitemap เหลือ root → ถามก่อนแตะ → คุณนุ๊กยืนยัน "ตั้งใจ"** = policy ใหม่ (นามบัตร = หน้าส่วนตัว เข้าผ่าน NFC/QR ตรง) → จดลง README + intake checklist [`b9d145a`](https://github.com/witsarutnook/penprinting-cards/commit/b9d145a) กัน session หน้าเปิด investigation ซ้ำ. **Lessons**: (a) Chrome ref-click โดนปุ่มข้างเคียง — สั่งคลิก ocean-blue แต่ minimal-white ถูก select (เกือบ save ธีมผิดใส่การ์ดลูกค้าจริง; จับได้เพราะ screenshot ก่อนกดบันทึก) → memory ใหม่ [[browser-ref-click-verify]] (b) verify env-var auth change โดยไม่แตะรหัสผ่าน: ระบบ fail-closed ให้ positive control ผ่าน session เดิมได้เลย
>
> ## ⏳ Pending (2026-08-29)
> 1. 🧪 **ทีมที่ได้รหัส admin ใหม่ลอง login 1 ครั้ง** — ยังไม่มีใครยืนยันรหัสใหม่ใช้ได้จริง (ชื่อที่ตั้งใน map จะโชว์ในคอลัมน์ "แก้ล่าสุด")
> 2. 💬 แจ้งลูกค้า ksk ดูธีมใหม่ `ocean-blue` ที่ [card.penprinting.co/ksk](https://card.penprinting.co/ksk) — อยากจูนสีเฉพาะจุดใช้ "override สีรายตัว" ใน /admin ได้ไม่ต้องแตะโค้ด
> 3. 🔁 carryover 8/28 ยังค้าง: smoke มือถือจริง `/demo` (iPhone+Android → บันทึกรายชื่อ) · เขียน NFC จริง 1 ใบ (`?s=n`) · ยืนยันที่อยู่ `rathakul` (การ์ดถูกแก้วันนี้ระหว่างติ๊ก noindex แต่ยังไม่ confirm ว่าที่อยู่ 9/54 พิมานธานี ถูกต้องหรือ copy พลาดจาก ksk)
> 4. 💬 carryover 8/27: ไล่แชทค้าง OA ช่วง Thunder outage (ถ้ายังไม่ทำ) · 🔐 hardening reissue LINE token (optional — watch ปิดแล้ว ไม่เร่ง)
> 5. 💡 backlog เดิม: DB-backed paper prices (Neon) · L4 year filter (ผูก §13, deadline 2027-05-18)
>
> ---
>
> **Session 2026-08-28 (Mac) — 💳 โปรเจกต์ใหม่: penprinting-cards (นามบัตร NFC landing pages) — spec → plan → build → LIVE ใน session เดียว:** คุณนุ๊กปรึกษางานใหม่ (ลูกค้าสั่งนามบัตร NFC อยากได้ landing page รวมลิงก์แบบตัวอย่าง K.S. Ideal) → brainstorm ตัดสิน: **product ขายซ้ำ · แยกโปรเจกต์+subdomain `card.penprinting.co/<slug>`** (เหตุผลชี้ขาด: URL ฝังใน NFC/QR ที่พิมพ์แจกแล้วแก้ไม่ได้ตลอดชีพ → ต้อง decouple จาก marketing site) · เฟสแรกทีมจัดการข้อมูล (JSON ต่อราย + Zod strict, build fail เมื่อ data พลาด) · template เดียว + **3 theme presets** (`luxury-gold`/`minimal-white`/`fresh` — คุณนุ๊กสั่งเพิ่ม 2 ตัวหลังกลาง session) + vCard บันทึกรายชื่อ. Execute แบบ subagent-driven (implementer + spec review + quality review ต่อ task, 11 tasks): repo ใหม่ [`witsarutnook/penprinting-cards`](https://github.com/witsarutnook/penprinting-cards) ~26 commits, **31 unit tests** (มี WCAG contrast invariant ทุก preset — จับ footer minimal-white 1.19:1 ก่อน ship), Vercel project + domain (คุณนุ๊ก import เอง) → **smoke prod เขียวครบ**: /demo 200 + theme + vCard headers/CRLF/ไทย + 404 + robots/sitemap + root 307. Review catches สำคัญ: (1) **Next force-static route demotion** — การ์ด `enabled:false` ตัวเดียวทำ non-200 prerender → route ทั้งเส้น demote เป็น runtime → slug มั่ว 500 แทน 404; fix = filter enabled ใน `generateStaticParams` ของ vcf route (2) `.strict()` ทั้ง schema กัน typo key หายเงียบ (3) tel:/mailto: ห้าม `_blank` (LINE in-app browser) (4) logo SVG → LINE/FB share preview ไม่ขึ้น (validate เตือน) + แชร์ลิงก์ผ่าน LINE ต่อ `?openExternalBrowser=1`. Docs: spec+plan ใน `penprinting-cards/docs/superpowers/` + README + intake checklist + workspace CLAUDE.md updated.
>
> **[ต่อในเซสชันเดียวกัน — เฟส 2 จบใน session เดียวเช่นกัน] 🗄️ penprinting-cards เฟส 2: DB-backed + หน้า /admin — LIVE:** คุณนุ๊กถาม "ทำหน้า setting ให้ admin ใส่ข้อมูลผ่านเว็บได้มั้ย" → เลือกทาง B (DB เต็มรูปแบบ) → spec+plan (14 tasks, deploy-safe ordering) → subagent-driven อีกรอบ (~18 commits ปลายที่ [`fa43675`](https://github.com/witsarutnook/penprinting-cards/commit/fa43675)): **Neon database ใหม่ `cards`** (instance เดิม, ตารางเดียว payload jsonb + Zod strict ทั้ง read/write) · **Vercel Blob** เก็บโลโก้ (store Public + read-write token) · **`/admin`** ครบ: login HMAC (รหัสแยกชุด, isActiveUser revocation, rate-limit) + ตาราง + ฟอร์ม new/edit (links editor / theme picker 3 presets / override hex advanced / upload โลโก้ / vcard รวม title) · **rendering cutover**: หน้าการ์ด+vcf+sitemap อ่าน DB ผ่าน `unstable_cache` tag ต่อราย ไม่มี TTL → **กดบันทึก/toggle ใน admin เห็นผลบนหน้าจริงทันที (E2E verified บน prod ผ่าน Chrome: disable→CardDisabled+vcf 404→enable กลับ · แก้ nameEn ★→โผล่ทันที→แก้กลับ)** · **หน้าแรก `/`** เปลี่ยนจาก redirect เป็นหน้าขายบริการ NFC (indexable, contact จริงจาก penprinting-web: lin.ee/6pxIW7z + 080-314-0707) · **retire ระบบไฟล์ทั้งชุด** (content/ + lib/cards + validate + import endpoint ตาม PATTERNS §2.5) — tests สุดท้าย 49/49. Ops (คุณนุ๊กกดเอง ผม guide + ผมรัน migrate/import ผ่าน Chrome session): Neon db + Blob store + env 3 ตัว (Secret) — เจอ 2 หลุมจริง: ลืม `{` ใน ADMIN_AUTH_USERS (fail-closed = "รหัสผ่านไม่ถูกต้อง") + Blob dialog ต้องเลือก Public + ติ๊ก read-write token เอง. Review catches สำคัญรอบนี้: ADMIN_AUTH_USERS ผิด shape = fail-open (แก้เป็น shape-guard + Object.hasOwn) · Next 15 server-action bodySizeLimit 1MB default ทำโลโก้ 2MB ตายก่อนถึง validation (→3mb) · React 19 auto form-reset ทำไฟล์โลโก้หายเงียบตอน error round-trip (→เก็บ File ใน state) · sitemap metadata route ต้อง force-dynamic. Final review: **Phase 2 done** — residual จดไว้: action-layer tests ยังไม่มี (backlog) · legacy /cards/ branch ใน schema dead-ish · cache tag cards:list บัสต์กว้าง (ตั้งใจ).
>
> **[แถมหลังปิดงาน — bug report สดจากคุณนุ๊กระหว่างใช้ /admin จริง]**: กรอกลูกค้าจริงรายแรก (`ksidealgroup` — เค.เอส.ไอเดียล กรุ๊ป ✅ live แล้วทั้งหน้า+โลโก้ Blob+vCard) แล้วเจอ dropdown ทุกแถวเด้งเป็น `phone` ตอนฟอร์มคืน error → root cause = React 19 auto form-reset บน `<form action>` (ผิว select/checkbox — คนละผิวกับ File input ที่ review จับไว้ก่อนหน้า) → fix ที่ราก: onSubmit + preventDefault + dispatch ใน startTransition ([`85ecf2a`](https://github.com/witsarutnook/penprinting-cards/commit/85ecf2a)) — E2E verified บน prod: error โชว์ + select/checkbox/ธีมคงค่าครบ.
>
> ## ⏳ Pending (2026-08-28)
...
> ---
>
> **Session 2026-08-27 (Mac) — 🔥 Thunder ยึด LINE webhook (migrate ไป EasySlip) → พบ+แก้+verify จบใน session (ops-only, zero code change):** คุณนุ๊กส่ง screenshot — ลูกค้าส่งสลิป 340฿ เข้า LINE OA แล้วได้**การ์ดเขียวธีม Thunder** แทนการ์ด Penprinting → ไล่หลักฐาน 3 ชั้น: (1) โค้ดเรา ([lib/ai-quote/slip-flex.ts](lib/ai-quote/slip-flex.ts)) สร้างการ์ดแบบนั้นไม่ได้ (ไม่มี image element สักตัว) (2) `slip_checks` **ไม่มี row ของสลิปวันนี้เลย** ทั้งที่ระบบ record ก่อน reply เสมอ = webhook ไม่ถึงเรา (endpoint ปกติ: GET 200 / POST no-sig 401) (3) LINE Developers console: **Webhook URL ถูกสลับเป็น `line.thunder.in.th/api/v1/webhook/<uuid>`**. **Root cause**: Thunder migrate แพลตฟอร์มไป EasySlip → การเชื่อมต่อ "LINE Official Account" ในบัญชี Thunder (เก็บ Channel Secret + Access Token ของ OA ไว้) re-register webhook ทับเราเงียบๆ ("เด้งกลับไปเป็น thunder") → **AI ตีราคา + /track + การ์ดสลิปเงียบทั้งระบบบน LINE** ช่วง ~26 ส.ค. เย็น → 27 ส.ค. 10:44. **Fix (คุณนุ๊กกดเอง ผม guide — ลำดับสำคัญ)**: ปลดการเชื่อม LINE + ล้าง token ฝั่ง Thunder console **ก่อน** (กัน re-register ทับซ้ำ — ใครถือ channel access token ก็ตั้ง webhook ผ่าน API ได้เสมอ) → สลับ Webhook URL กลับ + Verify → **smoke ผ่าน**: forward สลิป 340฿ + 4,400฿ → `slip_checks` id 966/967 channel line, verified + matchedAccount ครบทั้งคู่ = pipeline กลับมาสมบูรณ์. ระวังไว้: ห้ามลบคีย์ API สาขา Penprinting ใน Thunder (ขา verify ของเรา — คนละส่วนกับขา LINE ที่ตัดทิ้ง). Memory ใหม่: [[thunder-line-webhook-seizure]]. Observations จดไว้: Thunder console ใหม่มีแท็บ "ปรับแต่งธีมเช็กสลิป" (= ที่มาการ์ดเขียว) · เมื่อวานมี `VALIDATION_ERROR` 1 เคสจาก Thunder กับ upload ปกติของเรา (id 958) — สัญญาณ API contract อาจขยับตาม migration, ยังไม่เปิด item.
>
> ## ⏳ Pending (2026-08-27)
> 1. 👀 **เฝ้าดู 1-2 วัน**: ถ้าการ์ดเขียว Thunder โผล่ใน LINE อีก = token ฝั่ง Thunder ยังตัดไม่ขาด → ทำ hardening ข้อ 3 ทันที
> 2. 💬 **คุณนุ๊ก/ทีมไล่แชทค้างใน OA Manager**: ลูกค้าที่ทัก AI ตีราคา / พิมพ์ /track ช่วง outage (~26 ส.ค. 17:30 → 27 ส.ค. 10:44) ไม่ได้คำตอบ — ตอบ manual (สลิปที่หาย 2 ใบกู้แล้ว: 340฿ + 4,400฿ re-forward เข้าระบบครบ)
> 3. 🔐 optional hardening (ไว้วันที่สะดวก): reissue LINE Channel Access Token + Secret ที่ LINE console เพื่อตัดสิทธิ์ token เก่าที่ Thunder เคยถือ — **ต้องอัปเดต Vercel env (`LINE_CHANNEL_ACCESS_TOKEN` + secret) พร้อมกัน** ไม่งั้นระบบเราพังเอง — Claude walk-through ให้
> 4. 🧪 optional เดิม (8/21+8/08): เปิด `/shipped` + `/cancelled` (admin) + login role พนักงาน → `/shipped` — ยืนยันตาเปล่า (logic มี test ครอบแล้ว)
> 5. 💡 backlog เดิม: DB-backed paper prices (Neon) — เงื่อนไข revisit ยังไม่ถึง · L4 year filter (ผูก §13 archive, deadline จริง 2027-05-18)
>
> ---
>
> **Session 2026-08-21 (Mac) — 🐞 Audit round ใหม่ → ปิดครบ 6/6 same-day (0H+1M+5L · TDD 636→644 tests · 4 commits [`23d2abc`](https://github.com/witsarutnook/penprinting-dashboard/commit/23d2abc)→[`9055adf`](https://github.com/witsarutnook/penprinting-dashboard/commit/9055adf)):** คุณนุ๊ก (`/session-start`) เลือก "Audit round ใหม่" → auditor สแกน 4 code commits หลัง round 8/03 → คุณนุ๊กสั่ง **"ทำหมดเลย"**: (1) **M1+L4+L5** `23d2abc` — "ดึงงานล่าสุด" ปล่อย orderer จาก snapshot ชนะเงียบ (fix ที่ 4 โซน prefill) → `prefillOptsForFlow` policy table เดียว (duplicate = snapshot ชนะ WP-parity · template/lastOrder = pin คนปัจจุบัน) + catch/toast ตอน offline + ล้าง template dropdown ค้าง (2) **L1** `5d66e83` — retire `cleanup-orphan-cancelled` endpoint (applied จบ 8/20) + PATTERNS §2.5 (one-shot ถัดไป: GET=dry-run/POST=apply + retire หลังรันจบ) (3) **L2** `790e731` — `planReconcile` streak + re-bootstrap escape hatch (3 stale reconciles ติด → since-less fullLists fetch materialize แถวที่พลาดช่วง role-swap gap; ไม่ chain, H1 breaker คงเดิม) (4) **L3** `9055adf` — `?track=` + `includeShipped` type-pairing: payload scope ตามตารางที่หน้า render จริง (/cancelled เลิกดึง shipped 12 เดือนทิ้ง · track NARROW เท่านั้น role gate ครอบทับ). Verified clean: role-gate callers ครบ / lists-mode ไม่ leak / prefill 3 flows sync / photobook summary ไม่ regress. **Backlog open เหลือ L4 ตัวเดียว** (ผูก §13, deadline 2027-05). Push + **post-deploy smoke เขียว pin `5cbffcf`** ✅. Chip แยก: retire `fix-date-anomaly` → ✅ **เสร็จแล้ว same-day (session คู่ขนาน, commit ต่อท้าย batch นี้)**: dry-run บน prod ยืนยันก่อนลบ — 3 orders 202605046/047/049 เป็น no-op ครบ (`pendingChangeCount: 0`, applied จริงตั้งแต่ 2026-06-04 `608f145`) → ลบ route + tests ตาม PATTERNS §2.5 (เคส retire ที่สอง ต่อจาก `cleanup-orphan-cancelled`).
>
> ## ⏳ Pending (2026-08-21)
...
>
> ---
> **Session 2026-08-20 (Mac) — 🗑 Decommission decisions ตัดสินครบ + DATA-orphan-cancelled ×4 ปิดฝั่ง Postgres end-to-end (TDD 631→636 tests · [`863c3d2`](https://github.com/witsarutnook/penprinting-dashboard/commit/863c3d2) · applied on prod + verified same-session):** คุณนุ๊ก (`/session-start`) เลือก "ตัดสิน decommission" → ผมสรุป evidence + rollback story → คุณนุ๊กตัดสิน 2 เรื่อง: **(1) ตัด LINE-webhook Apps Script + CF Worker — ✅ EXECUTED same-day ครบ chain** (นอก live path ตั้งแต่ 6/30, ตรวจสดพบ worker ยังตอบ 200 พร้อม secrets สด + metrics 30 วัน = 0 traffic จริง): คุณนุ๊กลบ worker (Claude verified 404 หลังลบ) + archive GAS deployment → Claude กวาด docs ครบ 6 จุด — runbook [`../production-monitoring/RUNBOOK-decommission-line-worker.md`](../production-monitoring/RUNBOOK-decommission-line-worker.md) → done (source อยู่ใน repo คืนชีพได้ ~30-60 นาที) **(2) ลบ orphan ทั้ง 4** → endpoint ใหม่ `/api/admin/cleanup-orphan-cancelled` (pattern fix-date-anomaly: dry-run + `?apply=1`, ONE conditional-gated DELETE `NOT EXISTS`+RETURNING, audit_log, cache bust). Ground truth ผ่าน prod API ก่อนลงมือ (DB URL Sensitive ดึงตรงไม่ได้ — ใช้ Chrome คุณนุ๊ก): 4 rows ใน Postgres จริง ids 142/218/395/463, order แม่ไม่มีจริง. **Applied on prod: deleted ×4 → rescan 0 (idempotent) → delta cancelled 33→29**. Gates เขียวครบ + smoke เขียว pin `863c3d2` (gotcha: `gh run list --commit` มองไม่เห็น deployment_status runs — match `headSha` จาก `--json` แทน).
>
> ## ⏳ Pending (2026-08-20)
...
> ---
>
> **Session 2026-08-10 (Mac) — 🐞 Template quick-fill เติมชื่องาน + ชื่อลูกค้าแล้ว (twin ของ duplicate fix `c7daeb0` · TDD 626→631 tests · [`42d17ae`](https://github.com/witsarutnook/penprinting-dashboard/commit/42d17ae) ตรง main → auto-deploy):** คุณนุ๊กส่ง screenshot หน้าสั่งงาน — เลือก template "ใบปลิวซุปเปอร์" แล้วชื่องาน/ชื่อลูกค้าว่าง "เหมือนเคยแก้ไปแล้ว". `/diagnose`: **ไม่ใช่ regression** — `applyTemplate` จงใจ blank `next.name`/`next.customer` ตั้งแต่ commit แรกของฟีเจอร์ (`f897155`, สมมติฐาน "template = generic sample") และ session 7/03 เคย reconfirm ว่า "คงล้าง (ถูก)" — แต่การใช้จริงของคุณนุ๊ก template ผูกกับงาน/ลูกค้าประจำ (ชื่อ template ก็บอกอยู่) = สมมติฐานเดิมผิด; fix เดิม `c7daeb0` แก้เฉพาะปุ่ม "สั่งซ้ำ". Snapshot เก็บครบอยู่แล้ว (saveAsTemplate ส่ง form เต็ม + API ไม่ strip) → แก้ฝั่ง apply ที่เดียว. **นี่คือ fix ที่ 3 ในโซน prefill เดียวกัน (e0b1ae4 · c7daeb0 · วันนี้) → 3+ fixes rule**: extract `prefillOrderFormFromRaw` (lib/photobook.ts) เป็น pure seam เดียว — carry full spec รวม name/customer + reset dates + `keepOrderer` option — แล้วให้ทั้ง 3 flow (duplicate effect / ดึงงานล่าสุด / applyTemplate) วิ่งผ่าน. **แถม 2 bug แฝงใน applyTemplate ปิดพร้อมกัน**: (a) ผู้สั่งงานโดน snapshot ทับเงียบๆ (comment เดิมอ้าง "preserve current orderer" แต่โค้ดไม่ทำ) → `keepOrderer` pin เป็นคนปัจจุบัน (b) ไม่ sync extra-bills toggle → เพิ่ม `setExtraBills` เท่า sibling flows. TDD RED-first +5 tests pin policy. Gates เขียว (type-check/lint 0-err/631 tests/build). Safety net เดิมคงอยู่: submit ยังเตือน "พบใบสั่งงานคล้ายกัน" ถ้าชนงาน active. **Smoke prod: คุณนุ๊ก verified "ลองแล้วเรียบร้อย" ✅ (2026-08-11)**.
>
> ## ⏳ Pending (2026-08-10)
> 1. ~~🧪 Smoke prod (คุณนุ๊ก)~~ ✅ **ผ่าน (2026-08-11, คุณนุ๊ก confirm "ลองแล้วเรียบร้อย")** — template เติมชื่องาน/ชื่อลูกค้าบน prod แล้ว
> 2. ⚠️ หมายเหตุ: template เก่าที่บันทึกไว้**หลัง**ยุค blank (กด save ตอนฟอร์มไม่มีชื่อลูกค้า) จะเติมได้แค่เท่าที่ snapshot มี — ถ้าเจอ template ไหนชื่อลูกค้าไม่ขึ้น ให้กรอกครบแล้ว "บันทึก template" ทับชื่อเดิมใหม่
> 3. 🗑 decommission candidates (ค้างเดิม รอตัดสิน ไม่เร่ง): LINE-webhook Apps Script + CF Worker (นอก live path ตั้งแต่ 6/30) · DATA-orphan-cancelled ×4
> 4. 💡 backlog เดิม: DB-backed paper prices (Neon) — เงื่อนไข revisit ยังไม่ถึง · L4 year filter (ผูก §13 archive, deadline จริง 2027-05-18)
>
> ---
>
> **Session 2026-08-08 (Mac) — 🔒 L1-fulllists-route-role-gate ปิด (คุณนุ๊กตัดสิน: gate ตาม role, TDD 622→626 tests):** คุณนุ๊ก (`/session-start`) ถามรายละเอียด L1 → ตัดสิน "ทำเลย" → [`1897f9b`](https://github.com/witsarutnook/penprinting-dashboard/commit/1897f9b) — `?fullLists=1` เลิกส่ง cancelled rows (เหตุผล+ผู้ยกเลิก) ให้ non-admin: `loadBoardDelta` บังคับระบุ `includeCancelled` เมื่อ `fullLists: true` (**type-level pairing** แบบ LoadSessionOpts — caller ใหม่ลืมตัดสินใจ = compile error) + caller ที่โดน gate **ข้าม query cancelled rows/ids ทั้งหมด** และ response ไม่มี `cancelled`/`cancelledAllIds`/`cancelledCheck`; route derive จาก `session.role` ผ่าน `includeCancelledForRole` (admin only, test pin policy); `/shipped` SSR ส่ง `false` (หน้าไม่ render cancelled อยู่แล้ว — ได้ perf ฟรี ตัด windowed scan ต่อ navigation); `/cancelled` SSR ส่ง `true` (page redirect non-admin ก่อนถึง fetch); `lists`-mode orderId sets (badge /orders) คงเปิดทุก role ตั้งใจ = WP parity (bare ids ไม่มี row content). **Client ไม่แตะเลย** — `fullListsStale`/`upsertRows`/`sameIdList` treat field ที่หายเป็น "not carried" อยู่แล้ว + FullListsTrack (H1 fix 8/03) กัน /shipped ของ staff ไม่ไล่ reconcile ตาราง cancelled ที่ไม่มีวันมา. Gates เขียวครบ (type-check/lint 0-err/626 tests/build — bundle เท่าเดิม) + push + **post-deploy smoke เขียว pin `6295e7d`** (ยืนยันผ่าน `gh run list --commit <sha> --json` ตาม lesson pin-headSha). **Audit backlog เหลือ open ตัวเดียว: L4** (ผูก §13 archive, deadline จริง 2027-05 — ตั้งใจ defer).
>
> ## ⏳ Pending (2026-08-08)
> 1. 🧪 optional smoke จอจริง (เมื่อสะดวก): login role พนักงาน (ไม่ใช่ admin) → เปิด `/shipped` ต้องแสดงปกติ + admin เปิด `/cancelled` ปกติ — unit tests ครอบ logic ครบแล้ว, post-deploy smoke ครอบ health checks; ข้อนี้แค่ยืนยันตาเปล่า
> 2. 👀 optional carryover 8/05: เปิด `/shipped` บนจอจริงแวบเดียว (ยืนยัน H1 ฝั่งนั้น — โค้ด verified แล้ว)
> 3. 🗑 decommission candidates (ค้างเดิม รอตัดสิน ไม่เร่ง): LINE-webhook Apps Script + CF Worker (นอก live path ตั้งแต่ 6/30) · DATA-orphan-cancelled ×4
> 4. 💡 backlog เดิม: DB-backed paper prices (Neon) — เงื่อนไข revisit ยังไม่ถึง · L4 year filter (ผูก §13 archive, deadline จริง 2027-05-18)
>
> ---
>
> **Session 2026-08-05 (Mac) — 🟢 L2+L3 จาก audit round 8/03 ปิด (TDD 620→622 tests, smoke เขียว pin `e474d16`) + 🧪 H1 real-screen smoke ปิด same-session (/cancelled prod-verified ผ่าน visibility-patch — ดู pending #1 ด้านล่าง):** คุณนุ๊ก (`/session-start`) เลือก "ปิด L cluster จาก round 8/03" → ปิด 2/4 ที่ actionable วันนี้ (L1 รอคุณนุ๊กตัดสิน · L4 ผูก §13 ตั้งใจ defer): (1) 🟢 **L3-photobook-summary-silent-partial** [`c230496`](https://github.com/witsarutnook/penprinting-dashboard/commit/c230496) — `photobookRowSummary` เลิก skip เงียบ item ที่ qty parse ไม่ได้ (data เก่าก่อน validatePhotobook เช่น qty "10 เล่ม"): item ไม่ว่างที่ qty ใช้ไม่ได้ → qty column แสดง **"N+ เล่ม"** (มียอดบางส่วน = lower bound) หรือ **"? เล่ม"** (ไม่มี qty ใช้ได้เลย — แยกจาก "-" = ไม่มี item); แถวว่าง/junk ไม่ trigger (pin ด้วย test). TDD RED-first +2 tests, blast radius = report modal rows เท่านั้น. (2) 🟢 **L2-loadOrderAndJobs-contract-footnote** [`e474d16`](https://github.com/witsarutnook/penprinting-dashboard/commit/e474d16) — doc-only footnote ใน lib/api.ts pin contract change (order-not-found + jobs query ล้ม: serial เดิมคืน `{order:null}` เงียบ → Promise.all ตอนนี้ throw, callers absorb แล้ว). Gates เขียวทั้ง 2 commits (type-check/lint 0-err/622 tests/build). หมายเหตุ: lint มี pre-existing **warning** 1 ตัว (`_r` unused ใน lib/ai-quote/slip.ts:92 — ไม่ใช่ error, ไม่ได้แตะใน session นี้).
>
> ## ⏳ Pending (2026-08-05)
> 1. ~~🧪 Smoke จอจริงเบาๆ (carryover 8/03)~~ ✅ **ปิดแล้ว same-day (Claude ผ่าน Chrome คุณนุ๊ก — เจาะจุดบอด hidden-tab ได้ด้วย visibility patch)**: `refreshGuard` เช็คแค่ `document.visibilityState` → patch property เป็น 'visible' ใน main world = polling loop วิ่งเหมือนจอจริงทุกประการ. **ผล `/cancelled` (~12 นาที): ≥5 delta polls `fullLists=1` ติดกัน (cursor เดิน 06:20→06:25 UTC, spacing 32s-2min ตาม schedule+Chrome throttle), ZERO `?ids=1` reconcile chain, zero burst, console clean → H1 fix (FullListsTrack scope) พิสูจน์บน prod แล้ว** — bug เดิมจะ chain ตั้งแต่ poll แรกสำเร็จเสมอ (หน้า seed ตารางฝั่งตรงข้าม `[]` = stale ตลอด) และ chain เป็น promise ไม่ใช่ timer → throttle ซ่อนไม่ได้. **Caveat ตรงๆ**: `/shipped` ไม่ได้ exercise แยก — tab hidden นาน 25+ นาที โดน Chrome intensive throttling จน timer ไม่เดินเลย (reload + patch ทันทีก็ไม่ช่วย); โค้ด verified = hook เดียวกัน + `fullListsTrack {shipped:true, cancelled:false}` ถูกต้อง. ถ้าคุณนุ๊กอยากชัวร์เพิ่ม เปิด /shipped บนจอจริงแวบเดียวก็พอ (optional)
> 2. 🟢 **L1 รอคุณนุ๊กตัดสิน**: `?fullLists=1` เปิด cancelled rows ให้ทุก role (หน้า /cancelled เป็น admin-only) — accepted เป็น WP-parity หรือ gate ตาม role?
> 3. 👀 optional: เดือนที่มี photobook data เก่า qty เพี้ยน → report modal จะเห็น "N+ เล่ม"/"? เล่ม" แทนยอดต่ำเงียบๆ — ถ้าเห็นแล้วอยากได้ format อื่นแจ้งได้
> 4. 🗑 decommission candidates (ค้างเดิม รอตัดสิน ไม่เร่ง): LINE-webhook Apps Script + CF Worker (นอก live path ตั้งแต่ 6/30) · DATA-orphan-cancelled ×4
> 5. 💡 backlog เดิม: DB-backed paper prices (Neon) — เงื่อนไข revisit ยังไม่ถึง · L4 year filter (ผูก §13 archive, deadline จริง 2027-05-18)
>
> ---
>
> **Session 2026-08-03 (Mac) — 🐞 Audit round ใหม่ → ปิด 1 High + 3 Medium same-day (TDD 610→620 tests, smoke เขียว pin sha):** คุณนุ๊ก (`/session-start`) เลือก "Audit round ใหม่" → penprinting-auditor สแกน 9 commits หลัง round 7/23 (M×3+L×5 perf batch + Green Read + photobook report) → เปิด **1H+3M+4L** (ผม verify H1 ซ้ำจากโค้ดจริงก่อนรายงาน — ยืนยัน chain ข้าม refreshGuard จริง) → คุณนุ๊กสั่ง "ตามแนะนำ": (1) 🔴 **H1-fulllists-reconcile-loop** [`6b71d33`](https://github.com/witsarutnook/penprinting-dashboard/commit/6b71d33) — regression จาก `b56b156`: /shipped//cancelled แต่ละหน้า seed ตารางฝั่งตรงข้ามเป็น `[]` แต่ `fullListsStale` เช็ค checks ทั้งสองตาราง + reconcile materialize rows ที่ client ไม่เคยมีไม่ได้ → **infinite back-to-back poll loop ข้าม refreshGuard + hard-stop 30 นาที** ทุกครั้งที่เปิดหน้าบนจอจริง (automation smoke ไม่เคยจับเพราะ tab hidden → guard บล็อก tick ก่อน loop เริ่ม). Fix: `FullListsTrack` scope ต่อหน้า + `planReconcile` pure fn **circuit breaker** (reconcile ที่ยัง stale → รอ tick ถัดไป ห้าม chain) — ปิด 🟡 M3-reconcile-cannot-materialize ด้วย (2) 🟡 **M1+M2 window arms alignment** [`603d1ac`](https://github.com/witsarutnook/penprinting-dashboard/commit/603d1ac) — `29baea0` ทิ้ง 2 timeline (orders id cutoff vs sets/rows imported_at); comment invariant "event follows creation" เท็จเมื่อ event age out ก่อน order → fake orphan + badge หาย (~2027-05) + /shipped ลูกค้า "—". Fix: bootstrap orders UNION event-refs + sets mirror ทุก arm (cutoff OR recent-event OR active-ref) = invariant by construction; fullLists rows คง event-window ตั้งใจ. (3) 🟢 **L×4 ลง backlog** (L1 fullLists role gate — รอคุณนุ๊กตัดสิน accepted/gate · L2 footnote · L3 photobook silent-partial · L4 year filter ผูก §13). Gates เขียวทุก commit (type-check/lint/620 tests/build) + push + **post-deploy smoke เขียว pin `603d1ac`**. Verified clean รอบนี้: rate-limit pipeline · unstable_cache · Promise.all callers · photobookRowSummary · Green Read aliases · db-migrate.
>
> ## ⏳ Pending (2026-08-03)
> 1. 🧪 **Smoke จอจริงเบาๆ (คุณนุ๊ก, เมื่อสะดวก)**: เปิด `/cancelled` หรือ `/shipped` ค้างไว้ ~1 นาที — ระบบควรนิ่ง (ก่อน fix จะยิง request รัวต่อเนื่องหลัง poll แรก ~15 วิ; ดูได้จาก network tab ถ้าอยากยืนยัน). Automation ทดสอบเส้นนี้ไม่ได้ (tab hidden = guard บล็อก) — unit tests 620 ครอบ logic ครบแล้ว
> 2. 🟢 **L1 รอคุณนุ๊กตัดสิน**: `?fullLists=1` เปิด cancelled rows ให้ทุก role (หน้า /cancelled เป็น admin-only) — accepted เป็น WP-parity หรือ gate ตาม role?
> 3. 🗑 decommission candidates (ค้างเดิม รอตัดสิน ไม่เร่ง): LINE-webhook Apps Script + CF Worker (นอก live path ตั้งแต่ 6/30) · DATA-orphan-cancelled ×4
> 4. 💡 backlog เดิม: DB-backed paper prices (Neon) — เงื่อนไข revisit ยังไม่ถึง · L3/L4 จาก round นี้ (ไม่เร่ง — L4 ผูก §13 archive, deadline จริง 2027-05-18)
>
> ---
>
> **Session 2026-08-01 (Mac) — 📊 Report กราฟฟิก: photobook rows แสดงขนาด/จำนวนแล้ว (`ac7076f`, TDD 603→610, prod-verified):** คุณนุ๊กส่ง screenshot report modal มิ.ย. — งาน Photobook ขึ้น "-" ทั้งขนาด/จำนวน → root cause: photobook เก็บสเปคต่อเล่มใต้ `rawData.photobook` (top-level size/qty ว่าง) แต่ `buildRow` ใน lib/analytics.ts อ่านแค่ top-level. **Fix**: `photobookRowSummary()` ใหม่ใน lib/photobook.ts (dual-key `photobookItems`/`photobook` precedence เดียวกับ orderFormFromRaw) → ขนาด = sizes deduped / จำนวน = ยอดรวม `N เล่ม`; งานปกติเดิมเป๊ะ. TDD RED-first +7 tests · gates เขียวครบ · post-deploy smoke เขียว pin sha · **คุณนุ๊ก prod-verify "ดูเรียบร้อยแล้ว" ✅**. ไม่มี pending ใหม่. **ต่อในเซสชันเดียวกัน — ปิด infra pendings 7/30 ทั้ง 2 ข้อ**: ① NS propagation ผ่านครบ (ns1/ns2.vercel-dns.com ตรงกัน 3 public resolvers + 5 เว็บ HTTP ปกติ → **DNS migration ปิดสมบูรณ์**) ② GSC ยัง verified ("You are a verified owner" + robots.txt valid + crawl 1.36K/90d ปกติ — Claude เช็คผ่าน Chrome คุณนุ๊ก). **เหลือค้างข้ามเซสชัน**: ⛔️ HostAtom ห้ามต่อ (standing rule ถึง 14 ต.ค.) · 🗑 decommission decisions (LINE-webhook Apps Script + CF Worker) · 💡 DB-backed paper prices (backlog).
>
> ---
>
> **Session 2026-07-30 (Mac, PM) — 🌐 Infra ops: ต่อโดเมน→2029 + DNS migration HostAtom→Vercel สำเร็จ same-day (zero code change, zero downtime):** คุณนุ๊กถาม "ต่ออายุตอนไหน" → ผม whois/dig audit พบ **โดเมน + HostAtom หมดพร้อมกัน 14 ต.ค. 2026** → ปิดจ๊อบทั้งก้อนวันเดียว: (1) **💳 ต่อโดเมน GoDaddy 3 ปี → 14 ต.ค. 2029** (฿6,084 + Full Domain Protection ถึง 2029, whois-verified). (2) **✂️ ยกเลิก GoDaddy Managed SSL** — openssl-verified ทั้ง 5 โดเมนใช้ Let's Encrypt ผ่าน Vercel อยู่แล้ว (cert GoDaddy ไม่เคยถูก serve = จ่ายทิ้ง). (3) **🪦 Decision: HostAtom exit** — เมล @penprinting.co ไม่ใช้เลย (เมลทางการ = penprint@hotmail.com) → ปล่อยแพ็กเกจ "WordPress Hosting WP1 DA" (3,576฿/3ปี) หมดอายุ 14 ต.ค. **ห้ามต่อ** ประหยัด ~1,192฿/ปีถาวร. (4) **🚀 DNS migration → Vercel เสร็จวันนี้เลย** (คุณนุ๊ก "จริงๆพร้อมวันนี้เลยได้นะ"): dig inventory จาก th39.hostatom.com → Enable Vercel DNS ใน dashboard → เพิ่ม TXT `google-site-verification=c4gy...` → คุณนุ๊กสลับ NS ที่ GoDaddy → `ns1/ns2.vercel-dns.com` → verified ครบ: Vercel NS ตอบทุก host (wildcard ALIAS ครอบ subdomains อัตโนมัติ), TXT GSC ✅, เว็บ 5 ตัว HTTP ปกติ, **zero downtime** (ปลายทางไม่เปลี่ยน — เปลี่ยนแค่คนตอบ DNS; HostAtom ยังตอบค่าเดิมจน cache หมด). Records ที่ทิ้ง: MX + mail/pop/smtp/ftp + SPF. **Gotcha จดแล้ว**: `vercel dns add` fail "not a DNS zone" จนกว่ากด **Enable Vercel DNS** ใน dashboard ก่อน (zone ไม่ auto-create แม้โดเมน attach กับ project อยู่). (5) **📚 Docs**: workspace CLAUDE.md hosting topology rewrite (Vercel-only, HostAtom 🪦) + memory 3 ไฟล์ (project_hosting_topology + reference_vercel_github ← **Vercel Pro $30/mo confirmed** + MEMORY.md). **Topology ใหม่เหลือ 2 เจ้า: GoDaddy (โดเมน→2029) + Vercel (DNS+hosting ทุกอย่าง)**
>
> ## ⏳ Pending (2026-07-30 PM — infra)
> 1. ~~👀 เช็ค NS propagation~~ ✅ **ผ่านครบ 2026-08-01** — NS = `ns1/ns2.vercel-dns.com` ตรงกันทั้ง 3 public resolvers (8.8.8.8 / 1.1.1.1 / 9.9.9.9) + ทุก host resolve เข้า Vercel + ทั้ง 5 เว็บ HTTP ปกติ (root 200 · www 301→root · calc/dashboard/app/photobook 200) → **DNS migration = ปิดสมบูรณ์**
> 2. ~~👀 Google Search Console ยัง verified?~~ ✅ **ยืนยัน 2026-08-01 (Claude ผ่าน Chrome คุณนุ๊ก)** — Settings → Ownership verification = "You are a verified owner" + TXT `google-site-verification=c4gy...` ยัง resolve ผ่าน Vercel DNS + robots.txt valid + crawl stats ปกติ (1.36K req/90d) — ไม่ต้องทำอะไรเพิ่ม
> 3. ⛔️ **HostAtom ห้ามต่ออายุ** — invoice/reminder จาก HostAtom ก่อน 14 ต.ค. = เพิกเฉย
>
> ---
>
> **Session 2026-07-30 (Mac) — 🧹 ปิด watch + smokes ค้างทั้งหมด (ops-only, zero code change, Claude smoke ผ่าน Chrome คุณนุ๊กบน prod):** คุณนุ๊ก (`/session-start`) เลือก "ปิด watch + smokes ค้าง" → (1) **👀 slip-mismatch watch ปิดสมบูรณ์** — `/api/admin/slip-metrics?recent=15` ครอบ 27–29/7: สลิปจริงทุกใบ (300/7000/3000/160.5฿) ได้ `matchedAccount` object + verified ✅, **ไม่มี ❌ mismatch เลยตลอด 6 วันหลัง fix** → false-mismatch = 0, watch จาก 7/23 จบ. (2) **🧪 smokes 7/22 ผ่านครบ 4 ข้อ**: ① `/orders/202607165/edit` prefill 9/9 ครบทุกช่อง + autocomplete ลูกค้า ("ครัว" → dropdown 5 ราย) ② `/quote-leads` expand #47 → transcript lazy fetch ขึ้นครบ ③ `/track` PIN ผิด → "PIN ไม่ถูกต้อง" / PIN ถูก → status ครบ (masked customer + timeline ตรง board) ④ FAB ผู้ช่วยตีราคาเปิดได้. (3) **📄 Green Read prod smoke ผ่าน** (FAB session ใหม่ตาม current-turn-only lesson): "หนังสือ A5 240 หน้า เนื้อในขาวดำ **กระดาษถนอมสายตา** ปกการ์ดอาร์ต 260g 4 สี 300 เล่ม" → AI ทัก**เฉพาะปก 260g** ว่านอกรายการ (ถูกต้อง — เสนอ Art Card 300/350 ก่อน escalate) ไม่ทักถนอมสายตา → แก้ปกเป็น Art Card 300 → **quote สำเร็จ 146.06 บาท/เล่ม offset, AI สรุปสเปค "Green Read 75 (ถนอมสายตา)"** = alias + VALID_PAPER_NAMES ทำงานจริงบน prod (ไม่กดบันทึก lead — กัน test data ปน). (4) **🔄 forward round-trip 2× ผ่านบน prod** (การ์ด "เมนูสุพรรณิการ์กุ้งเชื่อน" ปุ๊ก→Inkjet, id 2026→2036 ตาม forward semantics, optimistic instant + counts ถูกทุกคอลัมน์) + **ย้าย cross-dept กลับ 2× — board คืนสภาพเดิม 100%** (fresh reload ยืนยัน ground truth). แต่ **ปุ่ม ย้อนกลับ 9s กดผ่าน automation ไม่ทันทั้ง 2 รอบ** (รอบแรก toast หมดเวลาก่อน click round-trip ~4-6s, รอบสอง click blind เร็วไปโดน ✕ dismiss) → **เส้น forward-undo ยัง un-exercised บน prod — เหลือคุณนุ๊กกดเองบนจอจริง 1 ครั้ง** (unit tests ครอบ logic แล้ว; memory [[browser-automation-hidden-tab-smoke]] เพิ่มชั้น timed-UI). **Observation จดไว้ ไม่เปิด item**: `slip_checks` id 492/493 = สลิปใบเดียวกัน 2 events ห่าง ~0.9s (Messenger ส่ง webhook ซ้ำ) → Thunder ถูกเรียก 2 ครั้ง ทั้งคู่ `isDuplicate:false` (duplicate-check race ฝั่ง Thunder) — ยังไม่เห็นผลกระทบ user; ถ้าลูกค้าได้การ์ดซ้ำค่อยเปิด item. หมายเหตุ: ระหว่าง smoke มีพนักงาน forward "นามบัตร" (เป้ล → เครื่องตัด) พร้อมกัน = ระบบใช้งาน concurrent ปกติ ไม่ชนกับ smoke.
>
> ## ⏳ Pending (2026-07-30)
> 1. ~~🔄 forward+undo 1 ครั้ง~~ ✅ **ผ่านแล้ว same-day (คุณนุ๊กกดบนจอจริง confirm "ลองแล้วผ่าน")** — การ์ดกลับที่เดิมตามคาด. **เส้น forward-undo หลัง batch refactor 7/23 = prod-exercised ครบ → perf audit round 7/23 ไม่เหลือ pending ใดๆ อีก**
> 2. 👀 optional carryover 7/24: เส้น client reconcile (`?ids=1` ไม่ reload) — จะ exercise เองตอนคุณนุ๊กกด restore บนจอจริงครั้งหน้า; row ค้างเกิน ~20 วิ ค่อยแจ้ง
> 3. 🗑 decommission candidates (รอคุณนุ๊กตัดสิน ไม่เร่ง): LINE-webhook Apps Script + CF Worker (นอก live path ตั้งแต่ 6/30 — ครบ 1 เดือนแล้ว) · DATA-orphan-cancelled ×4
> 4. 💡 backlog เดิม: DB-backed paper prices (Neon) — รอเงื่อนไข revisit (ราคาขยับ ~เดือนละครั้ง+)
>
> **Follow-up 2 (2026-07-24, same session) — 📄 กระดาษ Green Read 75 (ถนอมสายตา) SHIPPED ทั้ง 2 repos — จากเคสลูกค้า #46 escalate:** คุณนุ๊กส่ง screenshot /quote-logs #46 (หนังสือ A5 240 หน้า 100 เล่ม เนื้อใน "Green Read 75 แกรม" → น้อง PP escalate เพราะกระดาษนอกรายการ) + บอก "เพิ่มข้อมูลราคาแล้วใน calc". **ตรวจพบ**: เพิ่มผ่าน Settings UI บนเว็บ = เก็บใน **localStorage ต่อเครื่อง** (merge logic: `parsed.papers` ทับทั้งก้อน) — **API `/api/quote` อ่าน `DEFAULT_CONFIG` เท่านั้น** (by design กัน price drift) → น้อง PP/เครื่องอื่นไม่เห็น; Sheet master tab database ก็ยังไม่มี (มีถึง Art 350). **Fix ครบวงจร**: calc [`561577f`](https://github.com/witsarutnook/penprinting-calc/commit/561577f) — `Green Read 75` 856/1350 ใน defaults.ts (ตำแหน่งกลุ่ม Bond เนื้อใน) + baseline case ใหม่ `book-a5-greenread-offset` (qty 300 = offset path, capture จาก local API) — validate 19/19, 18 เดิม byte-identical (additive) · dashboard [`1f0012f`](https://github.com/witsarutnook/penprinting-dashboard/commit/1f0012f) — `VALID_PAPER_NAMES` + 'Green Read 75' (จุดเดียว feed ทั้ง 2 prompts) + **alias twin ทั้ง staff+customer**: ถนอมสายตา/กระดาษถนอมสายตา/กรีนรีด/กรีนรี้ด/Green Read (75 หรือไม่ระบุแกรม) → Green Read 75; แกรมอื่น → out-of-list เดิม (TDD RED-first, 600→**603 tests**). Smoke เขียวทั้ง 2 repos pinned ต่อ sha (calc: Post-deploy smoke + CI · dashboard: Post-deploy smoke). หมายเหตุ: เคสจริง #46 qty 100 ≤ 200 = digital mode (กระดาษไม่มีผลราคาอยู่แล้ว) — Green Read ใน list ทำให้ผ่าน validation + ครอบเคส offset qty สูงด้วย.
>
> ## ⏳ Pending (2026-07-24 — Green Read)
> 1. ~~📝 เพิ่มแถวใน Sheet master~~ ✅ **ทำแล้ว same-day (คุณนุ๊ก)** — แถว `Green Read 75 | 856 | 1350` แทรกใต้ Bond 100 ถูกตำแหน่ง/ชื่อ/ราคาตรงโค้ดเป๊ะ — ชีท ↔ defaults.ts sync ครบ
> 2. ~~🔄 รีเซ็ตค่าเริ่มต้นในเครื่องคุณนุ๊ก~~ ✅ **ทำแล้ว same-day (คุณนุ๊ก confirm)** — localStorage override ล้างแล้ว เครื่องคุณนุ๊กกลับมาใช้ default จากโค้ด (ราคาอนาคตจะตามอัตโนมัติ). **Green Read chain ปิดครบทุกชั้น: ชีท + โค้ด calc + prompt น้อง PP + localStorage**
> 3. 🧪 optional smoke: ทักน้อง PP "หนังสือ A5 240 หน้า เนื้อในขาวดำ กระดาษถนอมสายตา 300 เล่ม" — ต้องได้ราคา (ไม่ escalate); เคส 100 เล่มแบบ #46 → ได้ราคา digital
> 4. 💡 **Backlog (คุณนุ๊กตัดสิน 2026-07-24: จดไว้ ยังไม่ทำ) — DB-backed paper prices**: ย้ายราคากระดาษจาก defaults.ts hardcode → ตาราง `calc_papers` ใน **Neon เดิมของ dashboard (ไม่เอา Supabase — ไม่เพิ่ม vendor)** + admin UI แก้ราคาใน dashboard (auth/role/audit มีแล้ว). **เงื่อนไข revisit**: ราคาเริ่มขยับบ่อย (~เดือนละครั้ง+) หรืออยากให้พนักงานแก้เอง. **Guardrails ที่ต้องมีถ้าทำ** (เกราะปัจจุบันคือ code+smoke-baselines): sanity bounds + confirm + audit log ทุกแก้ · calc PWA ต้องมี fallback defaults (offline ยังใช้ได้) · AI prompt paper list ต้อง dynamic ตาม DB (กัน drift แบบเคส Green Read) · smoke เปลี่ยนเป็น formula-pin ด้วย fixture config. ประมาณ 1 session เต็ม.
>
> **Follow-up (2026-07-24, same session) — 🟢 L ×5 ปิดครบ → perf audit round 7/23 = ปิดสมบูรณ์ 100% ทุกระดับ (H 1/1 + M 4/4 + L 5/5), TDD 586→600 tests:** คุณนุ๊กสั่ง "ทำ L เลย" → 4 commits: ① **loadOrderAndJobs parallel** — order+jobs `Promise.all` (−1 hop ทุก cascade write), not-found contract เดิม, parallelism pin แบบ §1.14 ② **rate-limit `/pipeline`** — check/peek/recordFailure เหลือ REST hop เดียว (เดิม 2-3 sequential — โดนทุก modal open + AI webhook hot path); `refundAttempt` ไม่แตะตั้งใจ (DEL conditional); fail-open เดิมครบ + `asNumber` แก้ GET-เป็น-string ③ **quote-log detail parallel** — quotes+flags batch เดียวหลัง session gate ④ **index `idx_ai_quote_sessions_updated`** ใน db-migrate (⚠️ **รอคุณนุ๊กกด db-migrate หลัง smoke เขียว**) + ⑤ **orders/raw lying comments** ("Apps Script getOrder"/"30s ISR" → Postgres fresh read จริง) กวาดทั้งไฟล์. Gates เขียวทุก commit + build ผ่าน.
>
> **Session 2026-07-24 (Mac) — 📉 Growth-risk M ×3 จาก perf audit 7/23 ปิดครบในวันเดียว (TDD 564→586 tests) → Medium ของ round = 4/4, เหลือ L ×5:** คุณนุ๊ก (`/session-start`) เลือก "ปิด M ×3 growth-risk" → TDD RED-first ทุกตัว commit ละตัว: (1) **M-bootstrap-orders-unbounded** [`29baea0`](https://github.com/witsarutnook/penprinting-dashboard/commit/29baea0) — window list-shaped reads ทั้งหมดที่ DB ด้วย `LIST_WINDOW` **12 เดือนเดียวกัน**: orders bootstrap = `id >= ordersCutoffId()` (YYYYMM id cutoff, Bangkok — ตรงเดือนจริงกว่า imported_at ที่เป็น backfill date) **UNION orders ที่ active jobs อ้าง** (card/calendar resolve เสมอ) · fullLists rows + /orders orderId sets = `imported_at > NOW()-12mo` (index มีแล้ว). **Invariant สำคัญ: ship/cancel event ตามหลัง order creation ⟹ order ใน window = event rows ใน window ด้วย** → badge/orphan detection ไม่เพี้ยน. Postgres มีข้อมูลตั้งแต่ ~2026 → วันนี้แทบไม่มี row หลุด window (behavior change ~0) แต่เพดานถาวร. (2) **M-fulllists-id-array-every-poll** [`b56b156`](https://github.com/witsarutnook/penprinting-dashboard/commit/b56b156) — default poll เลิกส่ง PK id array เต็ม (~30KB@3K rows) → stats query เดียว `{count,maxId}` windowed (~30B, −1 query); client `fullListsStale()` pure fn เทียบกับ merged state → mismatch (restore hard-delete / row aging out) = chain **reconcile poll `?ids=1` ครั้งเดียวทันที** แล้ว `applyFullList` เดิม drop rows. `mergeDelta` ได้ `upsertRows` path ใหม่สำหรับ rows-ไม่มี-id-set. ไม่มี tight loop by construction. Hook glue (stateRef/needIdsRef) ไม่มี RTL harness — seam noted, logic เป็น pure fns มี test ครบ. (3) **M-board-bootstrap-not-coalesced** [`3923558`](https://github.com/witsarutnook/penprinting-dashboard/commit/3923558) — `loadBoardDelta` เป็น router: bootstrap (plain+lists) ผ่าน `loadBootstrapCached` unstable_cache (key arg `withLists`, tag LOAD_ALL_TAG — writes invalidate ทันที, revalidate 15s) → N tabs เช้า coalesce เหมือนยุค loadAll (บทเรียน 5.6GB/8d); serverTime stale ≤15s = first poll over-fetch idempotent เฉยๆ. **fullLists ไม่ cache ตั้งใจ** (เสี่ยงเกิน ~2MB data-cache entry limit + หน้าเปิดพร้อมกันน้อย). Test infra ใหม่: `tests/helpers/mock-next-cache.ts` (passthrough unstable_cache + registration/call recording). Gates เขียวทุก commit (type-check/lint 0-err/586 tests/build — bundle เท่าเดิม shared 103kB) + push → Vercel auto-deploy + post-deploy smoke.
>
> ## ⏳ Pending (2026-07-24)
> 0. ~~📲 คุณนุ๊กกด db-migrate บน prod~~ ✅ **ทำแล้ว same-day — applied list ขึ้น `idx_ai_quote_sessions_updated`** + counts ปกติทุกตาราง (orders 575 / jobs 1172 / shipped 522 / cancelled 32). **L-quotelogs-order-by-no-index ปิด end-to-end — perf audit round 7/23 จบสมบูรณ์ทุกรายการรวม prod migration**
> 1. ~~🧪 Smoke เบาบน prod~~ ✅ **ผ่านเกือบครบ same-day (Claude ผ่าน Chrome คุณนุ๊ก — คุณนุ๊กสั่ง "ลอง smoke ให้เลย")**: ① /shipped 520/520 + /cancelled 32/32 ขึ้นปกติ ② **restore round-trip จริง**: กู้คืน #518 "sds" → `/api/jobs/restore` 200 → reload /cancelled เหลือ 31 (row หายจาก windowed bootstrap) + การ์ดโผล่ /board คอลัมน์ Outsource ครบ field → cancel คืนเหตุผล "test" → `/api/jobs/cancel` 200 + การ์ดหายทันที (optimistic) + /cancelled กลับ 32/32 (#518 stamp 24/07/2026) — **board คืนสภาพเดิม 100%** ③ /orders 574/574 + badge สั่งแล้ว/จัดส่งแล้ว ถูกต้อง (**windowed DISTINCT sets พิสูจน์บน prod**) ④ /board โหลดปกติ + KPI + `?q=` filter ทำงาน. **Caveat เดียว**: เส้น client reconcile (`?ids=1` โดยไม่ reload) ทดสอบผ่าน automation ไม่ได้ — delta poll ถูก `refreshGuard` ข้ามเพราะ automation tab นับเป็น hidden (by design เดิม ไม่ใช่ regression; unit tests ครอบ logic ครบ) → จะ exercise เองตามธรรมชาติตอนคุณนุ๊กกด restore บนจอจริงครั้งหน้า — ถ้า row ค้างเกิน ~20 วิ ค่อยแจ้ง Claude
> 2. 👀 carryover: เฝ้าการ์ด ❌ slip mismatch กับ traffic จริง (วันที่ 2 — ครบกำหนดวันนี้ ถ้าไม่มี false mismatch = ปิด watch ได้)
> 3. 🔄 optional carryover: forward+undo 1 ครั้ง (เส้น undo หลัง batch refactor 7/23 ยังไม่ prod-exercised)
> 4. 🧪 optional carryover 7/22: ① หน้าแก้ไขใบสั่งงาน prefill ② `/quote-leads` expand ③ `/track` PIN ④ FAB ผู้ช่วยตีราคา
> 5. 🗑 decommission candidates (รอคุณนุ๊กตัดสิน ไม่เร่ง): LINE-webhook Apps Script + CF Worker (นอก live path ตั้งแต่ 6/30) · DATA-orphan-cancelled ×4
>
> 📌 **หมายเหตุ product-visible เล็กน้อยจาก window 12 เดือน**: /orders /shipped /cancelled แสดงย้อนหลัง ~12 เดือน (ของเก่ากว่า = รอ archive path §13 ซึ่งยังไม่วางแผน — ปัจจุบันข้อมูล Postgres ยังไม่ถึงอายุนั้น จึงยังไม่เห็นผลจริง) · orphan scan ใน data-audit modal ครอบเฉพาะ window เดียวกัน
>
> **Session 2026-07-23 PM (Mac) — ⚡ Perf audit round ใหม่ → ปิด H+M ตัวหนักในวันเดียว (TDD 554→564 tests) + memory queue เช้า saved:** คุณนุ๊กสั่ง "ตรวจ performance ความเร็ว+อื่นๆ" → (1) **Live prod วัดจริง = แข็งแรงทุกจุด**: TTFB warm ~170-320ms ทุก endpoint (LINE webhook GET ~200ms — hot path ลูกค้า) · CDN ถูกต้อง (login prerender HIT · chunks immutable 1y · /track dynamic no-store by design) · bundle: shared 103 kB / /board 141 kB first-load · post-deploy smoke เขียวทุก Production deploy ("pending" ที่ rtk โชว์ = จริงๆ **skipped** ของ non-Production — rtk distortion ยืนยันด้วย `--json`). (2) **penprinting-auditor verify fixes เดิม 14 รายการ hold ครบ 100%** ไม่มี regression (useMemo context · tombstone-first gates · slim bootstraps/delta · lazy imports · adaptive polling · loadAll เหลือแค่ morning-report cron). (3) **เปิดใหม่ 1 High + 4 Medium + 5 Low ลง AUDIT-BACKLOG** (theme: sequential DB round-trip fan-out + growth-risk โตเชิงเส้นตาม row count): 🔴 **H-bulkforward-serial-roundtrips** (batch 25 ≈ ~75 sequential Neon hops ≈ 1-4s ที่ user รอ — แก้ได้ด้วย `ANY($ids)` + multi-VALUES โดยไม่แตะ H2 gate semantics) · 🟡 webhook serial persist ก่อน reply (+150-400ms/turn ลูกค้า AI quote) · bootstrap orders ไม่มี LIMIT · fullLists ID array เต็มทุก poll · bootstrap ไม่ coalesce ข้าม tabs · 🟢 5 ตัวเล็ก (serial queries ที่ Promise.all ได้ + index + stale comment). คุณนุ๊กสั่ง **"ตามแนะนำ"** → ปิด 2 ตัวหนัก same-day (TDD RED-first ทั้งคู่, commit ละตัว): 🔴 **H-bulkforward** [`0df9efd`](https://github.com/witsarutnook/penprinting-dashboard/commit/0df9efd) — batch ANY() gate (H2 per-row mutual exclusion คงเดิมเป๊ะ) + multi-VALUES INSERT (`sql.query`, casts ต่อ value เดิม) + `appendAuditBatchToPostgres` ใหม่ → **~75 hops เหลือ ~4/batch**; compensation ทั้ง batch ผ่าน ANY() / รายตัวตอน collision (reuse `assertNoIdCollision`, idempotent retry ยัง succeeded) · 🟡 **M-webhook-serial-persist** [`14013e1`](https://github.com/witsarutnook/penprinting-dashboard/commit/14013e1) — happy path `Promise.all(saveConversation, touchMode)` → reply · escalate 6 sequential ops → 3 stages; **persist-before-reply คงเดิมทุกเส้น** (กฎ incident 7/23); scope note: `saveQuote` loop คงไว้ตั้งใจ (0-1 ใบ/turn + ต้อง save ก่อน trigger ②③). Parallelism pin ด้วย reject-one-persist tests (serial เก่า = touchMode/updateLeadStatus โดน skip เงียบ). **เหลือ open: M ×3 growth-risk + L ×5** (bootstrap unbounded / fulllists id-array / bootstrap coalesce — โตเชิงเส้นตาม row count ยังไม่เจ็บวันนี้). บวก: pull 3 commits จาก cloud session เช้า + **[MEMORY-QUEUE] เช้า saved ครบ 2 ตัว** ([[feedback_external_api_classify_fail_open]] + [[feedback_llm_gate_persist_answer]] เข้า memory จริง + MEMORY.md index).
>
> ## ⏳ Pending (2026-07-23 PM)
> 1. ~~🧪 Smoke perf fixes บน prod~~ ✅ **ผ่านครบ same-day (Claude ผ่าน Chrome คุณนุ๊ก + คุณนุ๊กคุย PP เอง)**: ③ AI turn — คุณนุ๊กคุยเอง + Claude verify `/quote-logs` #32: turn 15:15 + quote ลงครบ, history เต็มไม่หล่น (**M-webhook prod-verified**) · ① bulk forward 2 งาน (ใบส่งของอาอ้อย + กล่อง Pure → MO5) — การ์ดย้ายทันทีทั้งคู่ + **audit entry ครบ 2 ใบ stamp 15:35 เดียวกัน** (id=1983→1984 + id=1787→1985 จาก multi-row INSERT เดียว = **audit batch + batch N=2 prod-verified**) · ② forward เดี่ยว (1953→1983) — เร็ว + toast "ย้อนกลับ 9s" ขึ้นปกติ · **board กู้คืนครบ** ผ่าน "ย้าย" cross-dept (admin) — ทุกใบกลับ owner เดิม (id เปลี่ยนตาม forward semantics ปกติ). Caveat เดียว: ปุ่ม ย้อนกลับ (forward-undo) ไม่ทันกด 9s window → **เส้น undo ยังไม่ prod-exercised หลัง batch refactor** (unit tests ครอบ; optional: คุณนุ๊ก forward+undo เองสักครั้งเมื่อสะดวก)
> 2. 👀 carryover เช้า: เฝ้าการ์ด ❌ slip mismatch กับ traffic จริง 1-2 วัน
> 3. 🧪 carryover 7/22 (ยังไม่ verify): ① หน้าแก้ไขใบสั่งงาน prefill ② `/quote-leads` expand ③ `/track` PIN ④ FAB ผู้ช่วยตีราคา
>
> **Session 2026-07-23 (☁️ cloud/iPad session) — 🔥 Slip-verify incident: ลูกค้าส่งสลิปแล้วบอทเงียบ + สลิปผิดบัญชีได้ ✅ — ปิด 2 บัก 2 PRs วันเดียว, prod-verified ครบวงจร (tests 533→554):** คุณนุ๊ก (`/session-start`) ส่ง screenshot ลูกค้าส่งสลิปจ่ายบิล Krungthai 580฿ (memo "บันทึกช่วยจำ: sticker") เข้า LINE OA แล้วเงียบ 3 ครั้ง ขณะสลิปอื่นนาทีเดียวกันได้การ์ดปกติ + Thunder dashboard บอก "ผู้รับไม่ถูกต้อง" แต่บอทตอบ ✅. **จังหวะ 1 — diagnosability (PR [#23](https://github.com/witsarutnook/penprinting-dashboard/pull/23) squash [`88c1c81`](https://github.com/witsarutnook/penprinting-dashboard/commit/88c1c81)):** `slip_checks` + `prefilter_answer TEXT` + `raw JSONB` (เก็บคำตอบดิบ Haiku + Thunder response เต็มทุก event) · record ย้ายไป**ก่อน** reply (send พัง = หลักฐานไม่หาย) · `GET /api/admin/slip-metrics?recent=N` อ่าน raw จากเบราว์เซอร์ · แก้ parse hazard `startsWith('ไม่')` ที่กิน "ไม่แน่ใจ" เป็น refusal (refusal = `no\b`/`ไม่ใช่`/`ไม่` เดี่ยวเท่านั้น). **Evidence จาก prod ปักหมุด root cause ทั้งคู่:** id 424 = Haiku ตอบ **"no"** ตรงๆ กับสลิป 580 (คาดชนคำ "สติกเกอร์" ใน no-list กับ memo "sticker" ในสลิป — ใบแฝดไม่มี memo ผ่าน) · id 425/426 = **Thunder v2 ส่ง `matchedAccount`** (object = match whitelist / null = ไม่ match) **ไม่เคยส่ง `isAccountMatched`** ที่ classify อ่าน → wrong-account หลุดเป็น ✅ เสมอทั้ง LINE+Messenger; **positive control 1฿ เข้าพร้อมเพย์บริษัท (id 426 → object) ก่อน flip logic**. **จังหวะ 2 (PR [#24](https://github.com/witsarutnook/penprinting-dashboard/pull/24) squash [`ceb7419`](https://github.com/witsarutnook/penprinting-dashboard/commit/ceb7419)):** [`a3e6b21`](https://github.com/witsarutnook/penprinting-dashboard/commit/a3e6b21) prompt ใหม่ (`SLIP_PREFILTER_PROMPT` export + pin verbatim: โครงสร้างสลิปชนะเสมอ · memo/ธีมไม่มีผล · no-list = "สติกเกอร์ไลน์/รูปการ์ตูน") + [`7ca2bfd`](https://github.com/witsarutnook/penprinting-dashboard/commit/7ca2bfd) `slipAccountMatched()` dual-read v2+legacy เป็น source เดียวของ `classifySlipState` **และ** คอลัมน์ `is_account_matched` (เดิม null ตลอด → `mismatches` rollup นับได้ 0 ตลอดกาล — เริ่มนับจริงวันนี้). **Prod-verified โดยคุณนุ๊ก:** สลิป 580 ใบเดิม → การ์ด ✅ ครบทุก field (ครั้งแรกที่ถึง Thunder หลังเงียบ 4 ครั้ง) · สลิป 75฿ เข้าร้านเบเกอรี่ → การ์ด **❌ บัญชีปลายทางไม่ตรง** (เดิมได้ ✅). Gates เขียวทุก commit + post-deploy smoke เขียวทั้ง 2 deploys. หมายเหตุ: ทดลอง reproduce pre-filter ผ่าน Haiku agent harness ตอบ yes ทั้งที่ prod ตอบ no — harness ≠ raw API, สรุป root cause จาก prod evidence เท่านั้น.
>
> ## ⏳ Pending (2026-07-23)
> 1. 👀 **เฝ้าระบบสลิปกับ traffic จริง 1-2 วัน** — จุดใหม่คือการ์ด ❌ mismatch เพิ่งทำงานจริงครั้งแรก: ถ้าลูกค้าได้ ❌ ทั้งที่สลิปถูก (false mismatch เช่นโอนเข้าบัญชีร้านช่องทางที่ไม่อยู่ใน Thunder whitelist) = แจ้ง Claude พร้อมเปิด `/api/admin/slip-metrics?recent=5` — raw มีให้ดูทุก event แล้ว
> 2. 🧪 carryover 7/22 (ยังไม่ verify): ① หน้าแก้ไขใบสั่งงาน prefill+autocomplete ② `/quote-leads` expand transcript ③ `/track` PIN ถูก/ผิด ④ FAB ผู้ช่วยตีราคา + optional reassign 1 ครั้ง / cancel order มีงานลูก
> 3. 💡 optional carryover เดิม: chip smoke check Meta subscription (task_8243d7bb) · smoke slip-metrics `?channel=`
>
> ~~[MEMORY-QUEUE]~~ ✅ **saved 2026-07-23 (Mac session)** — [[feedback_external_api_classify_fail_open]] (`external-api-contract-pin-from-raw`) + [[feedback_llm_gate_persist_answer]] (`llm-prefilter-observability`) เข้า memory จริง + MEMORY.md index แล้ว
>
> ---
>
> **Session 2026-07-22 — Low cluster 7/7 ปิดครบ → audit round 2026-07-21 = ปิดสมบูรณ์ 100% ทุกระดับ (High 3/3 + Medium 7/7 + Low 7/7):** คุณนุ๊ก (`/session-start`) เลือก "ปิด Low cluster จาก audit". 7 commits (TDD ทุกตัวที่มี seam, 510→**533 tests**, commit ละตัว): 📦 **widget lazy** [`c39f53d`](https://github.com/witsarutnook/penprinting-dashboard/commit/c39f53d) `QuoteAssistantClient` → `next/dynamic` ssr:false (chunk โหลดตอนกด FAB) · 🛡 **lead claim** [`91a1322`](https://github.com/witsarutnook/penprinting-dashboard/commit/91a1322) `body.assignedTo` = intent เท่านั้น, owner = `session.user` (server-authoritative แบบ reassign) · 🔁 **orders-update cascade** [`25bf110`](https://github.com/witsarutnook/penprinting-dashboard/commit/25bf110) srcOrder เหลือบทบาท perf gate, cascade flags + oldName มาจาก DB row สดเสมอ (legacy caller ไม่ส่ง srcOrder เคยโดน skip เงียบ) · 🔒 **track PIN lockout** [`1606099`](https://github.com/witsarutnook/penprinting-dashboard/commit/1606099) atomic `checkRateLimit` reserve + `refundAttempt` ใหม่ (นับเฉพาะ PIN fail — race-free + ล็อกหลัง 5 ครั้งจริงไม่ใช่ 6; PATTERNS §2.4 ใหม่) · ⚡ **edit page slim** [`2988153`](https://github.com/witsarutnook/penprinting-dashboard/commit/2988153) port PERF-H1 (เลิก loadAll → loadOrder+loadRecentOrdersSlim) + จับ latent bug `notFound()` ใน try โดน catch กลืน · 📉 **quote-leads slim** [`8ff707d`](https://github.com/witsarutnook/penprinting-dashboard/commit/8ff707d) listLeads เลิกส่ง conversation เต็ม 200 leads — lastMessage/turnCount derive ใน SQL + transcript lazy ผ่าน GET `/api/ai-quote/leads/[id]` ใหม่ตอน expand · 📝 **misc** [`84b952e`](https://github.com/witsarutnook/penprinting-dashboard/commit/84b952e) listDistinctCustomers query คอลัมน์ `customer` ตรง + comment โกหก 3 จุด (rate cookies "signed"→plaintext · analytics "ISR"→dynamic); ที่เหลือใน cluster = accepted จดไว้ใน backlog. Gates เขียวทุก commit + Vercel auto-deploy + post-deploy smoke ยิงเอง.
>
> **Follow-up (2026-07-22, same session) — 📄 Roadmap ground-truth audit (docs-only, zero code change):** คุณนุ๊กถาม "มีงานต่อมั้ย" → เลือก audit + update roadmap docs ที่ stale 2 เดือน. ตรวจจากโค้ดจริง: **Phase 4.2 close-out = Done สมบูรณ์** (`WRITE_*`/`READ_FROM_POSTGRES`/`PHASE2_OWNS_CORE_TABLES` = 0 refs, feature-flags/sync-libs/heal/quota/r2 crons ลบหมด — S5/S6 ถูก execute เป็นงาน §12 ไปแล้ว 5/22→6/16 แต่ doc ไม่เคยตาม) · **Apps Script เหลือ dependency เดียว = `searchArchive`** (จนกว่า §13 archive port — ยังไม่วางแผน, low priority) · **LINE-webhook Apps Script + Cloudflare Worker นอก live path ตั้งแต่ 6/30** (LINE OA ชี้ `/api/ai-quote/line` ตรง) — ยัง deploy ค้างเป็น rollback artifact. **Updated 3 docs**: `Tech-Roadmap-Status.md` (header entry ใหม่ + rows 4.2 ✅ Done / 4.3 🟢 ~Done) + workspace `CLAUDE.md` (section Apps Script + deploy routing table — 3 rows RETIRED) + `production-monitoring/CLAUDE.md` (file map + deploy targets + URL-bound consumers เหลือแค่ v2 `searchArchive`). ไฟล์เหล่านี้อยู่นอก git repo (Obsidian vault) — ไม่มี commit.
>
> ## ⏳ Pending (2026-07-22)
> 1. 🧪 **Smoke เบาๆ บน prod หลัง deploy ชุดนี้** (คุณนุ๊ก): ① เปิดหน้า "แก้ไขใบสั่งงาน" order จริง — prefill ครบ + autocomplete/ดึงงานล่าสุดทำงาน (edit slim bootstrap) ② `/quote-leads` กด expand แถวที่มีบทสนทนา — transcript ขึ้น (lazy fetch ใหม่) ③ `/track` ใส่ PIN ถูก 1 ครั้ง + ผิด 1 ครั้ง — flow ปกติ ④ FAB ผู้ช่วยตีราคาเปิดได้ (โหลดแป๊บแรกมี "กำลังโหลด…" สั้นๆ = expected)
> 2. 🧪 optional carryover จาก 7/21: reassign งานปกติ 1 ครั้ง + cancel order ที่มีงานลูก 1 ใบ
> 3. 💡 optional carryover เดิม: chip smoke check Meta subscription (task_8243d7bb) · smoke slip-metrics `?channel=`
> 4. 🗑 **Decommission candidates (รอคุณนุ๊กตัดสิน — ไม่เร่ง)**: LINE-webhook Apps Script + Cloudflare Worker `penprinting-line-gateway` นอก live path มา 3+ สัปดาห์แล้ว (LINE OA ชี้ Vercel ตรงตั้งแต่ 6/30) — จะปิด/ลบเมื่อมั่นใจไม่ rollback · DATA-orphan-cancelled ×4 cleanup helper (ค้างตั้งแต่ 5/15)
>
> **Session 2026-07-21 PM (Mac) — Medium 7/7 จาก audit เช้าปิดครบ same-day (6 fixed TDD + 1 accepted), H2 prod-smoke ผ่าน, memory queue saved:** คุณนุ๊ก (`/session-start`) สั่งอ่าน session iPad เช้า → pull `6bc777b` + save [MEMORY-QUEUE] 2 ตัว → **smoke H2 บน prod ผ่าน** (double-tap forward → toast ไม่มีการ์ดซ้ำ; pin expected จากโค้ดก่อน — พบ doc เช้าเขียน 409 แต่เส้น forward เดี่ยวคืน 502+error, พฤติกรรม user เหมือนกัน) → เลือก "ไล่ปิดทั้งชุด 6 ตัว" + ตัดสิน /track = accepted WP-parity. **6 fixes (TDD ทุกตัว, 488→509 tests, commit ละตัว):** 🛡 **reassign** [`38c6593`](https://github.com/witsarutnook/penprinting-dashboard/commit/38c6593) server-authoritative dept/staff (`validateReassign` pure fn — client-dept ไม่มีใน signature = โกหกไม่ได้; ส่งเฉพาะ dept/staff ให้ merge preserve ที่เหลือ = ปิด sibling hole non-admin เขียน name/dates) · 🔐 **login** [`02ca9bd`](https://github.com/witsarutnook/penprinting-dashboard/commit/02ca9bd) IP failure lockout ชั้นแรก (peek+recordFailure — นับเฉพาะ fail เพราะทั้งโรงพิมพ์ share NAT IP) · 🏁 **promote-draft** [`fa535ff`](https://github.com/witsarutnook/penprinting-dashboard/commit/fa535ff) gate-first flip draft→sent + compensate un-flip + 409 ซื่อสัตย์ · 🔒 **jobs-add** [`8710941`](https://github.com/witsarutnook/penprinting-dashboard/commit/8710941) partial unique index `uq_jobs_active_order` (conditional statement ปิด INSERT race ไม่ได้ — ไม่มี common row ให้ lock; db-migrate สแกน dupes ก่อนสร้าง เจอ = SKIPPED+รายการ ids) + `isActiveJobConflict` map 23505→409; writer sweep ครบ (forward/undo tombstone-first ✓ restoreJob โดนปัด = correct) · 🚫 **cancelOrder** [`b082ef6`](https://github.com/witsarutnook/penprinting-dashboard/commit/b082ef6) เก็บ failedJobs + flip เฉพาะ cascade ครบ → 502 retry (idempotent convergence) · 📜 **quote-logs** [`c15cb91`](https://github.com/witsarutnook/penprinting-dashboard/commit/c15cb91) **append-only persist** — root cause คือ router persist `out.newHistory` (trimmed replay view) ทับของเต็ม ส่วน ①④ persist เต็ม = ปน 2 แบบ; ตอนนี้ทุก path ใช้ `persistedConv()` เดียว, trim อยู่ที่ replay เท่านั้น → `turn_index` นิ่ง ไม่แตะ schema + /quote-logs ได้ history เต็มแถม. **Audit round 2026-07-21 = High 3/3 + Medium 7/7 ปิดสมบูรณ์.** Lesson: rtk summarize `next lint` deprecation notice เป็น "Errors: 1" — ของจริงต้องรัน `rtk proxy` ดู (ESLint ✔ clean).
>
> **Follow-up (2026-07-21 PM, same session) — 📓 สมุด digital mode SHIPPED ทั้ง 2 repos (calc → dashboard twin) + prod smoke เขียว:** คุณนุ๊กถามถึงงาน "สมุดเงื่อนไข digital เหมือนหนังสือ" จาก session iPad เช้า → ตรวจแล้ว**ไม่มีร่องรอยใน repo ไหนเลย** (calc: 0 branch/PR/commit ใหม่ · โค้ดยังเขียน "สมุดไม่ใช้ digital") = งานยังไม่เคยเริ่ม → คุณนุ๊ก pin "mirror หนังสือทุกข้อ" ทำใหม่ตั้งแต่ต้น. **calc ([`ecb30af`](https://github.com/witsarutnook/penprinting-calc/commit/ecb30af)):** extract `computeBookDigital` shared helper — computeBook + computeNotebook เรียกฟังก์ชันเดียวกัน = เลข 2 ชนิดงานตรงกันโดย construction (digital ไม่มีเพลท → ข้อแตกต่าง single-plate ของสมุดไม่เกี่ยว); NotebookForm ได้ Digital badge + banner + label mode-aware ครบแบบ BookForm; spec D2 amended. **Verify แน่นตาม convention 7/20:** qty 201 response **byte-identical pre/post** · qty 200 = digital เลขตรงมือ (A5 BW 64น. = 49.50/เล่ม) · baselines 16→**18** (2 cases ใหม่ capture จาก local API: boundary-200-BW 49.50 + A4 color/BW-mixed 306.10) · local parity 19 checks เขียว · **prod smoke จาก deploy จริงเขียว**. **dashboard twin:** staff prompt mode line ครอบสมุด (TDD +1 = **510 tests**) — customer prompt ไม่แตะ by design.
>
> ## ⏳ Pending (2026-07-21 PM)
> 0. ~~📓 smoke สมุด digital~~ ✅ **ผ่าน (same-day, คุณนุ๊ก confirm)** — สมุด digital prod-verified ครบวงจร. **งานสมุด digital = ปิดสมบูรณ์**
...
> ---
>
> **Session 2026-07-21 (☁️ cloud/iPad session) — Audit round เต็ม (perf + bugs, 4 subagents คู่ขนาน) → พบ 3 High + 7 Medium + Low cluster → ปิด High ครบ 3/3 วันเดียว, PR [#21](https://github.com/witsarutnook/penprinting-dashboard/pull/21) merged เข้า main:** คุณนุ๊ก (`/session-start`) สั่ง "audit ความเร็ว + bug" → fan-out 4 subagents (perf-backend · perf-frontend · bugs-write-actions · bugs-auth/AI/track) — ยืนยันของที่ปิดแล้ว hold ครบ (PERF-H1/H2/M1-M3 · M5 · HINT-1 · delta-sync clean · signature verify · middleware matcher) + เปิดใหม่ log ครบใน AUDIT-BACKLOG ([`e79af1e`](https://github.com/witsarutnook/penprinting-dashboard/commit/e79af1e)). คุณนุ๊กเลือก "log ก่อนแล้วทำ H1-H3": **H1** ([`b18b978`](https://github.com/witsarutnook/penprinting-dashboard/commit/b18b978)) `PendingMutationsProvider` ส่ง context value เป็น object literal สด → ทุก Card (~100-200 ใบ) re-render ทุก delta poll ล้ม `arePropsEqual` memo (sibling providers memo ถูกหมด ตัวนี้ตัวเดียวหลุด) → `useMemo` ครอบ value, verify stability chain ทั้งเส้นก่อนแก้ (`pollNow` stable เพราะ `pollOnce` deps `[]`). **H2/H3** ([`887b8e7`](https://github.com/witsarutnook/penprinting-dashboard/commit/887b8e7), TDD 485→**488**) ทุก job transition (forward/ship/cancel/delete) เคยเป็น multi-statement + tombstone unconditional ไม่มี lock → double-click forward = **2 การ์ดจาก job เดียว** · ship∥cancel = id เดียว 2 ตาราง terminal → fix ทั้ง class ด้วย **conditional-tombstone-first gate** (`UPDATE ... WHERE id=? AND phase2_deleted_at IS NULL RETURNING ...` เป็น statement แรก — atomicity ระดับ statement = racing transitions mutual-exclusive, ผู้แพ้ได้ 409/failed[]) + **compensate un-tombstone** เมื่อ terminal INSERT ล้ม; `deleteJob` ยุบเหลือ 1 statement; `cancelJob` ได้ `RETURNING raw` แทน pre-SELECT; mock-postgres เพิ่ม `queueError` ยิง reject ทดสอบ compensation paths. Semantics เปลี่ยนโดยตั้งใจ: retry หลังสำเร็จ = 409 ซื่อสัตย์แทน success เงียบ. **PR #21 squash [`721cdff`](https://github.com/witsarutnook/penprinting-dashboard/commit/721cdff) merged (คุณนุ๊กสั่งในแชต)** → Vercel auto-deploy + post-deploy smoke ยิงเอง. Gates เขียวทุก commit (pre-commit hook: type-check/lint 0-err/488 tests) + build ผ่าน.
>
> ## ⏳ Pending (2026-07-21)
> 1. ~~📲 Smoke บน prod~~ ✅ **ผ่านหมด (same-day, Mac session — คุณนุ๊ก confirm)**: forward ปกติ + double-tap forward รัว → ได้ toast แจ้ง ไม่มีการ์ดซ้ำ (**H2 พิสูจน์บน prod**) + post-deploy smoke workflow เขียว. หมายเหตุจากตอน pin expected: เส้น forward เดี่ยว ([app/api/jobs/forward/route.ts:108](app/api/jobs/forward/route.ts)) คืน **502** + `failed[0].error` ("Job not found or already forwarded — refresh and retry") ไม่ใช่ 409 ตามที่ entry เช้าเขียน — 409 ใช้กับเส้น ship/cancel; พฤติกรรมต่อ user เหมือนกัน
> 2. ~~🟡 Medium 7 ตัวจาก audit ยัง open~~ ✅ **ปิดครบ 7/7 same-day (Mac session PM)** — 6 fixed + /track PIN = accepted WP-parity (คุณนุ๊กตัดสิน). ดู entry 2026-07-21 PM ด้านบน
> 3. 💡 optional carryover เดิม: chip smoke check Meta subscription (task_8243d7bb) · smoke slip-metrics `?channel=`
>
...
> ---
>
> **Session 2026-07-20 — 3 features ship + 1 watch ปิด, ทุกอย่าง prod-verified in-session — pending = 0 🎉:** คุณนุ๊ก (`/session-start`) เลือก A (ปิด watch hint) + งานใหม่ book digital → ตามด้วย ซอง/แฟ้ม + quote-logs viewer ในวันเดียว. **A — hint Messenger watch ปิด:** ครบ 3 วันหลัง suppression ทำงานจริง (7/17→7/20) ไม่มีเคสแทรกผิดที่ → item จบ. **📚 Book digital (superpowers เต็ม: brainstorm D1-D8 → spec → plan → inline execute):** หนังสือ `qty ≤ 200` คิดราคา digital แทน offset — เนื้อในแยกสีต่อชุด (สี = `digitalPricing` เดิม A3=30·A4=15·A5=7.5·ตัด16=15 / **ขาวดำใหม่ `digitalPricingBw`** A3=1.4·A4=0.7·A5=0.5·ตัด16=0.7 — ตัด16 = ราคา A4 ตามคุณนุ๊ก pin) + ปก 2 หน้า×ราคาสีเสมอ + เข้าเล่ม 2.50 เดิม + A2 ผ่าน API → offset guard กัน NaN + กระดาษที่เลือกไม่มีผลราคา. calc 8 commits ([`cdbf824`](https://github.com/witsarutnook/penprinting-calc/commit/cdbf824) spec → [`6cd1192`](https://github.com/witsarutnook/penprinting-calc/commit/6cd1192) foundation → [`b9874c6`](https://github.com/witsarutnook/penprinting-calc/commit/b9874c6) branch → [`c60e88c`](https://github.com/witsarutnook/penprinting-calc/commit/c60e88c) badge UI → [`5d8a3a6`](https://github.com/witsarutnook/penprinting-calc/commit/5d8a3a6) baselines 2 ใหม่ boundary-200-BW + color/BW-mixed → [`77d2b74`](https://github.com/witsarutnook/penprinting-calc/commit/77d2b74) CLAUDE.md) — verify: local API เลขเป๊ะ (29.50 / 177.50 / 1,532.50) + **qty 201 response IDENTICAL byte-ต่อ-byte กับก่อนแก้** + **prod smoke 17/17 เขียวจาก deploy จริง** + dashboard prompt twin [`2fda57f`](https://github.com/witsarutnook/penprinting-dashboard/commit/2fda57f) (mode ครอบหนังสือ + ห้าม AI ทักราคา digital ว่าผิดปกติ, TDD 471) + **คุณนุ๊ก smoke มือถือ + น้อง PP ผ่าน same-day**. **📦 ซอง/แฟ้ม:** [`72f0b04`](https://github.com/witsarutnook/penprinting-dashboard/commit/72f0b04) — `QTY_UNITS` order-form จุดเดียวครอบทุก flow. **🔍 Quote Logs Viewer (brainstorm 4 decisions [admin-only · tag ระดับข้อความ · ทุกช่องทาง · หน้าใหม่] → spec [`f16691f`](https://github.com/witsarutnook/penprinting-dashboard/commit/f16691f) → plan [`cf38448`](https://github.com/witsarutnook/penprinting-dashboard/commit/cf38448) → inline 5 tasks, TDD 471→485):** `/quote-logs` ดูบทสนทนาลูกค้า↔น้อง PP ย้อนหลังทุก session + tag 🚩 "AI ตอบผิด" + โน้ต = worklist ปรับ prompt. [`df7b4b4`](https://github.com/witsarutnook/penprinting-dashboard/commit/df7b4b4) per-turn `ts` ผ่าน `mkTurn` stamp ทุก persist path (LINE/Messenger ครบแน่นอน — turn เก่าไม่มีเวลา by design) · [`b9033fb`](https://github.com/witsarutnook/penprinting-dashboard/commit/b9033fb) ตาราง `ai_quote_turn_flags` (**snapshot role+text กัน index drift** — sanitizeHistory trim 40 turns ได้) + `lib/ai-quote/logs.ts` single-query list + `mergeTimeline` pure fn · [`57c1011`](https://github.com/witsarutnook/penprinting-dashboard/commit/57c1011) flags API admin+audit · [`524d3d0`](https://github.com/witsarutnook/penprinting-dashboard/commit/524d3d0) pages + FlagButton + nav "AI Logs" + middleware · [`fad8453`](https://github.com/witsarutnook/penprinting-dashboard/commit/fad8453) cross-link จาก /quote-leads · [`d77c18b`](https://github.com/witsarutnook/penprinting-dashboard/commit/d77c18b) counts. **Prod-verified โดยคุณนุ๊ก:** db-migrate applied (ตาราง flags + 30 sessions/28 quotes โชว์) + หน้าใช้งานจริง. **Lesson:** คุณนุ๊กกด db-migrate เร็วกว่า Vercel สลับ deployment → old code รัน migration เงียบๆ (applied list ไม่มีบรรทัดใหม่ = จับได้) → รอ smoke เขียวก่อนแล้วกดซ้ำ ([[feedback_db_migrate_stale_deploy]]).
>
> ## ⏳ Pending (2026-07-20)
> **ไม่มี** — ทุก feature prod-verified + smoke ผ่านครบใน session 🎉 (item เฝ้าระวังเดิมปิดหมด: hint Messenger watch จบ)
>
> ---
>
