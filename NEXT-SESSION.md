...
> 1. 🧪 **Smoke perf fixes บน prod** (คุณนุ๊ก): ① bulk forward 2-3 งานพร้อมกันจาก /board bulk-mode — การ์ดย้ายครบ + history tab ของงานใหม่มี audit entry ครบทุกใบ (audit batch ใหม่) + ลองสังเกตว่าเร็วขึ้น ② forward เดี่ยว 1 ครั้ง + undo 1 ครั้ง — flow เดิมปกติ (เส้นเดียวกับ batch) ③ คุยกับน้อง PP ในโหมด AI 1 turn — ตอบปกติ + `/quote-logs` เห็น turn ใหม่ (persist parallel ไม่หล่น)
