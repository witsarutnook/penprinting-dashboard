...
> 2. ✅ **Apps Script deployment archived + trigger ลบแล้ว (คุณนุ๊ก, 2026-09-05 เย็น)** — Apps Script อยู่นอก live path 100% แล้ว (โค้ด dashboard ไม่เรียก + env ถอด + deployment archive) · เหลือแค่ลบโปรเจกต์หลัง soak — ~~เดิม: 🗄 **คุณนุ๊ก — Apps Script editor** (scriptId ใน~~ `production-monitoring/apps-script/dashboard/.clasp.json`): Deploy → Manage deployments → **Archive** web-app deployment ของ dashboard · Triggers → ลบ `archiveOldData` ถ้ามี · **ยังไม่ลบโปรเจกต์** (rollback path ระหว่าง soak)
