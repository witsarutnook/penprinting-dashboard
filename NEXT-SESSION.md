...
_อัปเดตล่าสุด: 2026-05-09 mega-day — Phase 3.6 cutover ✅ DONE. WP retired via DNS switch HostAtom A→CNAME Vercel. app.penprinting.co alias dashboard.penprinting.co (same Vercel project). 9 hard-coded WP refs แก้ครบ (3 v2 + 4 marketing redirects + 2 Morning Report mirrors). Vercel `/production-monitoring*` → `/board` cushion. **Bonus perf bug** เจอ via static analysis — `createOrder` missing from PATHS_BY_ACTION map → fast-path order create stale 15-60s. Fix `e88f386` 1 บรรทัด. **Bonus quota tune** — bump warn 3000→5000ms cold-start friendly. 5 commits + 2 Apps Script pushes + DNS cutover. Monitor 1 สัปดาห์._
