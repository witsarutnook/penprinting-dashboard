...
> 2. 🗄 **คุณนุ๊ก — Apps Script editor** (scriptId ใน `production-monitoring/apps-script/dashboard/.clasp.json`): Deploy → Manage deployments → **Archive** web-app deployment ของ dashboard · Triggers → ลบ `archiveOldData` ถ้ามี · **ยังไม่ลบโปรเจกต์** (rollback path ระหว่าง soak)
