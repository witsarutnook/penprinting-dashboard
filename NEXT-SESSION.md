...
>
> **Session 2026-08-20 (Mac) — 🗑 Decommission decisions ตัดสินครบ + DATA-orphan-cancelled ×4 ปิดฝั่ง Postgres end-to-end (TDD 631→636 tests · [`863c3d2`](https://github.com/witsarutnook/penprinting-dashboard/commit/863c3d2) · applied on prod + verified same-session):** คุณนุ๊ก (`/session-start`) เลือก "ตัดสิน decommission" → ผมสรุป evidence + rollback story → คุณนุ๊กตัดสิน 2 เรื่อง: **(1) ตัด LINE-webhook Apps Script + CF Worker** (นอก live path ตั้งแต่ 6/30, ตรวจสดพบ worker ยังตอบ 200 พร้อม secrets สด) → runbook [`../production-monitoring/RUNBOOK-decommission-line-worker.md`](../production-monitoring/RUNBOOK-decommission-line-worker.md) ส่งให้แล้ว — **รอคุณนุ๊กกดใน console** (source อยู่ใน repo คืนชีพได้ ~30-60 นาที) **(2) ลบ orphan ทั้ง 4** → endpoint ใหม่ `/api/admin/cleanup-orphan-cancelled` (pattern fix-date-anomaly: dry-run + `?apply=1`, ONE conditional-gated DELETE `NOT EXISTS`+RETURNING, audit_log, cache bust). Ground truth ผ่าน prod API ก่อนลงมือ (DB URL Sensitive ดึงตรงไม่ได้ — ใช้ Chrome คุณนุ๊ก): 4 rows ใน Postgres จริง ids 142/218/395/463, order แม่ไม่มีจริง. **Applied on prod: deleted ×4 → rescan 0 (idempotent) → delta cancelled 33→29**. Gates เขียวครบ + smoke เขียว pin `863c3d2` (gotcha: `gh run list --commit` มองไม่เห็น deployment_status runs — match `headSha` จาก `--json` แทน).
>
> ## ⏳ Pending (2026-08-20)
> 1. 🗑 **คุณนุ๊กทำ decommission ตาม runbook** — A: Cloudflare delete worker `penprinting-line-gateway` · B: GAS archive deployment โปรเจกต์ **LINE Webhook** (⚠️ ห้ามแตะโปรเจกต์ Dashboard — ยังใช้ `searchArchive`) → เสร็จแล้วแจ้ง Claude verify + กวาด docs (monitoring.md / CLAUDE.md ×2 / check-quota command ที่ยังอ้าง worker)
