...
> 3. **LINE จังหวะ 2 (สรุปจากคำถามคุณนุ๊กท้าย session)**: 💬 **เปิด `AI_QUOTE_LINE_HINT_ENABLED=true` + redeploy** (แนะนำทำก่อน — LINE ยังไม่มี hint ขณะที่ Messenger พิสูจน์ flow กับคนจริงแล้ว, copy ใหม่ shared รออยู่) · 🎨 rich menu รอรูปดีไซน์จากคุณนุ๊ก (kit `../line-oa-richmenu/` พร้อม — ได้รูป → Claude remap JSON → คุณนุ๊กรันสคริปต์) · smoke LINE ที่เหลือ (④ "สั่งเลย" + /track/สลิประหว่างโหมด) · revert gate "/" เมื่อพร้อม (shared 2 channel — ลูกค้าพิมพ์ "ขอราคา" ธรรมดาเข้าโหมดได้)
