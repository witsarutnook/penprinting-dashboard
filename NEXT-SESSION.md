...
>
> ## ⏳ Pending — 6/23 ทั้งหมด + verify ใหม่
> 1. **Merge PR [#2](https://github.com/witsarutnook/penprinting-dashboard/pull/2)** — ตอนนี้รวม prompt-tuning ด้วยแล้ว (`237b66d` commit บนสุด). คุณนุ๊กถือ merge ไว้รอ prompt-tuning เสร็จ → **ตอนนี้ push เสร็จแล้ว merge ได้**.
> 2. **Live behavior verify (ใหม่)** — prompt **content** ผ่าน test แล้ว แต่พฤติกรรมจริง (Haiku clarify น้อยลงจริงมั้ย) เป็น model behavior → **ต้อง smoke บน preview/prod หลัง deploy**. เทสต์: พิมพ์งานที่เคยถามเยอะ เช่น *"ใบปลิว 1000 ใบ"* (ไม่บอกกระดาษ/สี/หน้า) → ควร **ตีราคาเลย** + โชว์ *"📋 ประเมินจาก: A4 / 4 สี / 2 หน้า / Art 120 …"* แทนที่จะถามกลับ. งานหนังสือ → ควรถาม qty+หน้า+กระดาษ+สี รวมครั้งเดียว.
>
> ---
>
