...
>
> ~~[MEMORY-QUEUE]~~ **[MEMORY-QUEUE] (2026-07-23 — session เครื่อง Mac ถัดไป save เข้า memory จริงแล้วลบ tag):**
> 1. [[feedback_external_api_classify_fail_open]] (`external-api-contract-pin-from-raw`) — classification ที่เช็ค field ของ external API แบบ `=== false` จะ **fail-open เงียบ** เมื่อ API ไม่ส่ง field นั้น (Thunder v2 ใช้ `matchedAccount` แทน `isAccountMatched` → สลิปผิดบัญชีทุกใบได้ ✅ ตั้งแต่ ship): ก่อนเขียน/แก้ classify ต้อง capture raw response จริงลง DB ถาวร + ต้องมี **positive control** (เคสที่ควร match จริง) ก่อน flip logic — ไม่งั้น flip แล้วของดีทุกใบพังหมด
> 2. [[feedback_llm_gate_persist_answer]] (`llm-prefilter-observability`) — LLM gate ที่ drop เงียบ (pre-filter) ต้อง persist คำตอบดิบของ model ทุกครั้ง ไม่งั้น false-no วินิจฉัยไม่ได้เลย (สลิป 580 เงียบ 4 ครั้งโดยไม่มีร่องรอย) + คำใน no-list ของ prompt ชนกับข้อความ**ในเนื้อรูป**ได้ ("สติกเกอร์" vs memo "sticker") + เขียน evidence row **ก่อน** ยิง reply เสมอ + reproduce ผ่าน agent harness ≠ raw API (คนละ verdict ได้)
>
> ---
>
